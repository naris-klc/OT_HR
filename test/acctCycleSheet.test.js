...
test('ใบเดือนเดียวยังเป็น 24 / 58 / 19 / 19 / 74 — รวม 194mm', () => {
  const widths = widthsIn(sourceOf(SHEET));
  // ชุดหลังใน colgroup คือสาขาเดือนเดียว: ห้าช่อง ปิดท้ายด้วยแถบขาว 74mm
  const single = widths.slice(-5);
  // เคยเป็น [24, 54, 17, 17, 82] ตอนใบนี้เป็น 9pt — 2026-09-14 ตัวอักษรโตขึ้น
  // 2px ชื่อกับแบนเนอร์เดือนจึงกินความกว้างเพิ่ม และแถบขาวเป็นคนจ่ายให้
  assert.deepEqual(single, [24, 58, 19, 19, 74]);
...
test('ใบสองเดือนบีบคอลัมน์ ไม่ขยายกระดาษ — ยังรวม 194mm พอดี', () => {
  ...
  // เคยเป็น [22, 50, 50] คู่กับช่องอัตรา 12mm ตอนใบนี้เป็น 9pt
  assert.deepEqual(
    [code, name, note],
    [22, 57, 31],
  ...
  assert.deepEqual(
    [rate15, rate3, sum15, sum3],
    [14, 14, 14, 14],
    'ช่องอัตราของใบสองเดือน — `120.00` ที่ 10.5pt กว้าง 12.12mm และ td.n ใน '
      + 'app/print.css เหลือ padding ข้าง 0.8mm ให้มันพอดีในช่อง 14mm',
  );
...
test('แถบหมายเหตุยังกว้างพอสำหรับคำว่า วันเกิด', () => {
  ...
  assert.ok(
    noteMany >= 30,
    `แถบขาวเหลือ ${noteMany}mm — “วันเกิด” ที่ 10.5pt กว้าง 9.78mm บวก padding `
      + `ซ้าย 3mm และแถบนี้ nowrap`,
  );
});
...
test('ใบสองเดือนย่อชื่อเดือน — เต็มคำจะตกบรรทัดในแบนเนอร์ 28mm', () => {
  const code = sourceOf(SHEET);
 ...
 * ส่วนชื่อจดทะเบียนคือสิ่งเดียวบนแถวที่มองแล้วเดาไม่ได้ และนี่คือตารางที่ไปอยู่บน
 * หน้าปกที่ส่งให้บัญชี
 *
 * ⚠ เหตุผลข้อนี้เคยมีอีกครึ่งหนึ่งว่า *"และการ์ดของบริษัทนั้นก็แบกมันเป็น kicker
 * อยู่ข้างล่าง"* — **ตกไปแล้วเมื่อ 2026-09-11 ตอนบ่ายวันเดียวกัน** ขอมาว่า
 * *"เอาคำว่า บริษัทที่ ออกจากหัวตาราง"* kicker นั้นจึงถูกถอดทั้งบรรทัด พร้อมกับ
 * พร็อพ `index` และการคำนวณ `findIndex` ที่ป้อนมัน · **ตอนนี้ไม่มีที่ไหนในแอป
 * พูดลำดับของบริษัทอีกแล้ว** ซึ่งเป็นสิ่งที่ตั้งใจ ไม่ใช่ของตกหล่น: เลขลำดับ
 * มีความหมายได้ก็ต่อเมื่อมีรายการให้นับ และบนแท็บบริษัทเดียวไม่มีรายการนั้น
 */
test('จอส่งบัญชี — แถวบริษัทในตารางรวม เหลือบรรทัดเดียว', () => {
  ...
  // และทั้งจอก็ไม่พูดลำดับที่ไหนอีก — kicker ถูกถอดเมื่อ 2026-09-11 ดูหัวข้อข้างบน
  assert.doesNotMatch(code, /บริษัทที่ \{index\}/, 'kicker บริษัทที่ N กลับมาแล้ว');
  assert.doesNotMatch(code, /index=\{/, 'พร็อพ index กลับเข้ามาโดยไม่มีใครใช้');
  assert.doesNotMatch(code, /function CompanySheet\(\{ company, periods, index \}\)/);
...
/**
 * ขนาดตัวอักษรของใบส่งบัญชี กับความกว้างของคอลัมน์ เป็นตัวเลขชุดเดียวกัน
 *
 * `colgroup` ข้างบนไม่ได้เลือกมาให้สวย มันถูกวัดจากข้อความจริงที่ขนาดจริง —
 * ชื่อที่ยาวที่สุดในทะเบียน 52.4mm, `เดือนพฤศจิกายน 69` 33.78mm และ `120.00`
 * 12.12mm ล้วนเป็นตัวเลขที่ 10.5pt · ใครลดฟอนต์กลับเป็น 9pt แล้วปล่อยความกว้าง
 * ไว้ จะได้ใบที่ยังพิมพ์ออกมาถูกทุกตัวเลขแต่โปร่งผิดที่ ส่วนคนที่ขยับขึ้นอีกโดย
 * ไม่แตะความกว้าง จะได้ชื่อที่ถูกจุดไข่ปลาตัดและแบนเนอร์เดือนที่ตกบรรทัด ซึ่ง
 * ทำให้ `ROWS_PER_PAGE` ผิดทั้งใบ
 *
 * เทสต์นี้จึงไม่ได้ตรึง "10.5pt เป็นขนาดที่ถูก" มันตรึงว่า **ขนาดกับความกว้าง
 * ต้องถูกแก้พร้อมกัน** — ความกว้างข้อความโตตามขนาดฟอนต์แบบเชิงเส้นพอดี (วัดด้วย
 * Chrome 2026-09-14 ได้อัตราส่วน 1.1666 = 10.5/9 เป๊ะ) การคำนวณใหม่จึงเป็นการ
 * คูณ ไม่ใช่การเดา
 */
test('ใบส่งบัญชียังเป็น 10.5pt ทั้งใบ — colgroup ข้างบนถูกวัดที่ขนาดนี้', () => {
  const css = readFileSync(join(ROOT, 'app/print.css'), 'utf8');

  const cell = css.match(/\.acct th, \.acct td \{[\s\S]*?font: 400 ([\d.]+)pt\/1\.25/);
  assert.ok(cell, 'ไม่พบ font shorthand ของเซลล์ใบส่งบัญชีใน app/print.css');
  assert.equal(
    cell[1],
    '10.5',
    'ขนาดตัวอักษรในเซลล์เปลี่ยนแล้ว — ต้องคำนวณ colgroup ใน '
      + `${SHEET} ใหม่ทั้งสองชุดในคอมมิตเดียวกัน`,
  );

  const sheet = css.match(/\.acct \{[\s\S]*?font-size: ([\d.]+)pt;/);
  assert.ok(sheet, 'ไม่พบ font-size ของ .acct');
  assert.equal(sheet[1], '10.5', 'ตัวอักษรฐานของแผ่นกับของเซลล์ต้องเป็นขนาดเดียวกัน');

  // ช่องตัวเลขได้ padding ข้างแคบกว่าทั้งใบ เพื่อให้ยอดสามหลักอยู่ในช่อง 14mm ได้
  assert.match(css, /\.acct td\.n \{ padding-left: 0\.8mm; padding-right: 0\.8mm; \}/);
});
