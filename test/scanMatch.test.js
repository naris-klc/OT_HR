...
test('ป้ายทั้งสี่ใช้คำของฝ่ายบุคคล ไม่ใช่คำของเราเอง', () => {
  ...
  // ⚠ อันนี้เคยเป็น 'ไม่ตรง' — คำของฝ่ายบุคคลเองเมื่อ 2026-09-07 — และถูกเปลี่ยน
  // เป็น 'ไม่ได้สแกน' เมื่อ 2026-09-11 · เหตุผลอยู่ใน `SCAN_BADGE`: ไม่ตรง เป็น
  // คำ *เปรียบเทียบ* แต่สถานะนี้คือสถานะเดียวที่ไม่มีของอีกฝั่งให้เทียบเลย คนที่
  // เพิ่งอ่านว่า ไม่ครบ = สแกนออกก่อนเวลา จะอ่าน ไม่ตรง เป็นดีกรีที่สามของเรื่อง
  // เดียวกัน ซึ่งมันไม่ใช่ · ป้ายนี้เป็นสีแดงพร้อมกับคำ (`.chip.scan-none`) ส่วน
  // ไม่ครบ ยังเป็นส้มตามเดิม
  assert.equal(SCAN_BADGE.NO_SCAN, 'ไม่ได้สแกน');
  // คำที่ห้าไม่ใช่ของฝ่ายบุคคล และตั้งใจให้ยาวกว่า — มันคือฝั่งเวลาเริ่มโดยเฉพาะ
  // เหตุผลเดิมคือ "ถ้าเรียกมันว่า ไม่ตรง จะชนกับคำที่แปลว่า ไม่มีสแกนเลย เต็ม ๆ"
  // — การชนนั้นหมดไปแล้วตั้งแต่ 2026-09-11 คำนี้ยาวต่อเพราะความหมายของมันเอง
