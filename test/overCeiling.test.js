...
test('แผ่นยืนยันเตือนเรื่องเพดานกล่องเดียว และไม่พูดถึงการยกเว้นเพดานอีก', () => {
  ...
  /**
   * ⚠ จับเป็นตัวอักษรตรง ๆ ตรงนี้จนถึง 2026-09-12 — อ่านว่า `นำไปแสดงบนรายงาน
   * สรุป OT ส่งบัญชี` เพราะแผ่นนี้พิมพ์ประโยคนั้นเอง และแผ่นไม่อนุมัติก็พิมพ์
   * เองอีกฉบับ คนละสำนวน เทสต์จึงจับได้ฉบับเดียวโดยไม่มีอะไรบอกว่าอีกฉบับมีอยู่
   * ตอนนี้ทั้งสองแผ่นอ่านจาก `OVER_CEILING_REASON_SAY` ใน lib/caps.js — ตรงนี้
   * ตรวจว่าแผ่นนี้ขอประโยคนั้นมา ส่วนเทสต์ในไฟล์เดียวกันข้างล่างตรวจว่าไม่มี
   * แผ่นไหนพิมพ์เองอีก
   */
  assert.match(confirmModal, /\{OVER_CEILING_REASON_SAY\}/);
...
test('ทั้งสองแผ่นบอกที่ที่เหตุผลไปโผล่ ด้วยประโยคเดียวกัน ไม่ใช่คนละสำนวน', () => {
  /**
   * ── ⚠ มันเป็นสองฉบับ ในไฟล์เดียวกัน จนถึง 2026-09-12 ──────────────────────
   *
   * แผ่นยืนยันอนุมัติเขียนว่า "เหตุผลที่ระบุจะถูกบันทึกไว้ในประวัติของใบคำขอ
   * และนำไปแสดงบนรายงานสรุป OT ส่งบัญชี (ตรงตัวเลขชั่วโมงของพนักงานคนนี้)"
   * แผ่นไม่อนุมัติซึ่งอยู่ห่างไปราวร้อยบรรทัดเขียนว่า "เหตุผลที่กรอกด้านล่างจะ
   * ถูกบันทึกไว้ในประวัติของใบ และแสดงบนสรุป OT ส่งบัญชี ตรงตัวเลขชั่วโมงของ
   * คนนี้ด้วย" — ข้อเท็จจริงเดียวกัน คนละคำ คนละขนาดตัวอักษรด้วย เพราะฉบับ
   * หนึ่งอยู่ใน `.say` อีกฉบับอยู่ใน `marginTop` เปล่า ๆ
   *
   * ยังไม่มีอะไรพังตอนนั้น และนั่นคือเหตุผลที่ต้องมีเทสต์ตัวนี้: วันที่ใครสัก
   * คนแก้ประโยคนี้ให้ถูกขึ้น เขาจะแก้ฉบับที่เขาเปิดอยู่ฉบับเดียว แล้วอีกฉบับจะ
   * ยืนอยู่ต่อไปโดยไม่มีอะไรร้อง
   *
   * เทสต์นี้จึงไม่ได้ตรวจคำ แต่ตรวจ*จำนวนแหล่ง*: ประโยคต้องมาจาก lib/caps.js
   * และไฟล์จอต้องไม่มีชิ้นส่วนของมันเป็นตัวอักษรอยู่เลย
   */
  const queue = strip(read('components/ApprovalQueue.jsx'));
  assert.equal(
    (queue.match(/\{OVER_CEILING_REASON_SAY\}/g) || []).length,
    2,
    'แผ่นยืนยันอนุมัติกับแผ่นไม่อนุมัติต้องอ่านจากประโยคเดียวกันทั้งคู่',
  );
  // ชิ้นส่วนที่เป็นของประโยคนี้เท่านั้น — `บันทึกไว้ในประวัติของใบ` เฉย ๆ ใช้
  // ไม่ได้ เพราะกล่อง "ใบนี้ไม่มีหัวหน้าแผนกที่เซ็นได้" บนแผ่นเดียวกันก็พูด
  // ประโยคที่ขึ้นต้นแบบนั้น แต่มันเป็นคนละเรื่อง (การเซ็นแทนขั้นหัวหน้า)
  for (const piece of ['ประวัติของใบคำขอ', 'สรุป OT ส่งบัญชี', 'ตรงตัวเลขชั่วโมงของ']) {
    assert.ok(
      !queue.includes(piece),
      `"${piece}" ถูกพิมพ์เป็นตัวอักษรในไฟล์จออีกแล้ว — ประโยคนี้อยู่ที่ lib/caps.js`,
    );
  }
  // และที่ lib/caps.js ประโยคนั้นมีอยู่จริง ไม่ใช่ชื่อที่ import มาแล้วว่าง
  assert.match(
    read('lib/caps.js'),
    /export const OVER_CEILING_REASON_SAY = 'เหตุผลที่ระบุจะถูกบันทึกไว้ในประวัติของใบคำขอ '/,
  );
  // สองแผ่นเดียวกันต้องดูเหมือนกันด้วย ไม่ใช่แค่พูดเหมือนกัน
  assert.equal(
    (queue.match(/<div className="say">\{OVER_CEILING_REASON_SAY\}<\/div>/g) || []).length,
    2,
    'ประโยคเดียวกันต้องอยู่ในชั้นเดียวกันทั้งสองแผ่น',
  );
});
...
test('ตัวเลขที่เซ็นเกินเพดาน ขึ้นแดงบนใบที่พิมพ์ ไม่ใช่แค่บนจอ', () => {
  ...
  assert.match(sheet, /figureClass\(/, 'ช่องอัตราต้องอ่านสถานะเกินเพดาน');

  /**
   * ── ตั้งแต่ 2026-09-10 ใบมีได้สองเดือน และคำถามนี้ยังเป็นคำถามเดิม ──────────
   *
   * เดิมข้อนี้นับ `figureClass(row)` ให้ได้ 2 พอดี คือ 1.50 กับ 3.00 ของใบเดือน
   * เดียว ตอนนี้งวดจ่ายมีได้ถึงสองเดือน (พ.ย. + ธ.ค. จ่ายรวมในมกราคม) ใบจึงมีคู่
   * อัตราต่อเดือนแล้วปิดท้ายด้วยคู่รวม — สิ่งที่ต้องยังจริงคือ **คู่ ไม่ใช่ช่อง
   * เดียว** ทั้งสองที่:
   *
   *   คู่รายเดือน   อ่าน `figureClass(m)` — เดือนของตัวเอง จึงชี้ได้ว่าเดือนไหน
   *   คู่รวม        อ่าน `figureClass(row)` — ทั้งงวด
   */
  assert.equal(
    (sheet.match(/className=\{figureClass\(m\)\}/g) || []).length,
    2,
    'คู่รายเดือนต้องเป็นทั้ง 1.50 และ 3.00',
  );
  assert.equal(
    (sheet.match(/className=\{`\$\{figureClass\(row\)\} sum`\}/g) || []).length,
    2,
    'คู่รวมต้องเป็นทั้ง 1.50 และ 3.00',
  );
  assert.ok(sheet.includes("scope.overCeiling?.count ? 'n over' : 'n'"));
