...
/**
 * LINE ENDINGS NORMALISED BEFORE ANYTHING BELOW READS A CHARACTER — the same
 * fix, for the same reason, as the header of test/adminApproval.test.js and the
 * note in .gitattributes.
 *
 * Every assertion here is a regex with `\n` in it, matched against a file read
 * off disk. `core.autocrlf` is true on the machine this was written on but the
 * suite ran green there anyway, because the FIRST run happened before the file
 * had been checked out again — so all five multi-line assertions in this file
 * silently missed on the next CRLF checkout, against a tree with nothing wrong
 * in it. `SECTION_RE` parsed 0 sections and said so, which is the only reason
 * it was noticed rather than passing vacuously.
 */
