...
/**
 * ── กล่องรหัสผ่านแรกเข้าพูด "เข้าใช้งานได้ทันที" ครั้งเดียว ───────────────────
 *
 * ⚠ มันพูดสองครั้งจนถึง 2026-09-12 — ในกล่องเดียวกัน ห่างกันประมาณหนึ่งฝ่ามือ
 *
 * บรรทัดหัวลงท้ายว่า `ทุกคนเข้าใช้งานได้ทันที และจะมีแถบเตือนให้ตั้งรหัสของ
 * ตัวเองจนกว่าจะเปลี่ยน` แล้วใต้ตารางก็เขียนอีกว่า `ทุกบัญชีเข้าใช้งานได้ทันที
 * และจะเห็นแถบเตือนให้ตั้งรหัสผ่านของตัวเองจนกว่าจะเปลี่ยน` — ข้อเท็จจริง
 * เดียวกัน คนละคำ (ทุกคน/ทุกบัญชี · จะมี/จะเห็น · รหัส/รหัสผ่าน) โดยไม่มีอะไร
 * คั่นกลางที่ทำให้ครั้งที่สองเป็นคนละเรื่อง คนอ่านที่สังเกตว่าคำไม่เหมือนกัน
 * ต้องเสียเวลาตรวจว่ามันต่างกันที่ความหมายหรือเปล่า
 *
 * ท่อนที่ไม่ใช่การพูดซ้ำคือท่อนที่เหลืออยู่: รหัสผ่านถูกเก็บแบบเข้ารหัสทางเดียว
 * จึงไม่มีหน้าไหนเปิดตารางนี้ซ้ำได้ — ไม่ได้พูดไว้ที่อื่นบนจอนี้เลย และมันคือ
 * เหตุผลที่ปุ่มพิมพ์/คัดลอก/CSV เหนือมันเป็นนาทีเดียวที่ได้ใช้
 */
test('กล่องรหัสผ่านแรกเข้าไม่พูดเรื่องแถบเตือนซ้ำสองรอบในกล่องเดียว', () => {
  const admin = strip(readFileSync(join(ROOT, 'components/AdminView.jsx'), 'utf8'));
  const box = admin.slice(
    admin.indexOf('<strong>รหัสผ่านแรกเข้าของ'),
    admin.indexOf('<strong>รหัสผ่านแรกเข้าของ') + 3000,
  );
  assert.ok(box.includes('</Alert>'), 'หากล่องไม่เจอ — เทสต์นี้กำลังตรวจของว่าง');
  const alert = box.slice(0, box.indexOf('</Alert>'));
  assert.equal(
    (alert.match(/เข้าใช้งานได้ทันที/g) || []).length,
    1,
    'กล่องเดียวพูดเรื่องเข้าใช้งานได้ทันทีสองรอบอีกแล้ว',
  );
  assert.equal((alert.match(/แถบเตือน/g) || []).length, 1);
  // และท่อนที่ไม่ซ้ำยังอยู่ ไม่ได้ถูกลบไปพร้อมกับท่อนที่ซ้ำ
  assert.match(alert, /ระบบเก็บรหัสผ่านแบบเข้ารหัสทางเดียว จึงไม่มีหน้าใดแสดงรายการนี้ซ้ำได้/);
  // บรรทัดที่เงียบกว่านั้นใช้ชั้นเดียวกับทั้งแอป ไม่ได้เขียนขนาดตัวอักษรเอง
  assert.match(alert, /<div className="say">/);
});
