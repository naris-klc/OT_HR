...
/**
 * ปุ่มในคอลัมน์ตาราง ชิดขวาเสมอ — สั่งไว้ 2026-09-11.
 *
 * *"การวางปุ่มในคอลัมน์ตารางให้จัดชิดขวาเสมอ"* แจ้งมาจาก ตรวจสอบประจำเดือน
 * ซึ่งคอลัมน์ปุ่มกว้าง 104px เผื่อปุ่มสองตัว แต่ตอนนั้นวาดตัวเดียว ชิดซ้าย
 * เหลือที่ว่างท้ายคอลัมน์ 40px
 *
 * คอลัมน์ปุ่มกว้างเท่ากับแถวที่วาดปุ่มมากที่สุด แถวที่วาดน้อยกว่านั้นจึงเหลือช่องว่าง
 * เสมอ และทุกตารางในแอปมีแถวแบบนั้น — `.own-note` บนคิวรออนุมัติยืนแทนปุ่มสองตัว
 * แถวที่แก้ไขไม่ได้บนทะเบียนพนักงานวาดสองในสาม · ชิดซ้ายแล้วปุ่มของแต่ละแถวจะยืน
 * คนละตำแหน่งไล่ลงมา ชิดขวาแล้วมันตรงกันหมดไม่ว่าแถวไหนถือกี่ปุ่ม
 *
 * `td` ไม่ใช่คลาสเปล่า — `.row-actions` เป็นคลาสกลางที่ใช้นอกตารางก็ได้ และของที่
 * ไม่ได้อยู่ในตารางไม่มีขอบขวาให้หมายถึงอะไร · สิ่งที่สั่งคือ "คอลัมน์ตาราง"
 */
test('ปุ่มในเซลล์ของตาราง ชิดขวาเสมอ — ทุกตารางในแอป', () => {
  assert.match(css, /^td \.row-actions \{ justify-content: flex-end; \}$/m);

  // ไม่ผูกกับตารางใดตารางหนึ่ง: ทั้งสี่คอมโพเนนต์ที่ใช้คลาสนี้วางมันไว้ใน <td>
  // ถ้ามีตัวไหนย้ายออกไปนอกตาราง กฎนี้จะเลิกมีผลกับมันโดยไม่มีใครรู้
  for (const f of ['components/HrView.jsx', 'components/ApprovalQueue.jsx',
    'components/AdminView.jsx', 'components/EmployeeView.jsx']) {
    assert.match(read(f), /className="(row )?row-actions/, `${f} เลิกใช้ .row-actions แล้ว`);
  }

  /* ⚠ ใต้ 860px กฎนี้ไม่มีผล และตั้งใจให้เป็นแบบนั้น — ตารางกลายเป็นการ์ด และปุ่ม
     ถูกสั่ง `flex: 1 1 …` ให้เต็มแถว ไม่เหลือที่ว่างให้ `justify-content` แจก
     จึงไม่ต้องมี breakpoint ของตัวเอง */
  assert.match(css, /\.queue-table td\.act-col \.row-actions \.btn \{ flex: 1 1 40%;/);
});
