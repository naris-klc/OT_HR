...
import { api, thaiDate, dayName } from '../api.js';
...
function Employees() {
  ...
  const [form, setForm] = useState({ code: '', name: '', position: '', department: '', role: 'employee', password: '' });
  const [error, setError] = useState('');
  ...
  async function create(e) {
    ...
    try {
      ...
      setForm({ code: '', name: '', position: '', department: '', role: 'employee', password: '' });
      load();
    } catch (err) { setError(err.message); }
  }
  ...
  return (
    <div className="card">
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr><th>รหัส</th><th>ชื่อ-สกุล</th><th>ตำแหน่ง</th><th>แผนก</th><th>บทบาท</th><th>สถานะ</th></tr>
          </thead>
          <tbody>
            {rows.map((p) => (
              <tr key={p._id}>
