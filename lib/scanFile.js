...
export function buildScanSlots(companies = [], batches = []) {
  ...
  const unplaced = batches.filter((b) => !placed.has(String(b._id)));
  ...
  return {
    ...
    unplaced,
    /**
     * บริษัทที่งวดนี้ยังไม่มีไฟล์ของเขาสักเครื่อง — WHOSE ROWS HAVE BEEN
     * COMPARED AGAINST NOTHING AT ALL.
     *
     * `missing` one line up is SLOT-level and is the import card's question:
     * *which of the four files is still outstanding*. This is COMPANY-level and
     * is a different question with a different consequence — a person whose
     * company brought no file at all is not a person the machine disagreed
     * with, and marking their rows `ไม่มีสแกน` says they did not scan when the
     * truth is that nobody imported their terminal. Asked for on 2026-09-11:
     * *"ทำให้แถวคนเดมเทคขึ้นเทา ยังไม่นำเข้า และติ๊กได้ตามปกติ เหมือนกรณีไม่มี
     * ไฟล์เลย"*. See docs/plan-monthly-partial-scan-import.md.
     *
     * A COMPANY WITH ONE OF ITS TWO MACHINES IN COUNTS AS IMPORTED. Nothing on
     * an ใบ OT says which terminal a person uses, so calling that company
     * un-compared would turn rows that SHOULD be red grey — the opposite
     * mistake and the more dangerous one. The empty slot is still named, in the
     * import card, out of `missing`.
     *
     * ⚠ AN `unplaced` BATCH EMPTIES THIS LIST. A file whose company is `mixed`
     * or never resolved fills no slot, yet its punches are in the database and
     * ARE compared. It could hold anybody's rows, so no company can be called
     * un-imported while one exists; the honest answer is to claim nothing and
     * behave exactly as this screen did before the list existed.
     */
    notImported: unplaced.length
      ? []
      : companies
        .map((c) => c.key)
        .filter((key) => !slots.some((s) => s.company === key && s.batch)),
  };
}
