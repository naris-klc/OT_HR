...
const ACTIONS = [
  ...
  // The read the ลบแผนก dialog makes before it offers the button: it asks what
  // points at the row, and the answer decides which of two dialogs opens.
  ['GET', '/departments/:id', 'ดูแผนกและสิ่งที่อ้างถึงแผนกนี้'],
  ...
  ['GET', '/periods/:period', 'ดูสรุปสถานะงวด'],
  ...
  /* RETIRED 2026-09-03 — the three /birthday endpoints and the report below
     were deleted with ฝ่ายบุคคล's birthday work. THE LABELS STAY, because this
     table is what makes บันทึกระบบ readable and the rows it has to name are
     already written: a log line from August whose path no longer routes would
     otherwise print as a bare method and URL, which is the one thing this table
     exists to prevent. Nothing new can match them. */
  ['GET', '/birthday/queue', 'ดูคิวตรวจสวัสดิการวันเกิด (เลิกใช้แล้ว)'],
  ['POST', '/birthday/checks', 'บันทึกผลตรวจสวัสดิการวันเกิด (เลิกใช้แล้ว)'],
  ['POST', '/birthday/entries', 'ยื่นใบสวัสดิการวันเกิด (เลิกใช้แล้ว)'],
  ...
  ['GET', '/reports/birthday-check/:period', 'ดูรายงานตรวจสวัสดิการวันเกิด (เลิกใช้แล้ว)'],
];
...
export function statusClass(status) {
  /**
   * `Number(null)` is 0 and 0 is finite, so a record with NO status — a request
   * that never produced a response — would come back 'ok' and be painted green
   * on the one screen whose job is to show what went wrong. Checked before the
   * cast rather than after it, which is the only place the check works.
   */
