 ...
 * ceiling, and in a SENTENCE that is right: "รวมรออนุมัติ 9" and "ADM | 3 ชม."
 * both read worse with a dash in them, and the words around them already say
...
/**
 * ── `INCLUDES_PENDING` STOOD HERE — 'รวมใบที่รออนุมัติ', DELETED 2026-09-10 ──
 *
 * It said, and the argument was right: *one sentence for one fact, on every
 * screen that has to state it* — a figure counting requests nobody has
 * approved, worded once here rather than in whichever component said it first.
 *
 * IT LOST BOTH ITS CALLERS, one at a time. คิวรออนุมัติ dropped the line off
 * its rows on 2026-09-09 (the numbers became chips in the pop-up), and on
 * 2026-09-10 `pendingCapNote` — the last one — folded the qualifier into its
 * own lead word when HR asked for the line to be shortened:
 * `เพดานนับ 19 / 40 · รวมใบที่รออนุมัติ` became `รวมรออนุมัติ 19 / 40`.
 *
 * THE RULE IT ENFORCED IS NOT GONE, it moved down one function. The wording
 * still lives in this file, beside the arithmetic it describes, and
 * `test/queueCapUsage.test.js` still refuses to let a component spell it out —
 * it just guards a sentence now instead of a fragment of one. A constant with
 * no callers would have been a third place to look for a string that is only
 * written in one.
 */
...
/**
 * "รวมรออนุมัติ 35.5 / 40" — the line under a figure that does not count
 * everything its ceiling counts.
 *
 * WHAT IT IS FOR. ตรวจสอบประจำเดือน prints the hours its สถานะที่นับ filter
 * selected, because that is the report HR signs. A ceiling counts every request
 * still alive, because a department's remaining allowance is not a display
 * preference. Where those two differ the cell shows its own figure on top and
 * this one underneath, so nobody reads `16.5 / 40` in black on a month where
 * 35.5 of the 40 is already spoken for.
 *
 * ── ⚠ IT WAS TWICE THIS LONG UNTIL 2026-09-10 ────────────────────────────
 *
 * It read `เพดานนับ 19 / 40 · รวมใบที่รออนุมัติ` — a lead word, a figure, a
 * separator and a four-word qualifier, in a `.cap-sub` under a figure, on every
 * row of the month. Asked for by name: shorten it to **`รวมรออนุมัติ 19 / 40`**.
 *
 * IT LOST NOTHING BUT LENGTH, and that is worth showing rather than asserting:
 *
 *   · `เพดานนับ` said *what counts this figure*. The word above it already
 *     says เพดาน and the `/ 40` beside it is the ceiling; naming it a third
 *     time on the same line is the sentence describing its own column.
 *   · `รวมใบที่รออนุมัติ` said *what is included*. `รวมรออนุมัติ` says the same
 *     thing in the lead, where the reader is anyway, instead of after a `·`
 *     four words later.
 *
 * ── AND ONE BRANCH DISAPPEARED WITH IT ───────────────────────────────────
 *
 * The old lead had to switch to `รวมทั้งหมด` on a department with no ceiling,
 * because `เพดานนับ` would have named a limit that does not exist. The new lead
 * names no limit at all, so it is true either way and `capFigure` already
 * declines to print the `/ ไม่กำหนด` half. One sentence, one branch fewer.
 *
...
export function pendingCapNote(shownHours, capUsedHours, capHours) {
  ...
  return `รวมรออนุมัติ ${capFigure(capUsedHours, capHours)}`;
}
 ...
 */
const OVER_CEILING_FACT = 'รายการนี้เกินเพดาน OT ที่กำหนด';

export const OVER_CEILING_REASON_REQUIRED = `${OVER_CEILING_FACT} `
  + 'กรุณาระบุเหตุผลการอนุมัติ/ไม่อนุมัติรายบุคคล';

/**
 * The same sentence on the sheet that can only APPROVE — added 2026-09-02.
 *
 * ยืนยันรายการนี้ has one button and it says yes. Printing "อนุมัติ/ไม่อนุมัติ"
 * as its headline offers a choice that dialog does not have, on the one line
 * somebody reads to find out what is being asked of them. So the half that
 * applies is the half that is printed — and the FACT is shared, so the two
 * cannot drift into describing the same ceiling two different ways.
 *
 * `OVER_CEILING_REASON_REQUIRED` is untouched: it is what the server refuses
 * with on either route, where both halves really are in play, and it is what
 * กล่องไม่อนุมัติ prints above its own box.
 */
const OVER_CEILING_ASK_APPROVE = 'กรุณาระบุเหตุผลการยืนยันอนุมัติรายบุคคล';

export const OVER_CEILING_REASON_APPROVE = `${OVER_CEILING_FACT} ${OVER_CEILING_ASK_APPROVE}`;

/**
 * ที่ที่เหตุผลไปโผล่ — the sentence under BOTH ceiling boxes, said once here.
 *
 * ⚠ IT WAS WRITTEN TWICE, IN ONE FILE, UNTIL 2026-09-12. กล่องยืนยันอนุมัติ and
 * กล่องไม่อนุมัติ in components/ApprovalQueue.jsx each carried their own copy,
 * about a hundred lines apart, and the copies had drifted the way two copies
 * do: ประวัติของใบคำขอ against ประวัติของใบ · นำไปแสดงบนรายงานสรุป against
 * แสดงบนสรุป · the last clause in brackets on one sheet and trailing with ด้วย
 * on the other. Nothing had gone wrong yet, which IS the point — they said one
 * thing in two voices, so a correction to either would have left the other
 * standing and nothing in the tree would have failed.
 *
 * It is the argument `OVER_CEILING_FACT` above is here for, one step further
 * down the sheet: the headline says WHAT is being asked, this says WHERE the
 * answer ends up, and a reviewer meets both on either sheet.
 *
 * NOT THE FIELD, THE DESTINATION. "เหตุผลที่กรอกด้านล่าง" was the reject
 * sheet's wording and it points at a control the reader can already see; this
 * points at สรุป OT ส่งบัญชี, which is the part they cannot guess and the part
 * somebody reads months later.
 */
export const OVER_CEILING_REASON_SAY = 'เหตุผลที่ระบุจะถูกบันทึกไว้ในประวัติของใบคำขอ '
  + 'และนำไปแสดงบนรายงานสรุป OT ส่งบัญชี (ตรงตัวเลขชั่วโมงของพนักงานคนนี้)';

/**
 * หัวข้อกล่องเตือนของแผ่นยืนยัน — ใบเดียวหรือทั้งกอง
 *
 * The count is the case that matters: "รายการนี้" printed over a list of nine
 * names is the sheet describing one row while showing nine. What is being
 * ASKED does not change with the count, which is why that half is a constant
 * and not two sentences kept in step by hand.
 */
export function overCeilingApproveHead(count = 1) {
  return count > 1
    ? `${count} รายการที่เลือกไว้เกินเพดาน OT ที่กำหนด ${OVER_CEILING_ASK_APPROVE}`
