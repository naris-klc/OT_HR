...
/**
 * "เพดานนับ 35.5 / 40 · รวมใบที่รออนุมัติ" — for a screen showing one figure
 * while its ceiling is measured against another.
 *
 * ตรวจสอบรายเดือน prints the hours its สถานะที่นับ filter selected, which at
 * the default is อนุมัติแล้ว alone. The ceiling does not honour that filter and
 * cannot: a department's remaining allowance is not a display preference. So
 * the cell can be red while the number in it is comfortably inside the limit,
 * and this is the line that explains why — the figure the colour was decided
 * from, in the same words the queue uses for the same hours.
 *
 * `null` when the two agree, which is every row of a month with nothing
...
export function pendingCapNote(shownHours, capUsedHours, capHours) {
  ...
  return `เพดานนับ ${capFigure(capUsedHours, capHours)} · ${INCLUDES_PENDING}`;
}
