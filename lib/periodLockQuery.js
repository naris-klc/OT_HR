...
export async function closeChecks(period) {
  const [facets] = await OtEntry.aggregate([
    ...
    {
      $facet: {
        /* Every entry in the month, whatever its state. Not a close check —
           nothing about it can refuse a close — but the one figure that tells
           a month somebody worked from a month that never existed. The
           reminder on ตรวจสอบรายเดือน needs it: nagging about every unclosed
           month back to the epoch is how a prompt gets ignored. */
        entries: [{ $count: 'n' }],
