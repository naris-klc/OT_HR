 ...
 *
 * The first punch at or after this time is that day's **เวลาเข้างาน**, and the
 ...
 * arriving, and naming it เข้างาน would date the day's arrival hours before the
 * person walked in. There is no night shift here — *ไม่มีกะดึก*, 2026-09-04,
 ...
 *   dayPunches: Array<{time: string, next: boolean, checkIn: boolean}>,
...
export function checkEntryAgainstScans(entry, punches = [], opts = {}) {
  ...
  const dayEnd = entry.endsNextDay ? 2880 : 1440;
   ...
   * the same row, and `dayPunches` is what that row prints (since 2026-09-09 as
   * `เข้างาน … · สแกน …`, the arrival named and the rest listed).
   ...
   *
   * ── AND ONE OF THEM IS NAMED: เวลาเข้างาน ────────────────────────────────
   *
   * Since 2026-09-09 the first punch from 04:00 on is flagged `checkIn`, so the
   * row can print `เข้างาน 07:55 · สแกน 07:56, 22:56` instead of three times in
   * a row that the reader has to sort into an arrival and the rest. The whole
   * of the reasoning — and the reason it is the only naming this module does —
   * is on `SCAN_CHECK_IN_FLOOR_MINUTES`.
   *
   * `< 1440` holds it to `workDate` itself: on an overnight request the punches
   * past midnight wear `(+1)` and belong to the morning AFTER this row, so the
   * first of THEM is not this day's arrival. A day whose punches are all before
   * 04:00 has no เวลาเข้างาน and simply prints its times, as it always did.
   */
  const checkInPoint = onDay.find((p) => p.minutes >= SCAN_CHECK_IN_FLOOR_MINUTES
    && p.minutes < 1440) || null;
 ...
 * when the first punch from 04:00 on was pulled out of the list and printed as
 * `เข้างาน` (`scanCheckInTime`). The list is what is LEFT, and on the ordinary
 * two-punch day that is the single evening time. Nothing was dropped: the two
 * are drawn side by side on one line, `เข้างาน 07:21 · สแกน 19:30`.
 *
...
export function dayPunchLine(check) {
  const rest = (check?.dayPunches || []).filter((p) => !p.checkIn);
  if (!rest.length) return null;
  return rest.map((p) => `${p.time}${p.next ? ' (+1)' : ''}`).join(', ');
}

/**
 * `'07:55'` — the day's เวลาเข้างาน, or null when no punch fell on or after
 * 04:00 on `workDate`.
 *
