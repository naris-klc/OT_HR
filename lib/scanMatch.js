 ...
 *   start: object|null, end: object|null, punchCount: number, dayPunches: Array,
...
export function checkEntryAgainstScans(entry, punches = [], opts = {}) {
   ...
   * `alreadyInside` and `missingOtStart` below are the answer to. The END is
   ...
   * the distance one — a sentence may only quote a time the reader can see in
   * the `สแกน …` line printed on the same row, and `dayPunches` is that line.
   ...
   *
   * The one flag the verdict, the sentence and the chip all read, so they
   * cannot come to three different conclusions about one row. Two shapes, and
   * they are the two in WHAT STILL WARNS above: a door event after the claimed
   * start that disagrees, or no arrival on record at all that day.
   */
  const startFinding = !matched(start)
    && (Boolean(start && start.late) || !alreadyInside);

  /**
   * And the quiet fact, which is what is LEFT once the finding is taken out:
   * the start has no witness at the door, and there is nothing to do about it.
   * Never both — a row that is asking to be looked at does not also need to be
   * told that looking is optional, and the grey chip beside the amber one would
   * be exactly the two-marks-one-statement mistake `showsMissingOtStart`
   * already refuses on `no_scan` rows.
   */
  const missingOtStart = !matched(start) && !startFinding;
   ...
   * is still no. What is new is a MARK beside it, in the quiet tone, exactly as
   * `missingOtStart` is a mark on a row the verdict has stopped flagging. The
   * two are the same shape of thing and they are built the same way on purpose.
   *
   * ── NEVER BOTH — the same rule `missingOtStart` follows ──────────────────
   *
...
export function scanMismatchDetail(check) {
   ...
   *
   * It read `check.missingOtStart ? null : …` until 2026-09-04. The gate is
