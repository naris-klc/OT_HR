 ...
 *     สแกนออกตอน 19:30 น.*
 *   · **ไม่ได้สแกน** — *ไม่มีการสแกนนิ้วแต่ยื่นขอโอที* (HR wrote this one
 *     **ไม่ตรง**; it was reworded on 2026-09-11 — see below)
 *
 ...
 * on purpose: it is a start the machine actively disagrees with and nothing
 * about the end of the row is wrong.
 *
 * ── AND ไม่ตรง → **ไม่ได้สแกน** ON 2026-09-11 ─────────────────────────────
 *
 * HR's own word for it lasted four days. `ไม่ตรง` is a comparison — it asserts
 * two things that fail to agree — and this is the one state where there is no
 * second thing: nobody touched the reader that day. A reader who has just been
 * told ไม่ครบ means *สแกนออกก่อนเวลา* reads ไม่ตรง as a third shade of the same
 * kind of disagreement, and it is not one; it is the absence of the evidence
 * the other two are readings of. `ไม่ได้สแกน` says the whole of what happened
 * and cannot be read as a degree of anything.
 *
 * IT ALSO ENDED A COLLISION THIS BLOCK USED TO WARN ABOUT. The paragraph above
 * read "…and calling it ไม่ตรง would collide head-on with the word that now
 * means *no scan at all*" — the reason `START_OFF` could not be shortened.
 * ไม่ตรง is free now, and `เวลาเริ่มไม่ตรงกับสแกน` may keep or lose its length
 * on its own merits rather than to stay out of another badge's way.
 *
 * The chip went red with the word (`.chip.scan-none`), and **ไม่ครบ stayed
 * amber**, asked for in those terms.
 *
