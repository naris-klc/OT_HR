 ...
 *   dayPunches: Array<{
 *     time: string, next: boolean, checkIn: boolean, early: boolean,
 *   }>,
...
export function checkEntryAgainstScans(entry, punches = [], opts = {}) {
   ...
   * first of THEM is not this day's arrival.
   *
 ...
 * list of evening times reads as the small hours of the wrong morning. Those
 * are never `early` — the floor is a `workDate` clock time, not a distance
 * from midnight — so an overnight row keeps every time it ever had.
 *
