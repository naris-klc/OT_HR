...
/** What a Mongo id looks like written down — see `departmentScope`. */
...
/**
 * WHICH แผนก A MONTHLY DOCUMENT COVERS — the บทบาท's own answer, then the one
 * the screen asked for on top of it.
 *
 * The three lines this replaces stood in three routes — the report and the two
 * CSVs exported from under it — and were identical to the character:
 *
 *     if (teamOnly) filter.department = { $in: approvalDepartments(user) };
 *     else if (q.department) filter.department = q.department;
 *
 * They were identical because they had to be: the table, ส่งออกรายการ OT and
 * ส่งออกรายงานสรุปประจำเดือน are one screen and two buttons under it, and a
 * scope rule that drifted between them is a CSV that does not hold the rows its
 * own table showed. Three copies is three chances for that, and adding the
 * แผนก dropdown on 2026-09-10 would have made it three copies of a longer rule.
 *
 * ── IT CAN ONLY EVER NARROW ────────────────────────────────────────────────
 *
 * The property `teamScoped` states about `?scope=` is the property this states
 * about `?department=`, and it is what makes the parameter safe to accept from
 * a URL at all:
 *
 *   · Whole-company readers — ฝ่ายบุคคล, ผู้ดูแลระบบ, การเงิน on
 *     ตรวจสอบประจำเดือน — get the one แผนก asked for, or every แผนก when
 *     nothing is asked for. Either way it is a subset of what they already read.
 *   · A team-scoped reader gets one of the แผนก they sign for. A `?department=`
 *     naming a แผนก they do NOT sign for is IGNORED, not honoured and not
 *     refused: honouring it would hand them rows the approve rule says are not
 *     theirs, and refusing it would turn a stale tab into an error page. They
 *     get their own list, which is what they would have got without the
 *     parameter.
 *
 * So no value of `?department=` makes any of the three documents wider than the
 * บทบาท alone allows — which is the direction that matters, and the same
 * sentence `teamScoped` above is written around.
 *
 * ── WHICH แผนก IS BEING MATCHED ────────────────────────────────────────────
 *
 * `OtEntry.department` — where the hour was WORKED, snapshotted when the
 * request was filed. That is deliberately the same dimension ตรวจสอบประจำเดือน's
 * own แผนก column prints (the route populates the entry's department, not the
 * roster's), so the dropdown, the column, รวมทั้งหมด and both CSVs all cut the
 * month the same way.
 *
 * ⚠ IT IS NOT THE DIMENSION รายงาน OT แยกแผนก AND รายงาน OT ฝ่ายบัญชี USE.
 * Those group by the employee's สังกัดหลัก on the roster — `groupEntriesByEmployee`
 * in lib/accountingRows.js, and see test/reportDimension.test.js for the day
 * that changed and why. The two answers differ for exactly one person: somebody
 * whose แผนก was moved on ทะเบียนพนักงาน after they filed. That divergence is
 * older than this function and is not created by it; it is written down here
 * because this is now the place a reader will be standing when they meet it.
 *
 ...
 * เกิดข้อผิดพลาดภายในระบบ. Latent for as long as the parameter existed, because
 * no screen had ever sent one; the dropdown is what made it reachable by a
 * stale tab, a shared link or a typed URL, so it is answered here.
 *
 * It is treated as a แผนก THAT DOES NOT EXIST, not as no request at all: an id
 * that is well-formed and matches no department already returns an empty month
 * (measured on this database the same day), and the two cases are the same
 * question — *narrow to a department that is not there*. Answering one with
 * nought rows and the other with the whole company would make the difference
 * between a typo and a deleted แผนก visible as 29 people appearing.
 *
 * `{ $in: [] }` rather than the raw string: it matches nothing WITHOUT being
 * cast to an ObjectId, which is the throw itself.
 *
 * A 24-hex test and not `mongoose.Types.ObjectId.isValid` — this file is
 * deliberately free of the database layer (see the note on its imports), and
 * that function is loose in a way this must not be: it accepts ANY 12-character
 * string, so `?department=แผนกผลิต1ก` would pass it and throw further down.
 *
 * @param user  the signed-in account — `role`, and the แผนก it signs for.
 * @param q     the parsed query string; `scope` and `department` are read.
 * @returns `{ teamOnly, department }` — `department` is the `filter.department`
 *          clause, or `null` when the document is not narrowed at all. Both are
 *          returned because every caller needs `teamOnly` again afterwards, to
 *          decide whether the payroll half of the same scope applies.
 */
export function departmentScope(user, q) {
  const teamOnly = teamScoped(user.role, q.scope);
  const asked = q.department ? String(q.department) : '';
  if (!teamOnly) {
    if (!asked) return { teamOnly, department: null };
    // A แผนก that cannot exist, answered as a แผนก that does not — see above.
    return { teamOnly, department: OBJECT_ID_RE.test(asked) ? asked : { $in: [] } };
  }
  const mine = approvalDepartments(user).map(String);
  // One of theirs, or all of theirs. Never anybody else's — see above. Junk
  // needs no separate branch here: it is not on their list, so it is ignored.
  return { teamOnly, department: mine.includes(asked) ? asked : { $in: mine } };
}
...
const sessionKey = (e) => [
  ...
  e.workDate, e.startTime, e.endTime,
].join('|');
