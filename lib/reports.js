...
/**
 * The three answers to "what does the printed F-HR-027 carry" — see
 * `formPrintScope` in src/config/policy.js, which is where the wording is.
 */
export const FORM_PRINT_SCOPES = Object.freeze(['approved', 'screen', 'draft']);
 ...
 * 'approved' the request is not consulted at all: `?status=` is a query
 * parameter, anybody who can open the print page can type one, and a strict
 * setting that a URL can widen is not a setting. Under 'draft' the request is
 * likewise ignored, in the other direction — HR has said the sheet is a working
 * copy, and narrowing it per print would make two documents called the same
 * thing. Only 'screen' reads it, which is what that answer means.
 *
 ...
 *
 * The fallback under 'screen' is APPROVED-ONLY rather than the draft list.
 ...
 * all three would put queue rows on a signed sheet through the one path nobody
 * chose.
 *
 ...
 * and under 'approved' that is the only way to explain a print that ignored
 * สถานะที่นับ.
 *
...
export function formPrintStatuses(policy, requested) {
  ...
  const scope = FORM_PRINT_SCOPES.includes(raw) ? raw : 'approved';
  if (scope === 'draft') return { scope, statuses: reportStatuses(DRAFT_STATUSES) };
  if (scope === 'screen') return { scope, statuses: reportStatuses(requested, 'approved') };
  return { scope, statuses: ['approved'] };
}
 ...
 *
 * In practice: อนุมัติแล้ว + รอ HR on ตรวจสอบรายเดือน prints a clean sheet
 * somebody can sign, which is what that filter is chosen for.
 *
