 ...
 *
 * `signed` IS FIRST AND IS THE ONE THAT SHIPS, since 2026-09-07 — HR, in these
 * words: *ข้อมูลที่พนักงานยื่นขอโอที ต้องขึ้นในใบขออนุมัติทำงานล่วงเวลา ตั้งแต่
 * ตอนที่มีคนกดอนุมัติเลย*. It read three until then, and the strict one was
 * `approved`, which is the LAST signature rather than the first: a request the
 * หัวหน้า had approved was off the sheet until ฝ่ายบุคคล confirmed it — on the
 * very sheet ฝ่ายบุคคล confirm FROM, whose foot carries the เฉพาะฝ่ายบุคคล box
 * that is that confirmation.
 */
export const FORM_PRINT_SCOPES = Object.freeze(['signed', 'approved', 'screen', 'draft']);
...
/** The list the form route used to hard-code, and what 'draft' still means. */
 ...
 *
 * The fallback under 'screen' is the SHIPPED list rather than the draft one.
 * There are two callers with no filter to send — a พนักงาน printing their own
 * month, and any future screen that forgets — and for both of them the safe
 * reading of "follow the screen" is the strict one. Silently falling back to
 * all three would put รอหัวหน้า rows on a signed sheet through the one path
 * nobody chose. It was 'approved' alone until 2026-09-07 and moved with the
 * default: the fallback is the answer a caller who said nothing should get, and
 * that is whatever the shipped answer is.
 *
...
export function formPrintStatuses(policy, requested) {
  ...
  const scope = FORM_PRINT_SCOPES.includes(raw) ? raw : 'signed';
  if (scope === 'draft') return { scope, statuses: reportStatuses(DRAFT_STATUSES) };
  if (scope === 'screen') return { scope, statuses: reportStatuses(requested, SIGNED_STATUSES) };
  if (scope === 'approved') return { scope, statuses: ['approved'] };
  return { scope, statuses: reportStatuses(SIGNED_STATUSES) };
}
 ...
 *
 * SINCE 2026-09-07 THAT IS THE ORDINARY SHEET AND NOT A CHOSEN FILTER. อนุมัติ
 * แล้ว + รอ HR is what `signed` — the shipped answer — asks for, so this
 * function returns nothing at all on a normal print and the paper carries no
 * mark, which is exactly what the reasoning above says it should. It was
 * written for the filter and it is unchanged; only how often it is reached is.
 *
