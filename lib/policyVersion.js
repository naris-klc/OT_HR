...
export const ARITHMETIC_KEYS = Object.freeze([
  ...
  'breakThresholdHours', 'roundingMode',
...
export const COSMETIC_KEYS = Object.freeze([
  ...
  /**
   * ใครเห็นช่องติ๊กไหน บนฟอร์มบันทึก OT — 2026-09-16.
   *
   * COSMETIC on the plainest reading of the word this list has: they decide
   * whether a QUESTION is put on a screen. `flatDaily` and `noBreakTaken` are
   * entered fields either way, the write path takes both from anybody, and not
   * one stored figure goes stale when a mode moves — so replaying every entry
   * in flight over a change here would rewrite nothing.
   *
   * ⚠ WHAT DOES MOVE FIGURES IS DOWNSTREAM OF THEM AND IS NOT A REPLAY. A tick
   * the new rules no longer allow is cleared when somebody next opens that row
   * (`tickClearing` in lib/entries.js), and the hours change when they save the
   * correction — one entry at a time, through the ordinary write path, with a
   * `before` in its history like any other edit. That is a person's save, not a
   * policy replay, which is exactly why it must not be one: restating eight
   * hours on rows nobody has looked at is the thing `savePolicy` refuses to do
   * to approved work.
   */
  ...
  /**
   * The day of the following month after which an entry's period closes itself
   * to แก้ไข · ยกเลิก · ขอถอนใบ · ตัดสินคำขอถอน.
   *
   * COSMETIC, and the case is stronger than for the two submission windows
   * above rather than merely equal to them. Those two are at least read on the
   * write path that produces figures; this one is never handed to
   * `computeSession` at all. It decides whether a BUTTON may be pressed, on an
   * entry whose `segments`, `buckets` and `totals` were computed and stored
   * long before anybody asked.
   *
   * And a replay could not restate it even if it wanted to, for the reason
   * written over `maxAdvanceSubmissionDays`, in its sharpest form: the rule
   * asks what today's date is, and a replay always runs on a day that has
   * already moved on.
   */
  'cancelCutoffDay',
]);
...
export function figuresMoved(before, after) {
  ...
  // ชั่วโมงทำงานปกติ — a เหมารายวัน day's whole figure lives outside the rate
  // columns, so a policy that moves the core hours moves this and nothing else.
  // Left uncompared, the one restatement a flat entry can suffer would be the
  // one that never gets a `before`.
  if ((before.normalHours ?? 0) !== (after.normalHours ?? 0)) return true;
