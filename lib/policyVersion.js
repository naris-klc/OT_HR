...
export const COSMETIC_KEYS = Object.freeze([
  ...
  'proxyNoteOnForm',
  /**
   * Whether ฝ่ายบุคคล filing a birthday request from the check list also approves
   * it in the same act.
   *
   * COSMETIC on exactly the terms `proxySkipsOwnApproval` above it is, and the
   * comparison is worth making because this one moves a request further: an
   * entry written straight to `approved` is one nobody will approve again, and
   * that is a real difference to a queue. It is not a difference to a FIGURE.
   * The hours are computed by the engine from the two times that were typed,
   * against the same policy, before this flag is consulted at all — `segments`,
   * `buckets` and `totals` are bit-identical either way, and `sameArithmetic`
   * exists to answer whether stored figures went stale, not whether anything
   * changed.
   *
   * Replaying a month over a change to it would rewrite nothing and fill several
   * thousand histories with recomputes that moved no minutes.
   */
  'hrDirectApproveBirthday',
  /**
...
export function summariseReplay({
  ...
  /**
   * Which months a closed period kept out of this run, named rather than
   * counted into the `skipped` total and lost there.
   *
   * A replay that reports "42 scanned, 30 replayed, 12 skipped" over a closed
   * month reads as a normal run with some approved rows in it. The months are
   * the part somebody has to act on: if the figures were meant to move, an
   * administrator has to reopen those periods and run it again.
   */
  const closedPeriods = [...new Set(
    skipped.filter((s) => s?.reason === 'period_closed').map((s) => s.period).filter(Boolean),
  )].sort();
  ...
  return {
    ...
    skippedClosed: skipped.filter((s) => s?.reason === 'period_closed').length,
    closedPeriods,
...
/**
 * `closedPeriods` — the months ปิดงวด has finished, which a replay may not
 * touch at all.
 *
 * HR's rule, 2026-08-14: a closed month does not change until an administrator
 * opens it. Seven write paths enforce that (see test/periodLockRoutes.test.js)
 * and a replay was the eighth way in and the quietest — nobody presses an edit
 * button, a flag moves and a month that was signed off, exported and paid is
 * restated underneath it.
 *
 * CHECKED BEFORE `approved`, because it is the stronger statement and the wider
 * one: it covers the pending entries in a closed month too, which the approved
 * rule does not, and "the month is finished" is the reason a reader needs when
 * a row is skipped for both reasons at once.
 *
 * It is not an authority question. An administrator may already replay approved
 * entries with `includeApproved`; what this adds is that doing it to a closed
 * month takes the deliberate step of reopening the period — which leaves a
 * reason and a record that a flag change never would.
 */
/**
...
export function planRecompute(entries, { includeApproved = false, closedPeriods = [] } = {}) {
  const closed = new Set(closedPeriods);
  const replay = [];
  const skipped = [];
  for (const entry of entries || []) {
    if (closed.size && closed.has(entry?.period)) {
      skipped.push({ id: String(entry._id), reason: 'period_closed', period: entry.period });
      continue;
    }
