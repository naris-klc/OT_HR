...
/**
 * เดือนก่อนหน้ายังไม่ได้ปิด — พูดหรือไม่พูด.
 *
 * Here rather than in the component for the reason `closeRefusal` is here: the
 * bar, and anything else that later wants to say this, must describe the state
 * with the same rule. A second reading of "is last month a problem" written
 * inline in JSX is how one screen comes to nag about a month another screen
 * considers finished.
 *
 * IT IS ABOUT ANOTHER MONTH, so it is not a refusal and blocks nothing. The
 * bar this appears on may be reporting a perfectly closable month, or a closed
 * one — closing สิงหาคม while กรกฎาคม is still open is exactly the order in
 * which a month gets skipped, and the moment it is most worth saying.
 *
 * A month with no entries is silent. Every month before the system was
 * installed is an unclosed month, and a prompt that lists them is a prompt
 * people learn to scroll past.
 *
 * @param {object|null} previous  the /periods/<prev> response, or null
 * @returns {{ entries: number, pending: number } | null}
 */
export function previousMonthOpen(previous) {
  if (!previous || previous.closed) return null;
  const entries = previous.checks?.entries ?? 0;
  if (entries <= 0) return null;

  return { entries, pending: previous.pending ?? 0 };
}
