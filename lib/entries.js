 ...
 *
 * 08:00–17:00 is that same arithmetic, which is why `STANDARD_DAY` in
 * components/OtForm.jsx agrees with it rather than restating it; the two are
 * pinned to each other in test/flatDaily.test.js so the fill and the rule
 * cannot drift apart.
 */
export const FLAT_DAY_SPAN_MINUTES = 9 * 60;

/**
 * `'07:00'` → `'16:00'` — where a เหมารายวัน day that began at 07:00 ends.
 *
 * THE END IS DERIVED AND THE START IS NOT. That is HR's rule of 2026-09-07, and
 * it narrows the one before it rather than replacing it: both times were
 * editable when the tick was built, because somebody who came in at 07:30 files
 * 07:30, and that half is untouched. What is withdrawn is the free END. A flat
 * day is a FIXED LENGTH — the office day, bought whole — so its finish is not a
 * second thing to be typed but an answer to where it began, and a 07:00–18:00
 * flat request is a row that reads as eleven hours, pays eight, and carries
 * nothing that explains the gap.
 *
 * IT WRAPS, and the caller has to hear about it: a flat day starting at 16:00
 * ends at 01:00 the next morning, and the engine refuses an end that is not
 * after its start unless ข้ามคืน is ticked — `END_BEFORE_START`. So the form
 * asks `endsNextDayFor` about the pair this returns, rather than leaving a tick
 * box to be got right by hand about a time nobody typed.
 *
 * HERE RATHER THAN IN THE COMPONENT, for the reason `parseTime` above gives:
 * `npm test` is plain `node --test` with no JSX transform, so a helper living
 * inside a component can only ever be checked as source text — and this one has
 * a wrap in it.
 *
 * A time it cannot read answers `''`, the same blank the box it fills is
 * allowed to hold.
 */
export function flatDayEnd(startTime) {
  const t = parseTime(startTime);
  if (!t) return '';
  const end = (t.h * 60 + t.m + FLAT_DAY_SPAN_MINUTES) % (24 * 60);
