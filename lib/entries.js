 ...
 */
export function cancelPermission(user, entry, { policy, on } = {}) {
  /**
   ...
   */
  if (mayCorrectEntries(user) && isUntouchedSystemFiling(entry)) {
   ...
   *
   * The ฝ่ายบุคคล `void` branch stays above both, untouched: a row the system
   * wrote is not the employee's request and this rule never had anything to say
   * about it (§7).
   */
  const past = cancelCutoffRefusal(user, entry, policy, on || today());
  if (past) return { ok: false, status: past.status, error: past.error };
