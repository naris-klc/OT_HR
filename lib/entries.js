 ...
 * which it does not.
 *
 * A blank time answers false rather than guessing. Both fields come from
 * `<input type="time">`, which reads '' while somebody is still typing the hour
 * — and '' sorts before every real time, so a naive compare ticks the box
...
/**
 * นับสวัสดิการวันเกิดเองไม่ได้ — the sentence POST /api/entries refuses a person
 * with when the day they are filing for is their OWN สวัสดิการวันเกิด.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * WHY THE FORM DOES NOT SIMPLY HIDE THE DATE
 *
 * There is no "ประเภท OT" dropdown to take an entry off. The kind of day is not
 * chosen, it is RESOLVED — `resolveDayTypes` reads the person's stored วันเกิด
 * and the live policy and answers holiday-because-birthday — so the only thing
 * an employee ever picks is a date, and every date is on the calendar. Hiding
 * one would mean the browser working out whose birthday today is, which is both
 * the rule written twice and a birth date sent to a screen that must not have
 * one (`publicEmployee`, lib/employees.js).
 *
 * So the refusal is a function of values here, checked on the write path, and
 * the form learns it the way it learns `weekdayOtRefusal`: by asking the
 * preview. HR's rule, 2026-08-31 — สวัสดิการวันเกิด is granted by the company
 * and recorded by ฝ่ายบุคคล off the fingerprint scanner's export; it is not
 * something a person claims for themselves.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * AND WHY IT IS IN THIS FILE RATHER THAN lib/birthdayFiling.js
 *
 * Because POST /api/entries has to import it, and that file is the one thing
 * the ordinary submit path may not reach. The two-doors rule is pinned as a
 * whole-file ban — `birthdayFiling|birthdayDirectApproval|HR_VERIFIED_ACTION`
 * must appear nowhere in app/api/entries/route.js, see
 * test/birthdayDirectApproval.test.js — precisely so that no later edit can
 * import the single-signature shortcut "just for this one check". A refusal
 * belongs on the ordinary path anyway: it sits beside `submissionWindowRefusal`
 * above, which is the same shape of rule.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * WHAT IT DOES NOT REFUSE, AND WHY EACH IS DELIBERATE
 *
 * A PROXY FILING IS UNTOUCHED. A หัวหน้า pressing บันทึก OT แทนลูกทีม on a day
 * that happens to be their team member's birthday still files, and the request
 * waits at `pending_hr` for ฝ่ายบุคคล exactly as it always did — so the person
 * this rule reserves the decision for is the person who ends up making it. The
 * alternative was a 409 sent to somebody who cannot be told why: the หัวหน้า
 * does not know the date is a birthday, must not be told, and would read
 * "ปฏิเสธ" over a shift they watched happen. HR's answer, asked directly.
 *
 * THE TAIL OF AN OVERNIGHT SHIFT IS UNTOUCHED. This asks about `workDate` — the
 * day the request is FOR — and not about every date the session touches. A shift
 * filed against an ordinary Monday that runs past midnight into the filer's
 * birthday is Monday's OT: they did not ask for their birthday, the hours are
 * one continuous shift, and refusing the day over its tail would lose Monday
 * rather than protect Tuesday. ฝ่ายบุคคล still meets that day in
 * วันเกิดที่ยังไม่มีใบ, where `refuseDayConflict` shows them the overlap.
 *
 * `dayTypes` is the map `loadContext` resolved for this session, so the three
 * parts of the rule that could be got wrong by re-deriving it — the flag, the
 * leap-day answer, and a weekend or company holiday taking precedence — are
 * already answered. A birthday that fell on a Saturday reads
 * `reason: 'weekend'` here and this returns null, which is right: the day was
 * วันหยุด for everybody and the ordinary holiday request is the correct one.
 *
 * @returns {?string} the sentence to refuse with, or null to let the filing pass
 */
export function birthdayOtRefusal({ filer, employee, dayTypes, workDate }) {
  // Somebody else typed it — see above. `idOf` handles a populated document and
  // a bare id alike, which is what these two arguments are on the edit path.
  if (!filer || !employee || idOf(filer) !== idOf(employee)) return null;

  const day = dayTypes instanceof Map ? dayTypes.get(workDate) : dayTypes?.[workDate];
  // A bare 'holiday' string carries no reason and cannot be this. The engine
  ...
  return 'วันนี้เป็นสวัสดิการวันเกิดของคุณ — ยื่นขอ OT ของวันเกิดตัวเองไม่ได้ '
    + 'ฝ่ายบุคคลเป็นผู้บันทึกและอนุมัติให้ โดยตรวจจากบันทึกเวลาเข้า-ออกงาน (สแกนนิ้ว) '
    + '· ถ้ามาทำงานในวันนี้แล้วยังไม่เห็นรายการ ให้แจ้งฝ่ายบุคคล';
}
