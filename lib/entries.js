 ...
 * drawn at all.
 *
 * A second generated action goes in this list rather than into a second
 * predicate somewhere else — the rule `SYSTEM_FILED_ACTIONS` is written under,
 * for the same reason.
 */
 ...
 * (`isBirthdayWelfare`). The database this laptop serves holds no such row at
 * all: 0 of 431 carry `submit_birthday`, and none was ever voided. And the hole
 * it was patching is closed properly since 2026-09-15 — ฝ่ายบุคคล may cancel
 * ANY live entry, with a reason, which is `cancelPermission` below.
 *
 * The two history actions themselves (`submit_birthday`, `void`) are still in
 * src/models/OtEntry.js. They are vocabulary for reading OLD rows, not a
 * feature, and dropping them would fail validation on any row another
