 ...
 *
 * THE WAY BACK OUT OF A GENERATED ROW. ฝ่ายบุคคล presses สร้างใบวันเกิด and the
 * entries are confirmed in the same action, because a second press by the same
 * person checks nothing (see the birthday route). What that costs is the queue:
 * an ordinary request can be refused while it waits and withdrawn by the
 * employee, and an `approved` one can be neither — `approvalPermission` acts
 * only on `pending_*`, `cancelPermission` only before the first signature, and
 * there is no delete anywhere. Approving on creation would otherwise make one
 * mis-ticked checkbox a permanent figure.
 *
...
export function cancelPermission(user, entry) {
   ...
   * are a proposal ฝ่ายบุคคล accepted on their behalf, from a calendar rather
   * than from anybody's account of what they worked. The person who created it is
   * the person who can retract it, and `isUntouchedSystemFiling` is what makes
   * "can" run out: as soon as somebody edits or signs the row, it stops being a
   * proposal and this door closes.
   *
   * No reason is required, deliberately, though one is recorded when given. A
   * reason is owed for changing what somebody else established; unticking a
   * checkbox a minute late establishes nothing. Requiring one would buy a field
   * full of "ผิด" and a slower correction.
   */
