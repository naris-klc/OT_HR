 ...
 * print:
 *
 *   ตกค้าง (`outstanding`) — nobody has answered it yet. An entry sitting at
 *     `pending_mgr` is not on F-HR-027 at all, and a withdrawal request nobody
 *     has decided means a row that IS on the sheet may be about to come off it.
 *     Print now and the paper is wrong, or goes stale the same week.
 *
 ...
 *   `why`   — "ยังไม่ขึ้นในใบ OT ที่พิมพ์ออกมา". What that means for the sheet
...
export function periodItems({
  ...
  if (pending > 0) {
    outstanding.push({
      ...
      why: 'ยังไม่ขึ้นในใบ OT ที่พิมพ์ออกมา',
      text: `มีใบรออนุมัติค้างอยู่ ${pending} ใบ — ยังไม่ขึ้นในใบ OT ที่พิมพ์ออกมา`,
    });
  }
