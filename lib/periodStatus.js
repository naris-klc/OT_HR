...
export function periodItems({
  ...
  if (openWithdrawals > 0) {
    outstanding.push({
      ...
      /**
       * THIS LINE CAN NO LONGER CLEAR ITSELF, since 2026-09-14.
       *
       * `cancelCutoffDay` measures the ตัดสิน press at TODAY rather than at
       * `requestedAt`, so a request asked in time and left sitting past the
       * cutoff is out of the หัวหน้า's hands for good — ฝ่ายบุคคล is the only
       * way it ever becomes answered. A finished month's outstanding list will
       * therefore keep this row indefinitely unless somebody in HR goes and
       * decides it. That is the price of the word "ทั้งหมด", written down in
       * §13 of docs/plan-cancel-cutoff-day.md rather than softened, and this
       * screen is where it surfaces.
       */
