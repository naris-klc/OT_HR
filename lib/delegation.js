 ...
 */
import { idOf, isDepartmentManager, isProxyFiled } from './entries.js';
...
/**
 * The claim this person has on the manager's step for this entry, or null.
 *
 ...
 */
function managerClaim(user, entry, delegations, today) {
  if (isDepartmentManager(user, entry.department)) {
  ...
  const match = receivedOn(delegations, user, today)
    .find((d) => isDepartmentManager(d.from, entry.department));
...
export function approvalPermission({ user, entry, delegations = [], today, verb = 'อนุมัติ' }) {
   ...
   */
  if (isProxyFiled(entry) && idOf(entry.filedBy) === idOf(user)) {
    return {
      ok: false,
      status: 403,
...
export function publicDelegation(row, on) {
  ...
  const person = (p) => (p
    ? { id: idOf(p), code: p.code, name: p.name, role: p.role, departmentId: idOf(p.department) || null }
    : null);
