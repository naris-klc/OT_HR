...
/**
 * สิ่งที่การอนุมัติของผู้รับช่วงทิ้งไว้ — ประโยคกลางที่สองหน้าจอใช้ร่วมกัน
 *
 * ⚠ TWO SCREENS HAD TWO COPIES OF THIS UNTIL 2026-09-12. The strip at the top
 * of คิวรออนุมัติ tells the person who is standing in; the note on the card in
 * ผู้รับช่วงอนุมัติแทน tells the person who set the arrangement up. Both had to
 * say what an approval made under it leaves behind, and both said it in their
 * own words — ชื่อหัวหน้าเจ้าของคิว on one, พร้อมชื่อหัวหน้างานเจ้าของคิว on the
 * other, the same fact with a different name for the same person.
 *
 * ONLY THE MIDDLE IS SHARED, and that is the whole design. Each screen keeps
 * its own subject, because the subject is who the reader is — "การอนุมัติของคุณ"
 * to a stand-in, "ทุกการอนุมัติของผู้รับช่วง" to whoever appointed one — and
 * each keeps its own tail, because what each reader needs next is different:
 * the stand-in needs to know the owner can still approve, the delegator needs
 * to know the stand-in cannot hand it on again. What neither may own is the
 * sentence in between: it is a statement about what the SYSTEM records, and a
 * system that describes its own record two ways has two records as far as the
 * reader is concerned.
 */
export const DELEGATED_APPROVAL_RECORDED = 'จะถูกบันทึกว่า “ทำแทน” '
  + 'พร้อมชื่อหัวหน้างานเจ้าของคิว ทั้งในประวัติรายการและบนใบพิมพ์';
