 ...
 *
 * ฝ่ายบุคคล and Admin, and nobody else. HR's rule, 2026-08-13: วันหยุดวันเกิด
...
export function birthdayActionPermission({ user, subject = null }) {
  if (!['hr', 'admin'].includes(user?.role)) {
    return {
      ok: false,
      status: 403,
      error: 'รายการวันหยุดวันเกิดบันทึกได้เฉพาะฝ่ายบุคคล '
  ...
  if (subject && idOf(subject) && idOf(subject) === idOf(user)) {
    return {
      ok: false,
      status: 403,
      error: 'ลงนามรายการวันหยุดวันเกิดของตนเองไม่ได้',
    };
  }
...
export function birthdayDirectApproval({
  ...
  if (!isBirthdaySubject(employee)) {
    return { ok: false, status: 400, error: 'บทบาทนี้ไม่อยู่ในข่ายวันหยุดวันเกิด' };
  }
