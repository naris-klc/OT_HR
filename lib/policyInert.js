 ...
 */
/**
 * ตำแหน่งที่เลือกไว้ไม่ถูกอ่าน เมื่อโหมดเป็น "ทุกตำแหน่ง" — one sentence, both
 * ticks.
 *
 * Written once because the two rows are the same rule twice, and a second copy
 * would be the thing this file exists to prevent: the two sentences drifting
 * until one of them describes a mode that no longer exists.
 */
function positionListInert(policy, tick, tickName) {
  if (policy[`${tick}PositionMode`] !== 'all') return null;
  return {
    text: `ไม่ถูกอ่านกับค่าที่ตั้งอยู่ตอนนี้ — ข้อข้างบนตั้งไว้ว่าช่อง${tickName}แสดงกับ `
      + 'ทุกตำแหน่ง จึงไม่มีรายชื่อไหนถูกนำไปเทียบ · ตำแหน่งที่เลือกไว้ยังถูกเก็บ '
      + 'และกลับมามีผลทันทีที่เปลี่ยนข้อข้างบนเป็น “เฉพาะที่เลือก” หรือ “ยกเว้นที่เลือก”',
    causes: [`${tick}PositionMode`],
  };
}
...
const RULES = {
  ...
  /**
   * ตำแหน่งของช่องเหมารายวัน ← โหมดของช่องเหมารายวัน = ทุกตำแหน่ง.
   *
   * The list is still stored and still shown; it is simply not consulted while
   * the mode above it says everybody. Reachable in one press — the mode is the
   * row directly above — which is why it says which row to change rather than
   * only that nothing is happening.
   */
  flatDailyPositions(policy) {
    return positionListInert(policy, 'flatDaily', 'เหมารายวัน');
  },

  /** ตำแหน่งของช่องไม่พักเที่ยง ← โหมดของช่องไม่พักเที่ยง = ทุกตำแหน่ง. */
  noBreakPositions(policy) {
    return positionListInert(policy, 'noBreak', 'ไม่พักเที่ยง');
  },
