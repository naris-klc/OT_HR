 ...
 *
 * ── AND IT LEAVES OUT THE COMPANIES THAT BROUGHT NO FILE, SINCE 2026-09-11 ──
 *
 * `notImported` is `buildScanSlots`'s company-level answer, forwarded by the
 * route. Their rows are dropped BEFORE the comparison rather than re-labelled
 * after it, and that single cut is what makes every consequence fall out
 * without a rule of its own: those people never reach `people`, so they are
 * never in `flaggedBy`, so §5.2 lets them be ticked — which is §5.3 (*a month
 * with no import ticks normally*) applied a level down, to a company. `counts`
 * then describes only the half that was genuinely read, instead of carrying a
 * pile of `ไม่มีสแกน` that nobody can do anything about.
 *
 * ⚠ THE PRICE IS THAT THE CARD MUST SAY WHAT WAS LEFT OUT, which is why the
 * list is handed back rather than quietly applied: `✓ ทุกแถวที่เทียบได้ตรงกับ
 * ไฟล์สแกน` over a month whose other company was never read is exactly the
 * misreading ScanCompareCard exists to prevent. See
 ...
 * @param {string[]} [opts.notImported] company keys with no file for this งวด
 */
export async function compareMonthAgainstScans({
  period, statuses, department = null, notImported = [],
}) {
  const filter = { period, status: { $in: statuses } };
  if (department) filter.department = department;
  const found = await OtEntry.find(
    filter,
    'employee workDate startTime endTime flatDaily',
    /**
     * `company` is selected for the cut below and for nothing else. It costs
     * one field on a list this route already loads whole; asking the roster for
     * it separately would be a second query to learn something the populate is
     * already carrying the document for.
     */
  ).populate({ path: 'employee', select: 'code name company' }).lean();

  /**
   * AN EMPLOYEE WITH NO COMPANY ON RECORD IS NEVER DROPPED. `company` is
   * required with a pre-validate default (see src/models/Employee.js), so this
   * should not arise — and if it ever does, an unknown company is not a
   * known-missing one, and the safe reading is the one this screen had before.
   */
  const skip = new Set(notImported);
  const entries = skip.size
    ? found.filter((e) => !(e.employee?.company && skip.has(e.employee.company)))
    : found;
  ...
  return {
    ...
    /**
     * Echoed back so the screen draws ONE opinion and not two. The browser
     * holds `slots` and could work this out for itself; the day its answer and
     * this one disagreed, a row would read `ยังไม่นำเข้า` while still being
     * barred from the tick-box for carrying a flag — a contradiction on one row.
     */
    notImported,
  };
}
