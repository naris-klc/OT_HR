 ...
 * apart is what lets every edge of the comparison — overnight, เหมารายวัน, the
...
export async function scanChecksFor(entries = [], opts = {}) {
  ...
  /**
   * ONE DAY EITHER SIDE of the range the rows cover. An overnight request needs
   * the next morning's punches, and a request starting at 00:20 needs the
   * previous evening's to have a nearest-scan to quote at all.
   */
 ...
 */
export async function compareMonthAgainstScans({ period, statuses }) {
  const entries = await OtEntry.find(
    { period, status: { $in: statuses } },
    'employee workDate startTime endTime endsNextDay flatDaily',
  ).populate({ path: 'employee', select: 'code name' }).lean();

  if (!entries.length) {
    return {
      counts: { mismatch: 0, noScan: 0, flatDaily: 0, checked: 0 }, people: [], entryCount: 0,
    };
  }
