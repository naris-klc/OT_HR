 ...
 * the audit trail, the policy versions, the replay runs, the delegations and
 * the period locks are NOT — so a forced reseed does not return the database to
 * a clean state, it leaves those pointing at employees and entries that no
 * longer exist. Every count below is therefore a reason to stop, not a thing
 * that would be cleanly replaced.
 *
 ...
 * @param {number} counts.policyConfirmations HR's answers to [OPEN] items
 * @param {number} counts.delegations
 * @param {number} counts.periodLocks
...
export function seedSafety(counts = {}) {
  ...
  const {
    ...
    policyVersions = 0, policyConfirmations = 0, delegations = 0, periodLocks = 0,
  } = counts;
  ...
  if (policyConfirmations) findings.push(`คำตอบ [OPEN] ที่ HR ยืนยันไว้ ${policyConfirmations} ข้อ`);
  if (delegations) findings.push(`ผู้รับช่วงอนุมัติ ${delegations} รายการ`);
  if (periodLocks) findings.push(`งวดที่ปิดแล้ว ${periodLocks} งวด`);
...
export function seedRefusal(findings) {
  return [
    ...
    'และไม่ลบ ประวัติการแก้ทะเบียน เวอร์ชันนโยบาย ผู้รับช่วง งวดที่ปิด — ของพวกนั้น',
