 ...
 *
 * Same split as lib/birthdayCheck.js / lib/birthdayQueueQuery.js and
 * lib/rosterAudit.js / lib/rosterAuditLog.js, for the same reason: what counts
 ...
 * @param {?string[]} kinds  null = all six
 */
export async function loadCompliance(window = {}, kinds = null) {
   ...
   * merged list again. Not redundant: the query bound keeps four collection
   * scans proportional to the period asked for, and the second pass is what
   * applies it to rows nested INSIDE a document — a period lock's events and an
   * entry's history each carry their own timestamps, and the parent document's
   * dates say nothing about them.
   */
  const between = (field) => {
    if (!from && !to) return {};
    const clause = {};
  ...
  const [rosterAudits, entries, periodLocks, replayRuns] = await Promise.all([
    EmployeeAudit.find({
      ...between('createdAt'),
      /**
    ...
    /**
     * Not date-bounded in the query, deliberately: the events are nested and a
     * lock document carries no timestamp of its own that means anything about
     * them. There is one document per period — a handful — so the whole
     * collection is cheaper than the index gymnastics, and `complianceRows`
     * applies the window to each event.
     */
    PeriodLock.find({ 'events.action': 'reopen' }).sort({ period: 1 }).lean(),
   ...
   * across all four sources rather than a lookup per row.
   *
  ...
  const actorIds = [...new Set([
    ...
    ...periodLocks.flatMap((l) => (l.events || []).map((ev) => ev.by)),
  ...
  return complianceRows({
    ...
    periodLocks: periodLocks.map((l) => ({ ...l, events: (l.events || []).map(withCode) })),
