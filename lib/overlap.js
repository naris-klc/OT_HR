 ...
 * Older, and NOT made redundant by the rule above: the only pair it can still
 * catch is one that crosses a midnight, where the two requests are on two
 * different dates and share hours anyway.
 *
 ...
 *
 * IT DOES NOT REPLACE `findOverlaps`, and the reason is the midnight. A session
 * filed against the 5th that runs to 02:00 occupies four hours of the 6th while
 * printing on the 5th's line — so a request on the 6th is a different DATE and
 * can still claim minutes this one already holds. The date rule cannot see that
 * and the minute rule can. Both run, this one first, because it is the one the
 * employee can act on without thinking about windows: แก้ไขรายการเดิม.
 *
