...
/**
 * Did THIS entry skip the หัวหน้า step because its filer was that step?
 *
 * The same question `isOwnApprovalFiling` asks of one history row, asked of the
 * whole entry, and exported because two screens were answering it by ARITHMETIC
 * — `status === 'pending_hr' && !managerDecision?.at` — which is true of three
 * different things and says the words of only one of them:
 *
 *   · a filing that really did skip the step (this);
 *   · a request from แผนกจัดซื้อ or แผนกทรัพยากรมนุษย์, whose หัวหน้างาน IS
 *     ฝ่ายบุคคล by rule — nothing was skipped, the first signature is simply
 *     still to come (`hrHeadsDepartment` in lib/proxyFiling.js);
 *   · a request whose แผนก has no living signer at all.
 *
 * On the last two the screens were telling the reader that the system had
 * skipped a step for them, which it had not.
 *
 * IT MATTERS MORE SINCE 2026-09-09, not less, and that is why it is being fixed
 * on the way past: `proxySkipsOwnApproval` now defaults to `false`, so a fresh
 * proxy filing never skips anything and the sentence is left explaining
 * something that no longer happens on any row it can now reach. It stays because
 * the rows filed while the old default stood are still in the database (two on
 * prod) and are still read this way — the note is the only mark on them that
 * says so.
 */
export const skippedOwnApproval = (entry) => (entry?.history || []).some(isOwnApprovalFiling);
