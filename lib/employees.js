...
import { SIGNER_ROLES, isSigner, hrHeadsDepartment } from './roles.js';
 ...
 *     issues one. It read "the client shell lets such an account do exactly one
 *     thing — set its own password — so the window in which the guessable value
 *     works is 'until the person logs in for the first time', not 'forever'"
 *     until 2026-09-04, when ตั้งรหัสผ่านของคุณ was deleted at HR's request:
 *     signing in now reaches the app, and the flag draws a standing notice on
 *     the landing tab and over the form on ข้อมูลส่วนตัว instead of a gate.
 *     **The window therefore closes when somebody changes their password, not
 *     when they first log in** — the flag is still the whole of what makes this
 *     affordable, and it is now saying so rather than enforcing it.
...
export function unsignedStaff(roster, department, signers = null) {
  /**
   * A แผนก whose หัวหน้างาน IS ฝ่ายบุคคล strands nobody, and the empty column
   * on ตั้งค่าระบบ is its answer rather than its problem — see `signedByHr` on
   * src/models/Department.js. Read off the DOCUMENT, so a caller that hands in
   * a bare id gets the old reading: a warning about a department that has one,
   * which is a false alarm on a screen, and never a silent all-clear over a
   * team nobody can sign for. That is the same direction the `signers` default
   * below is chosen for.
   */
  if (hrHeadsDepartment(department)) return [];
