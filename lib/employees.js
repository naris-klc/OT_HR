 ...
 */
export function unsignedStaff(roster, department) {
 ...
 */
export function signingCoveragePermission(before, after, department) {
  const had = new Set(unsignedStaff(before, department).map((p) => String(p._id)));
  const lost = unsignedStaff(after, department).filter((p) => !had.has(String(p._id)));
