...
/**
 * Does one PLAIN STRING match the query? — the name half of the rule above,
 * for a list whose rows are not people.
 *
 * WHAT ASKS FOR IT. The ตำแหน่ง filter on ทะเบียนพนักงาน is forty Thai job
 * titles in one dropdown, and a dropdown that long needs a box to type in —
 * `searchable` on `PickOne`. What it types against is a label, not a person:
 * there is no code to normalise and no second field to spread the terms over.
 *
 * SO IT IS NOT A SECOND SEARCH RULE, IT IS THIS ONE WITH THE CODE HALF OFF.
 * Terms rather than one string, because word order is not something anybody
 * gets right against a list they cannot see yet — ผู้จัดการ ผลิต finds
 * ผู้จัดการแผนกผลิต1. And matched with the spaces taken out as well as
 * written, because Thai sets no space between words and the ones that are
 * there are exactly the keystrokes somebody leaves out.
 *
 * @param {unknown} text one row's label
 * @param {unknown} query
 * @returns {boolean} true for an empty query — no filter is not "no matches"
 */
export function textMatches(text, query) {
  ...
  const tight = line.replace(SPACES, '');
  return terms.every((t) => {
    const term = t.toLowerCase();
    return line.includes(term) || tight.includes(term);
  });
}
