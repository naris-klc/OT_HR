 ...
 */
import { cycleTag } from './accountingCycle.js';
...
export const printName = {
   ...
   *
   * `periods` because a งวดจ่าย can be two months since 2026-09-10 (พ.ย. + ธ.ค.
   * are paid together in January). `OT-accounting-2026-11+2026-12-all` —
   * the same `cycleTag` the CSV names its file with, so the two land beside
   * each other in a Downloads folder sorted by name, which is the whole reason
   * this shape follows the exports. A single month is `[period]` and comes out
   * exactly as it always did.
   */
  accounting: ({ period, periods, company = 'all' }) => (
    `OT-accounting-${cycleTag(periods?.length ? periods : [period])}-${company}`
  ),
  /** สรุป OT แยกแผนก. */
  ...
  /**
   * คู่มือการใช้งาน — the only document here with no เดือน in it, because it is
   * the only one that is not about a month. Nothing to sort into order, so
   * nothing ISO-shaped to carry.
   *
   * THE COUNT, ALWAYS, AND NEVER "ทั้งหมด" — the same rule `formBatch` states
   * and for the same reason read one step along: the reader picks which topics
   * go in, so a subset is the ordinary case rather than the exception, and a
   * file called `คู่มือการใช้งาน.pdf` holding three of eleven topics is a file
   * that says nothing about the eight that are missing. Eleven of eleven still
   * says `11หัวข้อ`, which is true and costs a reader nothing.
   */
  manual: ({ count }) => `คู่มือการใช้งาน-${count}หัวข้อ`,
};
