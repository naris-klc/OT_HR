 ...
 *
 * ── IT USED TO SPREAD ACROSS THREE CELLS, AND NOW IT IS ONE ────────────────
 *
 ...
 * `รวมชั่วโมง` was replaced by the printed forms' `รวม 1.5` / `รวม 3` pair,
 * which these hours cannot be put into — an entry whose employee no longer
 * resolves was never split by any row — and `หมายเหตุ` became the วันเกิด
 * column, which carries one word and no sentences.
 *
 * Finding columns by NAME does not throw when a column is missing; it writes
 * nothing. Left as it was, this would have gone on returning a row of the right
 * width with a bare "ไม่ถูกนับ" in it and every other cell blank — the warning
 * with its warning removed, which reads as a stray row rather than as hours the
 * sheet could not account for. That is the failure this shape exists to avoid,
 * and it is worth more than the tidiness of a number in a numeric column.
 *
 * So the label, the hours and the sentence share the one cell that already held
 * the label. Both exports do this; there is no second form.
 */
export function unaccountedCsvRow(headers = [], unaccounted, { label = 'ไม่ถูกนับ' } = {}) {
  const row = headers.map(() => '');

  const hours = round2(unaccounted?.hours || 0);
  const count = unaccounted?.count || 0;
  const i = headers.indexOf('ชื่อ-สกุล');
  if (i >= 0) {
    row[i] = `${label} · ${hours} ชม. · ${count} รายการอ้างถึงพนักงานที่หาไม่พบ — ยอดรวมข้างบนขาดไปเท่านี้`;
  }
