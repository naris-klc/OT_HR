 ...
 *                         the roster filter is applied — `active: true` and
 *                         every บทบาท but the ADMIN account since 2026-09-08 —
 *                         and all that filter decides is whose blank line gets
 *                         printed. It read `active: true, role: 'employee'`
 *                         until that day, which left 21 of 164 active people
 *                         off both sheets.
 *
...
// Relative, for the reason this module exists apart from lib/accounting.js:
// `node --test` resolves no `@/…` alias, and the properties this sheet has to
// have are worth testing. `lib/caps.js` is alias-free for the same reason and
// is imported rather than copied — what counts as "over a ceiling" is decided
// in one place or it is decided differently by the queue and by the sheet.
import { DAY_REASONS } from '../src/lib/otEngine.js';
import { wasOverCeiling } from './caps.js';
...
/**
 * One person's over-ceiling hours on this sheet, and the sentences behind them.
 *
 * WHY THE SHEET SAYS THIS AT ALL. Every figure on สรุป OT ส่งบัญชี is a total
 * of hours somebody signed for, and until now they all read the same: a person
 * who worked 12 hours inside their department's ceiling and a person who
 * worked 12 that went past it are the same number in the same colour, and the
 * only place the difference was ever visible was the queue — a screen
 * accounting does not open, showing rows that have since been signed and gone.
 * The month closes on a figure nobody can tell apart from an ordinary one.
 *
 * So a row that contains an over-ceiling entry is coloured, and the colour
 * carries the reason its approver gave. Not a warning — the hours are approved,
 * they are correct, and they are being paid. It is a note saying this figure
 * was a decision, and here is who made it and why.
 *
 * `hours` IS NOT THE ROW'S TOTAL. It is the hours in the entries that went over
 * a ceiling, which on a person with four ordinary shifts and one long one is
 * the long one alone. The row still prints its full total; this is what the
 * tooltip quantifies, and printing the whole total there would say the entire
 * month was over a limit when one night was.
 *
 * ── WHAT COUNTS AS OVER A CEILING HERE ─────────────────────────────────────
 * `wasOverCeiling`, which counts an entry whose ceiling was later WAIVED. The
 * waive sets `capExceeded` false, so the rows accounting most needs to see are
 * exactly the ones the live flag has stopped naming. The two are told apart in
 * the output rather than merged: `waived: true` carries ฝ่ายบุคคล's waiver
 * reason, `waived: false` carries the approver's, and the tooltip prints each
 * under its own sentence. Reading them as one list would attribute HR's
 * decision to the หัวหน้า who signed the row.
 *
 * A row with no reason recorded is still counted and still coloured. Entries
 * approved before 2026-09-02 have no `overCeilingReason` — the field did not
 * exist and nobody was asked — and dropping them would make the sheet quietly
 * under-report the older half of a month. `reason: null` says so, and the
 * screen prints "ไม่ได้บันทึกเหตุผลไว้" rather than an empty quotation.
 */
export function overCeilingOf(entries = []) {
  const over = (entries || []).filter((e) => wasOverCeiling(e));
  return {
    count: over.length,
    hours: round2(over.reduce((n, e) => n + hoursOf(e), 0)),
    notes: over.map((entry) => ({
      workDate: entry.workDate || null,
      hours: hoursOf(entry),
      /**
       * The approver's sentence, or ฝ่ายบุคคล's waiver, or neither.
       *
       * An entry can genuinely carry both — a หัวหน้า said why they were
       * passing it up, and HR later waived the ceiling outright — and the two
       * answer different questions. Both are returned; the screen prints both.
       */
      reason: String(entry.overCeilingReason || '').trim() || null,
      waivedReason: String(entry.capOverride?.reason || '').trim() || null,
      waived: Boolean(String(entry.capOverride?.reason || '').trim()),
    })),
  };
}
  ...
  for (const entry of entries) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        ...
        /**
         * สังกัดหลัก — the department on the ROSTER, not the one the entry was
         * filed under. Asked for on 2026-09-08: one person, one department, the
         * one they belong to.
         *
         * ⚠ THIS REVERSED A RULE, AND THE OLD ONE IS WORTH STATING. It read
         * `entry.department` with the note "somebody who transferred mid-month
         * keeps their hours where they were worked" — which also meant a closed
         * month could not be restated by a roster edit, the same property
         * README §บริษัท names as the difference between moving somebody's
         * company and moving their แผนก. That is now gone on the department
         * side: **moving somebody to a new แผนก moves their hours out of the
         * old one in every month, including months whose sheets are signed.**
         * Nothing in the app refuses that (ปิดงวด was withdrawn), so the guard
         * is that somebody knows. Measured on the live database the day it
         * changed: 2 entries in total and **0** of them filed under a
         * department other than their owner's, so no figure moved.
         *
         * The fallback is not the old rule creeping back. It fires only when the
         * roster department cannot be resolved at all — a hard-deleted แผนก, a
         * half-finished restore — where the entry still remembering one is
         * better than dropping the row into ไม่ระบุแผนก.
         *
         * `approvesDepartments` is NOT consulted here and never was: a หัวหน้า
         * who signs for seven departments (THT0054 does) is a member of one, and
         * that one is this. The report has always listed a person once, keyed by
         * employee id — this decides WHERE, not HOW MANY TIMES.
         */
        department: entry.employee?.department || entry.department,
 ...
 *
 * ── `fold`, AND WHY ONE OF THE TWO EXPORTS NEEDS IT ────────────────────────
 *
 * Finding columns by name only works while the columns exist. On 2026-09-09
 * `accounting.csv` lost both of the ones this line writes into: `รวมชั่วโมง`
 * was replaced by the printed form's `รวม 1.5` / `รวม 3` pair, which these
 * hours cannot be put into — an entry whose employee no longer resolves was
 * never split by any row — and `หมายเหตุ` became the วันเกิด column and carries
 * no other sentence. Left alone, both `put`s would have quietly found nothing
 * and the file would have printed a bare "ไม่ถูกนับ" beside eleven blank cells:
 * a warning with its warning removed, which is worse than no line at all.
 *
 * So `fold` writes the whole line — label, hours and sentence — into the
 * ชื่อ-สกุล cell that already held its label. `departments.csv` still has both
 * columns and still uses the spread-out form, which is why this is an option
 * rather than a rewrite.
 */
export function unaccountedCsvRow(headers = [], unaccounted, { label = 'ไม่ถูกนับ', fold = false } = {}) {
  ...
  const hours = round2(unaccounted?.hours || 0);
  const say = `${unaccounted?.count || 0} รายการอ้างถึงพนักงานที่หาไม่พบ — ยอดรวมข้างบนขาดไปเท่านี้`;

  if (fold) {
    put('ชื่อ-สกุล', `${label} · ${hours} ชม. · ${say}`);
    return row;
  }

  put('ชื่อ-สกุล', label);
  put('รวมชั่วโมง', String(hours));
  put('หมายเหตุ', say);
