 ...
 *
 * `activeFrom` is 'YYYY-MM-DD' — the first day the birthday holiday was in
 * force. A DATE and not a month, because that is the precision the arithmetic
 * works at; see the note beside the comparison below.
 */
export function birthdayQueue({
  ...
  for (const period of periods) {
    ...
    for (const row of found.rows) {
      /**
       * BEFORE THE BENEFIT EXISTED — the same refusal `birthdayMonth` makes one
       * screen over, and made here to the DAY rather than to the month.
       *
       * `periods` starts at the first of a month because a scan is per period,
       * so the earliest month in the window reaches back past a rule that began
       * partway through it. Every date in that overhang is a date the compute
       * path resolves under the version in force then — the rule off, the
       * birthday an ordinary working day — so a row offered here would open
       * บันทึก OT ให้ over a form that refuses it and computes 0 hours. The
       * screen is not the place to discover that; this comparison is.
       *
       * Rows only, not `uncheckable`: "ไม่มีข้อมูลวันเกิด" is a fact about a
       * roster row and is worth saying whatever month it is.
       */
      if (activeFrom && row.date < activeFrom) continue;
