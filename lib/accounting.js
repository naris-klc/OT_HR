...
import {
  groupEntriesByEmployee, reconcile, birthdayHoursOf, overCeilingOf,
...
/**
 * The one บทบาท that gets no blank line on either sheet.
 *
 * Named rather than written into the query, because what it means there is not
 * "an administrator" — it is "the system escalation account", which on this
 * roster is one row called ผู้ดูแลระบบ (README §ADMIN). If a real person is ever
 * given the admin บทบาท and works OT, their entries still put them on the sheet
 * with their hours: this decides blank lines only. See the roster pass below.
 */
...
const shapeDepartment = (d) => (d
  // `otMode` rides along because a row of noughts means two different things
  // depending on it — see `zeroRowReason`. Carried as the raw value rather than
  // as its Thai label: the screen and the CSV each phrase it their own way, and
  // a shape that carried the sentence would decide that for both.
  ? { id: String(d._id), code: d.code, name: d.nameTh || d.name, otMode: d.otMode || 'normal' }
  : null);
...
function pendingFor(groups) {
  return {
    ...
    /**
     * ตัวคนที่ค้าง ไม่ใช่แค่จำนวน — มีอยู่เพื่อการรวมงวดสองเดือนบรรทัดเดียว.
     *
     * `count` กับ `hours` ของสองเดือนบวกกันได้ตรง ๆ แต่ `employees` บวกไม่ได้:
     * คนที่มีใบค้างทั้งพฤศจิกายนและธันวาคมคือคนคนเดียว และ "ของพนักงาน 12 คน"
     * ที่จริงเป็น 7 คนคือประโยคที่ส่งคนไปนับคิวผิด `mergePending` ใน
     * lib/accountingCycle.js นับจากยูเนียนของอาร์เรย์นี้แทน
     *
     * ไม่ใช่การเปิดเผยอะไรใหม่ — จอนี้ได้ `employee.id` ของทุกแถวอยู่แล้วจาก
     * `shapeEmployee` และเปิดให้เฉพาะ `COMPANY_REPORT_ROLES` ซึ่งเห็นคิวทั้ง
     * เดือนอยู่แล้ว
     */
    ids: groups.map((g) => String(g._id)),
  };
}
...
export async function accountingReport(period, opts = {}) {
  ...
  const [confirmed, stuck] = await Promise.all([
    /**
     * The employee's OWN department comes with them, populated inside the
     * populate — that is what makes สังกัดหลัก available to the grouping in
     * lib/accountingRows.js, which is where the choice between it and the
     * entry's department is made and explained.
     *
     * `.populate('department', …)` on the entry STAYS. It is the fallback when
     * a roster row's department cannot be resolved, and it is what the
     * ไม่ถูกนับ list names an orphaned entry's department by — see
     * `unaccountedFor`, which has no employee left to ask.
     */
    OtEntry.find({ period, status: { $in: [...ACCOUNTING_STATUSES] } })
      .populate({
        path: 'employee',
        select: 'code name position company department',
        populate: { path: 'department', select: 'code name nameTh otMode' },
      })
      .populate('department', 'code name nameTh otMode')
      .lean(),
    ...
    OtEntry.find({ period, status: { $in: [...PENDING_STATUSES] } })
      .select('employee workDate startTime endTime totals createdAt')
      .lean(),
  ]);
  ...
  if (includeZero) {
    /**
     * EVERY ACTIVE PERSON, WHATEVER THEIR บทบาท — asked for on 2026-09-08,
     * and it is a widening of exactly one thing: whose BLANK line is printed.
     *
     * It read `role: 'employee'` until then, on the reading of §2 that managers
     * do not do ordinary OT so there is nothing to print for them. Counted on
     * the live database that day, that filter was leaving **21 of 164 active
     * people off both sheets**: 10 ผู้จัดการแผนก, 7 หัวหน้างาน, 1 การเงิน,
     * 1 ฝ่ายบุคคล, 1 ผู้จัดการฝ่าย — and the ADMIN account. A หัวหน้างาน who is
     * not on the sheet at all is indistinguishable from one who was missed,
     * which is the whole reason the blank line exists: it is accounting's
     * evidence that a person was CHECKED. §2 says a manager's ordinary OT is
     * not payable; it does not say the department has fewer people in it.
     *
     * `role: { $ne: 'admin' }` IS THE ONE EXCLUSION, and it is not about
     * permissions. `ADMIN` is the system escalation account (README §ADMIN) —
     * `ผู้ดูแลระบบ`, not somebody on a payroll — and a line reading that on the
     * paper accounting signs is one more name to ask about every month.
     * Confirmed with the user on 2026-09-08.
     *
     * IT IS STILL ONLY THE BLANK LINES. Anybody who DOES have entries is
     * already in `groups` from the pass above and stays there whatever their
     * role or status is today — an admin who files OT is on the sheet with
     * their hours, and so is somebody who left in March. That order is the
     * property test/accountingReconciliation.test.js exists to hold: widening
     * or narrowing this filter can never move an hour, only a blank line.
     *
     ...
     * been run.
     */
    const roster = await Employee.find({ active: true, role: { $ne: ADMIN_ROLE } })
      .populate('department', 'code name nameTh otMode')
      .select('code name position company department')
      .lean();
  ...
  const everyRow = [...groups.values()]
    .map((g) => {
      ...
      return {
        ...
        /**
         * The hours on this row that went past a department ceiling, and the
         * sentence whoever signed them gave for it — what the red figure and
         * its tooltip are drawn from. `{ count: 0, hours: 0, notes: [] }` on
         * every ordinary row, which is most of them.
         *
         * Computed here rather than on the screen because `entries` never
         * leaves this function (see `stripEntries`), and rightly: the sheet
         * sends totals, not a month of requests. It is the same reason
         * `birthdayHours` is a number on the row and not a segment list.
         */
        overCeiling: overCeilingOf(g.entries),
        entries: g.entries,
      };
    })
    /**
     * เรียงตามลำดับตัวเลขของรหัสพนักงาน — one sort, for the screen, the print
     * and the CSV alike, because all three read this function.
     *
     * `compareCodes` and not a bare `localeCompare`, since 2026-09-03: the
     * register spells one shape two ways (PM-0412 and PM00416) and comparing
     * them character by character puts every five-digit code above every
     * four-digit one, because the padding zero beats the digit it is padding.
     * See src/lib/employeeCode.js. ตรวจสอบประจำเดือน, both its CSVs and
     * รายงาน OT แยกแผนก order by the same comparator, so the documents of one
     * month can be laid beside each other.
     */
