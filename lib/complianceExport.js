 ...
 *   period_reopen    a closed month was opened again — ผู้ดูแลระบบ only, reason
 *                    required. The one path by which a figure that has been
 *                    signed off, exported and possibly paid becomes editable.
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ONE SHAPE FOR SIX SOURCES, AND WHAT THAT COSTS
 *
 * The six live in four collections with four different schemas. Exported as
 * four files they cannot be read as one timeline, which is the only way the
...
export const EVENT_LABEL = Object.freeze({
  ...
  period_reopen: 'เปิดงวดที่ปิดแล้ว',
...
/** ปิดงวด records → the reopens. A close is not an exception; a reopen is. */
export function fromPeriodLocks(locks = []) {
  const out = [];
  for (const lock of locks) {
    for (const ev of lock.events || []) {
      if (ev?.action !== 'reopen') continue;
      out.push({
        at: ev.at,
        kind: 'period_reopen',
        actor: actorLabel({ code: ev.byCode, name: ev.byName, role: 'admin' }),
        // The month IS the target — there is no person on the receiving end,
        // and leaving it blank would read as a record with a field missing.
        target: `งวด ${clean(lock.period)}`,
        source: 'หน้าจอ',
        detail: 'ทุกใบในเดือนนี้กลับมาแก้ไขได้อีกครั้ง',
...
export function complianceRows(sources = {}, { from = null, to = null, kinds = null } = {}) {
  const rows = [
    ...
    ...fromPeriodLocks(sources.periodLocks),
...
export function complianceCells(row) {
  return [
    row.at ? new Date(row.at).toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' }) : '',
