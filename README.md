...
and the engine know nothing about Next.js, so the whole suite — **2372 tests
...
- **แถบช่องติ๊กบนฟอร์มบันทึก OT เหลือสามช่อง เรียงใหม่ และเหมารายวันแสดงเฉพาะเจ้าหน้าที่บริการ**
  ...
  **แต่เทสต์ทั้ง 2374 ข้อผ่านอยู่ตอนที่แอปคอมไพล์ไม่ผ่าน** — คอมเมนต์ JSX ใน
  ...
  ไม่มีเทสต์ข้อไหนในชุดนี้เห็น เพราะไม่มีข้อไหนคอมไพล์ไฟล์นั้น · ❓ ยังไม่ได้เดินบนจอจริง
  ในฐานะผู้ใช้ที่ล็อกอิน และยังไม่ได้ deploy — ขณะนี้ **ไม่มีอะไรรันอยู่ที่ :3000 เลย**
...
- `npm test` — **2372 tests**, about 7 s, measured 2026-09-08 across 131
  files. It read **"2374 tests, all green … about 3.4 s"** until สวัสดิการวันเกิด began
  working itself out; `birthdayTick` was rewritten whole, which is a file that
  lost more cases than it gained, and no file was added or removed.
  **"All green" is not claimed on this line any more, and the reason is not this
  round.** 32 cases across six CSS-source files — `logCardMobile`,
  `complianceCardMobile`, `detailSheetCompact`, `batchBarSticky`,
  `detailModalFooter`, plus one apiece in `tablePager`, `withdrawalRowLayout` and
  `docsMatchCode` — fail in this checkout, and every one of them fails on
  `d72fa77` with nothing edited, so they were failing before anything here was
  touched. They read `app/styles.css` and one of AGENTS.md's file references;
  none of them reads a line this round changed. **Nobody has looked at them yet
  and this line is where that is written down**, rather than a figure that says
  green over a suite that is not. It read **"2366 … across 130"** until แถบช่องติ๊กบนฟอร์มบันทึก OT was cut to
