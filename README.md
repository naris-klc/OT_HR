...
### รันเป็นคอนเทนเนอร์แทน (Linux)

`Dockerfile`, `docker-compose.yml` และ `.dockerignore` ที่รากของ repo ทำให้แอปรัน
เป็นคอนเทนเนอร์เดียวที่ต่อกับ `mongod` ซึ่งอยู่บนเครื่อง — ฐานข้อมูลไม่ได้ถูกย้าย
เข้าไปไหน และ **ไม่มีบรรทัดไหนใน `.env` ต้องแก้** รวมทั้งพอร์ต ซึ่งเปลี่ยนจาก
environment ไม่ได้อยู่แล้ว เพราะ `start` คือ `next start -p 3000` และ `-p` ชนะ
`$PORT` เสมอ ทั้งในคอนเทนเนอร์และนอกคอนเทนเนอร์

```bash
docker compose up -d --build
```

**อ่าน [docs/docker.md](docs/docker.md) ก่อนรันครั้งแรก** — วิธีนี้ผูกกับ Linux,
มันแย่งพอร์ต :3000 กับโปรเซสบนเครื่องตรง ๆ และ health check ที่ตอบ 200 ก็ไม่ได้
แปลว่าคอนเทนเนอร์เป็นตัวที่ตอบ สามข้อนั้นมีวิธีตรวจอยู่ในไฟล์นั้น

**เดินจริงแล้วบนเครื่อง Linux เครื่องหนึ่ง ตั้งแต่ 2026-09-09** และตั้งแต่บ่ายวัน
เดียวกันเครื่องนั้นออกอินเทอร์เน็ตที่ **https://primus-ot.primus-iot.com** ผ่าน
named tunnel ของ Cloudflare ซึ่งเป็น profile ที่ต้องสั่งเปิดเอง ไม่ใช่ค่าเริ่มต้น
ของ deploy — รายละเอียดทั้งหมดอยู่ใน [docs/docker.md](docs/docker.md) *(ย่อหน้านี้เคยอ่านว่า "ณ 2026-09-09 ยังไม่มี
เครื่องไหนเสิร์ฟจริงด้วยวิธีนี้")*

เครื่องนั้น**ไม่ใช่**แล็ปท็อป Windows ที่ [§Status](#status) กับ
...
**Deploying the "บันทึกแทนให้รอหัวหน้าอนุมัติด้วย" change (2026-09-09)?** No
migration and no setting to change — `proxySkipsOwnApproval` is not in
`Setting.policy` on this database, so the new shipped default (`false`) is the
effective value the moment the build is live, and the routing reads
`ctx.livePolicy` rather than the recorded version, so it does not wait for
anybody to press anything. **Press บันทึกกฎที่ใช้อยู่เป็นเวอร์ชัน anyway**: a
changed default moves the effective policy away from version 31, and until it is
recorded every entry filed carries no `policyVersionId` — the same failure the
paragraph below describes for a key joining. Entries filed before the deploy are
untouched and keep reading by the old rule.
...
> 🟢 **และตั้งแต่ 2026-09-09 มีเครื่องที่สองแล้ว** — เครื่อง Linux เครื่องหนึ่งรัน
> แอปเป็นคอนเทนเนอร์ Docker ต่อกับ `mongod` ของตัวเอง และเปิดอุโมงค์ Cloudflare
> ออกอินเทอร์เน็ตไว้ ทั้งหมดอยู่ใน [docs/docker.md](docs/docker.md) ซึ่งเป็นเอกสาร
> ของ**เครื่องนั้น** ไม่ใช่ของเครื่องนี้
>
> ย่อหน้าข้างบนกับ [docs/network.md](docs/network.md) พูดถึงแล็ปท็อป Windows
> เครื่องเดิมทั้งหมด — ตัวเลข IP, งาน Task Scheduler, และ `deploy-ot.ps1`
> ไม่มีข้อไหนใช้กับเครื่อง Linux ได้ **อย่าอ่านสองชุดนี้ปนกัน** และเมื่อไหร่ที่
> ตัดสินใจว่าเครื่องไหนคือของจริง ต้องกลับมาเขียนไว้ตรงนี้ว่าอีกเครื่องเลิกใช้แล้ว
...
ข้าง ๆ *รายงาน OT การเงิน* ที่ครบทุกแผนก — เดือนเดียวกัน สองจอ คนละยอด และไม่มี
...
> **สามตัวแรกเรียก `teamScoped` ผ่าน `departmentScope` แล้ว ไม่ได้เรียกตรง ๆ
> อีกต่อไป** ตั้งแต่ 2026-09-10 · ประโยคข้างบนยังจริงทุกคำ แต่ถ้า grep หา
> `teamScoped(` ในสามไฟล์นั้นจะไม่เจอ — สามบรรทัดที่เคยเหมือนกันทุกตัวอักษร
> ถูกรวมเป็นฟังก์ชันเดียวใน `lib/reports.js` ตอนที่ตัวกรอง `แผนก` เพิ่มคำถามที่
> สองเข้าไปที่จุดเดียวกันนี้ · ดู §`ตรวจสอบประจำเดือน กรองเป็นแผนกได้`
...
ทั้งบริษัทเพื่อกระทบยอดกับ *รายงาน OT การเงิน* และแผนกตัวเองเพราะเป็นคนเซ็น
...
**และตั้งแต่ 2026-09-10 `?department=` เดินทางเดียวกันและมีสมบัติข้อเดียวกัน** ·
`departmentScope` ใน `lib/reports.js` ตอบทั้งสองคำถามพร้อมกันให้เอกสารทั้งสามใบ:
ผู้อ่านทั้งบริษัทได้แผนกที่ขอ (ซึ่งเป็นสับเซ็ตของที่อ่านได้อยู่แล้ว) ส่วนหัวหน้า
ได้เฉพาะแผนกที่ตัวเองเซ็น และค่าที่ชี้ไปแผนกอื่น**ถูกเมิน ไม่ใช่ถูกปฏิเสธ** —
เมินคือได้ลิสต์ของตัวเองเหมือนไม่ได้ส่งอะไรมา ส่วนปฏิเสธคือแท็บค้างกลายเป็น
หน้า error · ดู §`ตรวจสอบประจำเดือน กรองเป็นแผนกได้`
...
และ รายงาน OT การเงิน ได้ทั้งบริษัท (และ รายงาน OT ประจำทีม เฉพาะแผนกที่ตัวเอง
...
- คิวถามคำถามเดียวกัน แถวแบบนี้จึง**วาดปุ่ม อนุมัติ / ไม่อนุมัติ ไว้แบบกดไม่ได้**
  พร้อมเหตุผล แทนที่จะยื่นปุ่มที่กดแล้วได้ 409 ทั้งคู่ · ย่อหน้านี้เคยอ่านว่า
  “แสดงเหตุผล**แทนที่จะแสดงปุ่ม**สองปุ่ม” ซึ่งจริงจนถึง 2026-09-14 — ดู
  *ทำไม admin ถึงอนุมัติไม่ได้ และจอบอกเรื่องนี้อย่างไร* ข้างล่าง

### ทำไม admin ถึงอนุมัติไม่ได้ และจอบอกเรื่องนี้อย่างไร — 2026-09-14

**ถามมาตรง ๆ จากหน้าจอ พร้อมภาพ:** *รายการนี้ปุ่มหายไปไหน* และตามด้วย *ผมงง ผม
เปิดใน role admin ทำไมอนุมัติไม่ได้* · คำตอบของคำถามที่สองคือกฎข้างบนนี้ทั้งดุ้น
และมันย้อนแย้งพอที่จะงงได้จริง:

- **`signedManagerStep` ถูกถามก่อน `isHr` โดยตั้งใจ** — ดู `lib/delegation.js`
  ในบล็อก `entry.status === 'pending_hr'` · มันไม่ใช่ข้อเท็จจริงเรื่อง*บทบาท*
  ใครกดปุ่มแรกไปแล้วก็หลุดจากปุ่มที่สอง ไม่ว่าเขาจะเป็นอะไร
- **และ ผู้ดูแลระบบ คือบทบาทเดียวที่เซ็นขั้นหัวหน้าแทนได้** (`mayOverrideManagerStep`
  เปิดให้ `admin` เท่านั้น ฝ่ายบุคคล ถูกกันออกโดยตั้งใจ) — บัญชีที่ทำได้มากที่สุด
  จึงเป็นบัญชีที่มีโอกาสชนกฎนี้มากที่สุด ยิ่งใช้สิทธิ์เซ็นแทนหัวหน้า ก็ยิ่งใช้
  ลายเซ็นของตัวเองไปกับใบนั้นหมดแล้ว
- **ทางออกของใบนั้นคือคนอื่น** ฝ่ายบุคคล หรือ ผู้ดูแลระบบ อีกบัญชีหนึ่ง ·
  ⚠ **ถ้าติดตั้งไหนมี `admin` บัญชีเดียวและไม่มี ฝ่ายบุคคล เลย ใบที่ admin
  เซ็นขั้นหัวหน้าไปแล้วจะค้างถาวร** — กฎนี้ไม่มีทางลัดให้ และตั้งใจไม่มี

**และคำถามแรกคือคำถามเรื่องจอ ไม่ใช่เรื่องกฎ** สิ่งที่ตอบอยู่ตรงนั้นคือขีด `—`
กว้าง 32px: เหนือ 861px `.own-note` ถูกซ่อนเพื่อคืนความกว้างให้คอลัมน์ ประโยคจึง
เหลืออยู่ในทูลทิปอย่างเดียว และทูลทิปพูดกับเมาส์ที่มาหยุดนิ่งเท่านั้น ·
**ตอนนี้ทุกแถวที่ตัดสินไม่ได้ใส่ปุ่มคู่เดิมแบบปิดไว้** (`WatchActions`) —
สั่งมาว่า *ตามเงื่อนไขเดิมไงครับ ปุ่มขึ้นแต่ disable ไว้* · ปุ่มคู่ที่เทาไม่ใช่
คำสัญญา มันคือรูปร่างของการตัดสินที่หายไป ซึ่งคือสิ่งที่คนกำลังหาปุ่มมองหา

**ป๊อปอัปเคยตอบคนละอย่างกับแถว และนั่นคือรูจริง** กล่อง รายละเอียด รับพร็อพ
`mine` กับ `watching` ซึ่งครอบเหตุผลสองในสามข้อ — คนที่เซ็นขั้นหัวหน้าของใบนั้นเอง
ไม่เข้าข่ายทั้งคู่ **กล่องจึงวาดปุ่ม อนุมัติ ที่กดได้จริง บนแถวที่เพิ่งปฏิเสธจะ
วาดปุ่มให้** กดแล้วเดินทางไปหา 409 · ตอนนี้เหลือพร็อพเดียวคือ `blocked` จาก
`blockedNote` ซึ่งถาม `signableHere` เป็นบรรทัดแรก แล้วตอบครบทั้งสามข้อ
...
lib/pdfExport.js          a print view as a PDF file: the standalone document,
                          ...
                          ⚠ IT WAITS FOR THE FILE AND NOT FOR THE PROCESS,
                          since 2026-09-10: Edge 152 hands the run to a browser
                          of its own and the process this starts returns 0
...
**คำเตือนสองใบในโมดัลพูดจบในกล่องเดียว** — `modalNoticeFlow` is the file before
it, and it is a new file rather than four more cases in `standingBanners`
because the two rules are about different things: that one is about notices
nobody asked for, this one about the two an employee opens on purpose. It read
"2659 tests across 147 files" until **หกแถบยืนพื้นเหลือสายเดียว** the same
...
"2621 tests across 145 files" until **คอลัมน์ รายการ แยกใบตามสถานะ** the same
day — `monthCountStatus` is the file and eight of the ten cases are its
...
`<tbody>` moved with it rather than being added to. And it read
"2598 tests across 144 files" until **กล่องเตือนตอนแก้วันเกิดพูดตรงกับโค้ด** —
`birthDateReplay` gained one case and NO new file, which is the shape of a
round that changes what a screen SAYS rather than what it does; and it read
...
"2487 tests across 137 files", five files behind everything this side had done
since the third merge, so once again neither side's figure survived the join;
2541 + 8 = 2549 is the one taken after it. And it read
...
- **อนุมัติเกินเพดาน is gone from every screen.** The fourth button on an HR
  ...
  red on รายงาน OT การเงิน — on the screen since 2026-09-02, and on the
...
**And the sheets say it.** On สรุป OT ส่งบัญชี **and, since 2026-09-09, on สรุป
OT แยกแผนก** the row's `รวม ชม.` is drawn in `--danger-ink` when any entry
behind it went over a ceiling — the same red the queue uses for the same fact —
with the account beside it and a hover tooltip reading `รายการเกินเพดาน |
เหตุผลผู้อนุมัติ: …`. It is **not a warning**: the hours are approved, correct
and being paid, and the colour says this figure was a decision somebody had to
justify. A tint or an amber would make an ordinary signed month look like a
problem, which is how a mark stops being read.

It read *“And the sheet says it. On สรุป OT ส่งบัญชี …”* until 2026-09-09, when
ฝ่ายบุคคล asked for the same red and the same remark on the departmental count
— *หน้ารายงาน OT แยกแผนก เพิ่มตัวอักษรแดงชั่วโมงเกินเพดาน และวันเกิด เหมือนกับ
ฟอร์มของการเงิน*. A ceiling is a **department's** ceiling, so that sheet is where
the difference between 12 hours inside one and 12 hours past it is most worth
seeing. The figure, the colour, the tooltip and the sentences are one
implementation, not two: `OverCeilingFigure` and `OverCeilingNote` moved from
...
black when the marks landed on its screen, and ฝ่ายบุคคล sent back a printed
แผนกสาขาชลบุรี with its 45.00 unmarked — *หมายถึงเกินเพดาน ตัวเลขในฟอร์ม*. It is
the right way round to want: the screen is read by the person who already knows
what they signed, the sheet by the department who does not. Both papers now use
`#c00` with `print-color-adjust: exact`, on BOTH rate cells — `overCeiling`
counts entries and hours and does not split them by rate column, so colouring
one would put a decision on the paper that the data has not made. Department
TOTALS stay black: a `รวมชั่วโมงทำOT` in red would say the department was over
its own ceiling, which is a different claim. `departments.csv` carries no
colour, having nowhere to put one.

⚠ **Red ink survives a printer and not a photocopier.** A black-and-white copy
of a signed sheet carries no mark at all. The fix, if that ever matters, is a
word in the strip beside the grid, and it is HR's and accounting's to ask for.
...
| สรุป OT ส่งบัญชี | **รายงาน OT การเงิน** | `accounting` | ฝ่ายบุคคล · ผู้ดูแลระบบ · **การเงิน** *(อ่านอย่างเดียว)* |
...
*(**`accounting` แถวนี้ถูกเปลี่ยนชื่ออีกครั้งเมื่อ 2026-09-08** — ช่อง "It reads"
คือชื่อวันนี้ ไม่ใช่ชื่อที่ตั้งเมื่อ 2026-08-31 · แท็บนี้อ่านว่า* "รายงาน OT ฝ่ายบัญชี"
*ตั้งแต่ 2026-08-31 ถึง 2026-09-08 จึงเป็นแท็บเดียวในตารางที่เคยมีสามชื่อ ·
ฝ่ายบัญชีไม่เคยเป็น*บทบาท*ในระบบนี้ ส่วน การเงิน เป็น — และเป็นบทบาทที่เปิดใบนี้
คู่กับฝ่ายบุคคล ชื่อแท็บจึงเป็นชื่อของคนที่เปิดมัน · **คีย์ไม่ขยับตามเคย** เราต์
`/api/reports/accounting/:period` ไฟล์ `accounting.csv` และ `lib/accounting.js`
ยังชื่อเดิมทั้งหมด · ย่อหน้าที่ลงวันที่ไว้ก่อนหน้านั้นในเอกสารนี้ยังเรียกชื่อเดิม
โดยตั้งใจ ด้วยเหตุผลเดียวกับที่ชื่อก่อน 2026-08-31 ยังอยู่ในซอร์ส)*
...
| การเงิน | 6 | those four · **ตรวจสอบประจำเดือน · รายงาน OT การเงิน** |
| ฝ่ายบุคคล | 7 | the พนักงาน pair · รออนุมัติ OT · ตรวจสอบประจำเดือน · รายงาน OT การเงิน · รายงาน OT แยกแผนก · ตั้งค่าระบบ |
...
**One row in the sidebar is under none of the three headings, and it is not in
`tabs` at all** — คู่มือการใช้งาน, added 2026-09-09, drawn from a second
`<nav className="nav nav-help">` below the last block and above `.sidebar-foot`.
On a phone it is a `ช่วยเหลือ` group in the drawer, above `บัญชี`. **Every
บทบาท gets it, with no condition on either row**, which is what was asked for
(*"ให้ทุกสิทธิ์ดูได้"*).

**Why it is not a tab**, since every other screen is: a push carries `bar` with
it, the phone bar is capped at four columns per บทบาท, and a ผู้เซ็น already
fills all four with one screen apiece. A twelfth push for everybody makes that
bar five wide or turns one of its four presses into a sheet — paid on every
screen every day, to reach a page somebody opens twice a week. So it takes the
route `ข้อมูลส่วนตัว` has always taken: a name in `PAGE`, no row in `tabs`, and
an entry point on the foot of both menus. It wears `.nav`'s own class rather
than one of its own so that the collapsed rail's rules — the 44px square, the
label that goes, the tooltip that replaces it — already describe it; it is a
SECOND `<nav>` because `test/navActiveTab.test.js` counts the sources of
...
**Both of its doors were broken, and neither was broken by anything in this
app.** Reported on 2026-09-10 as *บันทึกเป็นไฟล์ PDF ไม่สำเร็จ — เกิดข้อผิดพลาด
ภายในระบบ* and *หน้ากระดาษตอนปริ้นขนาดไม่พอดีกับความกว้าง A4*, and they are two
different faults that happened to arrive together.

**The button was broken for all six print views, not for the manual.** `Edge`
152 hands a headless run to a browser of its own: the process `htmlToPdf`
spawned returned 0 IMMEDIATELY, with nothing on stderr, while the real browser
wrote the PDF about 1.2 seconds later and exited ~20ms after that (measured
here). The old code took `close` as the answer, read a path that did not exist
yet, and threw `ENOENT` — which `translate()` in `lib/http.js` answers with
`เกิดข้อผิดพลาดภายในระบบ`. So a browser-version change reached HR as an internal
error. It now waits for `%%EOF` at the end of the FILE, which is browser-
agnostic; a non-zero exit ends the wait early rather than being the answer. Two
things fell out of the same read: every message in `lib/pdfExport.js` was a
plain `Error` and was therefore being replaced by that same generic sentence —
including *ไม่พบเบราว์เซอร์ … ปุ่มพิมพ์ยังใช้ได้ตามปกติ*, the one place the app
says the OTHER button still works — so those throws are now a `PdfError` the
route repeats as 503; and the temp directory could not be deleted while the
browser still held its profile, which had been leaking **6MB of Edge profile per
attempt** into `%TEMP%`.

**The paper was the manual's alone.** `@page` is `margin: 0` for the whole app
and must stay that way — F-HR-027, ใบบัญชี and ใบสรุปแผนก carry their own 8mm
bands inside a full-bleed page box, and a margin there costs a second sheet per
person. The manual is prose and has no band of its own, so once `main {
padding: 0 }` and `.manual-sheet { padding: 0 }` had both run there was nothing
between the text and the edge of the paper: measured out of the PDF, ink from
**0.0mm to 210.1mm across 209.9mm of paper**, with 2.9mm of clear at the top. A
printer cannot lay ink within about 5mm of the edge, so the first and last
characters of every line were being cut off. It now takes a 180mm column
centred on the page for the sides — a margin on a block applies to every
fragment of it, so a หัวข้อ four pages long gets the band on all four — and a
NAMED page, `@page manual`, for top and bottom, which is the only thing that can
reach a page the stylesheet never sees and the only way to give this document a
margin without giving one to the three forms. Measured after: 15.0mm and 15.2mm
at the sides, 18.0mm at the top, on every page of a four-page หัวข้อ. ⚠ Firefox
ignores `page:`, so there the top and bottom stay as they were; the sides are
the box and hold everywhere, and the PDF is Chromium whatever the reader uses.
...
รายงาน OT การเงิน ครอบทุกแผนกอยู่แล้วโดยธรรมชาติของมัน และคนที่กระทบยอด
...
they answered ***นับตามชั่วโมงที่ทำจริง (ไม่ดูนาฬิกา)***.

The row above read *“a birthday falling Mon–Fri is a holiday for that person
alone: 08:00–17:00 goes to `ot15_holiday`, the hours either side to
`ot3_holiday`”* until then, and that was one sentence with two things wrong with
it now.

- **The split is a duration, not a clock.** A birthday was an ordinary วันหยุด,
  cut at the core-hours boundary exactly as a Saturday is. It is cut at the
  eighth hour of work instead — `birthdayFirstTierMinutes()` in
  [`src/lib/otEngine.js`](src/lib/otEngine.js), which is
  `flatDailyMinutes()`: core hours less the break, so a company that moves its
  office day moves this with it and there is no second number in ตั้งค่าระบบ for
  the two to disagree through. `applyBirthdayTiers()` is the pass that walks the
  segments and re-buckets them, splitting one where the eighth hour lands
  mid-stretch. **Most days do not move**: 08:00–20:00 is eight hours ×1.5 and
  three ×3 under either reading. What moves is work that does not start in the
  morning — **17:00–20:00 on one's own birthday was three hours ×3 and is now
  three hours ×1.5**, because it is the first three hours of the day.
- **The birthday now wins over a Saturday, a Sunday and the company calendar.**
  `resolveDayTypes()` asks the birthday FIRST. It asked it last for as long as
  the two answers were the same answer, and this file said so: *“a birthday that
  already falls on a Saturday, a Sunday or a company holiday adds nothing”*. The
  two now give different figures for the same shift — 20:00–23:00 is ×3 as a
  Saturday and ×1.5 as a birthday — so “adds nothing” had become a decision
  about which one to pay, taken silently on the ground that there was nothing to
  decide. HR decided it: ***ใช้กฎวันเกิดแทน***.

**Counted per date, and one day is one request.** A shift that begins on the
birthday and runs past midnight leaves the far side under that day's own rules;
a shift that runs INTO a birthday starts the count at nought at midnight. There
is no second entry for the first eight hours to have been spent by, because
หนึ่งวัน หนึ่งใบ (§หนึ่งวัน หนึ่งใบ) will not let two requests share a date.

**Measured after the lunch hole and before rounding.** An 08:00–17:00 birthday
is eight hours of work and lands wholly in ×1.5, which is what it read before
this rule existed. Somebody who ticks ไม่พักเที่ยง has worked nine, so the ninth
is ×3 — which is the rule doing what it says rather than an edge case.
...
alongside `dayType`. **One of the three is read by the arithmetic, and only one.**
This paragraph read *“The arithmetic never reads it”* until 2026-09-08, and that
was true for as long as a birthday was somebody's private Saturday; the tier rule
above is decided on `dayReason === 'birthday'`, so that value now changes a
...
**On สรุป OT แยกแผนก's SCREEN since 2026-09-09, and on nothing it prints or
exports.** This read *“Not on สรุป OT แยกแผนก. That sheet goes to management, not
to accounting, and it has no figure that reads as an error without the remark —
so adding it there would disclose something for no reason. It is HR's call to
make, not a consistency argument, and it is pinned as absent.”* until ฝ่ายบุคคล
made that call: *หน้ารายงาน OT แยกแผนก เพิ่ม … วันเกิด เหมือนกับฟอร์มของการเงิน*.

**And on its printed bundle since the same afternoon**, asked for in the same
conversation and answered in one line: *ให้มี — แบบเดียวกับใบส่งบัญชี*. So the
old paragraph's stated reason — *“that sheet goes to management”* — is not the
line any more, because management now gets the word. What IS the line, and was
the line all along, is **THE MONTH MAY BE DISCLOSED; THE DATE MAY NOT**: the
strip carries the word วันเกิด alone, no hours and no date, exactly as the
accounting paper's does, and neither `DepartmentPrint.jsx` nor
`DepartmentView.jsx` reads a `birthDate` or a `dayReason` to draw it — only the
hours the report already counted. `test/birthdayOnPaper.test.js` pins that.

The screen phrases it for its own columns — `วันเกิด · 8.00 ชม. คิดอัตราวันหยุด`,
...
birthday it is, on the ordinary OT form, and the request goes to the หัวหน้า and
then to ฝ่ายบุคคล like every other request. Two signatures, §6 unbent, one door.

#### There is no ช่องวันเกิด — the system works it out

**HR, 2026-09-08**: *เอาตัวเลือก “วันเกิด (สวัสดิการวันเกิดของตัวเอง)” ออก แต่
ให้ระบบรู้อัตโนมัติ*. From 2026-09-03 to that morning there was a **วันเกิด**
tick on the filing form and a second one inside `แก้ไขชั่วโมง`, and this section
read:

> It is **a claim, not a label**. Ticking it fills 08:00–17:00 into the two time
> boxes … and then the server checks it: `birthdayTickRefusal()` in
> `lib/entries.js` reads the `dayTypes` map `loadContext` already resolved and
> answers with a sentence when the day is not what the tick says.

**The paragraph under it is why the box could go**, and it was there from the
first day: *“It does not decide the hours.”* `resolveDayTypes()` has always read
the person's stored วันเกิด under the policy in force on the work date and
answered holiday-because-birthday, so an untouched form filed on one's own
birthday computed and paid exactly as a ticked one. The tick's only effects were
to fill in 08:00–17:00, to put a word on the หัวหน้า's queue — and to give
somebody a way to be wrong about a thing the server had already decided.

So the whole apparatus went with it: the two boxes, `birthdayTickRefusal()`, the
409 on three routes, the `birthdayRefusal` field the preview answered, and the
`mayCorrect` prop that existed to decide who was shown the second box. **Nothing
replaced them**, because nothing was left to check — the date IS the claim now,
and moving the date is what moves the answer.

**Nothing was ever stored, and that has not changed.** The tick was not a field
on `OtEntry`, and every screen that marks such a row reads it back off
`segments[].dayReason` (`isBirthdayWelfare()`). A second copy on the entry could
disagree with the engine, and there would be no way to tell which one was lying.

**What the person is told instead.** The form reports rather than asks: the ⓘ
line at the top says the day is theirs and that every hour of it counts, and an
`Alert` above the split says the first eight hours are ×1.5 and the rest ×3 —
`isOwnBirthday()` in [`components/OtForm.jsx`](components/OtForm.jsx), read off
the `dayReason` of the very segments drawn underneath, so the sentence and the
figures cannot disagree. It is still a round trip to the preview, for the same
reason the refusal was: `publicEmployee()` keeps `birthDate` off the roster the
browser holds.

**What it cost, said plainly.** A person who mistyped the DATE used to be told
so in words. They are told by the split instead — the notice appears when the
date is right and does not when it is wrong — which is a quieter signal than a
red banner and a greyed button. The trade is one control that could be wrong,
one refusal, one preview field and one prop out of the system.
...
There were **two refusals**, because the two mistakes had different ways out: a
tick on an ordinary workday (“pick the day that matches ทะเบียนพนักงาน”) and a
tick on a Saturday or a company holiday, which answered that the day was already
วันหยุด for everybody and that the hours **still counted in full**. Both are
gone, and the second is doubly gone: since 2026-09-08 a birthday on a Saturday
IS a สวัสดิการวันเกิด — see the tier rule above.
...
**และวันเกิดไม่ใช่เหตุผลที่จะปฏิเสธใบใด ๆ อีกแล้ว — 2026-09-08.** Two refusals
have stood in this place and both are withdrawn. It read *“และวันเกิดของตัวเอง
ยื่นเองไม่ได้เลย”* from **2026-08-31** to **2026-09-03**, when สวัสดิการวันเกิด
was a day the company granted and ฝ่ายบุคคล recorded off the fingerprint
scanner's export, and `birthdayOtRefusal` answered 409 on both write paths. From
**2026-09-03** to **2026-09-08** it read the opposite — `birthdayTickRefusal()`
answered 409 when the **วันเกิด** box was ticked and the day was not one, *“the
commonest correction there moves `workDate`, and without it a ticked request
could be filed on the one day the tick is true and dragged onto any other”*.

**The correction that paragraph existed for is still made, by arithmetic.** Both
write paths and the preview build their own `ctx.dayTypes` on every save, so a
request dragged off a birthday loses the birthday rates in the same act that
moved it, and one dragged onto somebody's birthday gains them. Nothing has to be
re-ticked, and nothing can be left ticked on the wrong day. See
§สวัสดิการวันเกิด above for what went with the box.

**Two things it deliberately did NOT refuse, and both are simply the ordinary
...
withheld on a proxy filing, since it would tell a หัวหน้า when their team member
was born; the hours are unaffected either way.

It used to be withheld in a second case as well — *“withheld when the box IS
ticked, since the ⓘ line at the top of the form has already said it”* — which
went with the box on 2026-09-08. The ⓘ line is still the other half of the
sentence, and it is chosen by the same `isOwnBirthday()`: the ordinary
*“เวลาทำงานปกติ จันทร์–ศุกร์ 08:00–17:00 น.”* is false on a สวัสดิการวันเกิด,
where every hour of the day counts.

**ฝ่ายบุคคล had a second copy of the box in `แก้ไขชั่วโมง` for one day** —
2026-09-07 to 2026-09-08, inside รายละเอียด on รออนุมัติ OT (`QuickEdit` in
[`components/ApprovalQueue.jsx`](components/ApprovalQueue.jsx)) — *“because a
request filed with the box in the wrong state had one way out of that screen and
it was ไม่อนุมัติ”*. It was shown only to ฝ่ายบุคคล and ผู้ดูแลระบบ, through a
`mayCorrect` prop carrying `mayCorrectEntries` down from the queue, so a
หัวหน้า could not be taught somebody's birth date through a refusal.

**Both the box and the prop are gone.** `mayCorrect` decided that one control
and nothing else, and a บทบาท rule threaded through two components with nothing
left to read it is the shape somebody re-uses six months later for a control it
does not answer for. Whether a reader may save a correction at all is still
answered, by `editPermission` on the press.

It read *"**เหมารายวัน stays and stays ungated** — whether a day was hired whole
is not a private fact about a person — and the ตำแหน่ง rule on it is on the
**filing form only**, because this panel is where a tick filed wrongly gets put
right"* **until 2026-09-10**, when HR closed it in one sentence:
*เหมารายวันเห็นเฉพาะตำแหน่งเจ้าหน้าที่บริการ HR และ admin เท่านั้น*. The panel now
asks **both** halves — the ตำแหน่ง of the person the row is FOR, and
`mayCorrectEntries` of the reader — so `mayCorrectEntries` is back in
[`components/ApprovalQueue.jsx`](components/ApprovalQueue.jsx), read inside
`QuickEdit` beside the control it decides rather than threaded down as a prop.
That is the shape the deletion above was arguing for, and it is the whole of the
difference: a rule read where it is drawn, not carried to it.
...
correction panel on รออนุมัติ OT asks the ตำแหน่ง of **the person the row is for**
...
**`แก้ไขชั่วโมง` on รออนุมัติ OT draws it too since 2026-09-10** — HR gave the
...
stands; `overTime` is a flag beside it, built exactly as missingOtStart was
until that one was withdrawn on 2026-09-09 (see below).
...
**There was a second MARK beside them and it is gone.** ไม่ได้สแกนเข้า OT — the
start of an OT that no door event witnessed — was drawn quiet from 2026-09-04
until ฝ่ายบุคคล withdrew it on 2026-09-09. See §ไม่ได้สแกนเข้า OT was withdrawn.

**On a `flatDaily` row none of the three verdicts draws** — the green
เหมารายวัน chip does instead, whatever the verdict. See below.
...
is what the direction test below is the answer to. The end is safe unbounded
...
since 2026-09-09 it is the whole of what the start side says. What used to be
LEFT over when it is false — a start with no witness — was the field
missingOtStart and a grey chip, and is now nothing at all.

| the day's punches, against a request `17:00–19:30` | before | now |
|---|---|---|
| `07:34, 19:30` — two punches, the common shape (**Case A**) | ⚠️ `เวลาเริ่ม ไม่มีสแกนใกล้เคียง` | ✅ no chip — the scan-out reached 19:30 · it read "⬜ ไม่ได้สแกนเข้า OT only" until 2026-09-09 |
...
| `07:21, 17:02, 17:35, 20:05` against **`18:00–20:00`** | ⚠️ `เวลาเริ่ม สแกน 17:35 ก่อนเวลา 25 นาที` | ✅ no chip (20:05 is 5 นาที over — under the 30-นาที block) |
...
##### ไม่ได้สแกนเข้า OT was withdrawn — 2026-09-09

There was a grey chip on those Case A rows, and it is gone. It was asked for on
2026-09-04, hours after the silence above was built: *"แสดง Badge/Flag Warning …
ไม่ได้สแกนเข้า OT เพื่อเตือนว่าไม่มีสแกนเข้าช่วง 17:00 น."* — on the reading that
the **verdict** answers *does somebody have to look at this row* (no) while a
**mark** says what the machine witnessed, and that the second is worth printing
even where the first is silent.

ฝ่ายบุคคล withdrew it on 2026-09-09: **ไม่ต้องแจ้งเตือนเพราะปกติพนักงานก็ไม่สแกน
กันอยู่แล้ว** — which is the same sentence the chip's own tooltip had been
carrying since the day it shipped. At 17:00 the person is already inside and
never touches the door, so the chip landed on **25 rows of 27** and said the
same thing about all of them.

**Grey did not save it, and that is the lesson worth keeping.** Grey was chosen
precisely because the count was known: a grey chip on twenty-five rows is a
column of labels rather than twenty-five errands. What a real month showed is
that a column of identical labels is read once and then skipped for good, while
still taking the width in จาก–ถึง that the marks which ARE about one row need.
**A mark true of nearly every row is not evidence about any row, whatever colour
it is drawn in.** These rows have now been marked twice — amber, then grey — and
unmarked twice, for the same reason both times. It read:

> **So it is grey, and the grey is the argument.** It lands on 25 rows of 27 —
> the same count that made the amber unreadable. `.chip.scan-noin` is its own
> class rather than `.chip.scan-none`'s … **The `title` carries the second
> half** … **It draws on neither a flat day nor a `no_scan` row.**

**What went with it**: the field missingOtStart on the check, the two strings
MISSING_OT_START, the gate showsMissingOtStart, the component
...
because it is really about `SCAN_MATCH_TOLERANCE_MINUTES`. The helper this was
asked for in names those clock times because that is what the shift is, and for
a request beginning at 17:00 the two agree exactly. They part on two requests
the window cannot see: one filed `18:00–20:00`, where 17:00–17:30 is not where
its start is at all, and one whose OT-in scan is at 17:40 — where the window
reports "no scan" while a scan sits right there disagreeing by 40 นาที. The
tolerance answers both, and it is the number ฝ่ายบุคคล can still move.
...
two badges** — เหมารายวัน beside a scan badge — and at 191px of content they do
not fit on one line. The pair that was measured in 2026-09-07 was
ไม่ได้สแกนเข้า OT beside `ไม่ครบ · ขาด 47 นาที`; the first of those was withdrawn
on 2026-09-09 and the wrapping rules stand, because two chips still meet here. They wrapped flush against each other, a
...
the queue, รายงาน OT การเงิน and รายงาน OT แยกแผนก, so it belongs on the rule.
...
**And since 2026-09-09 the ลงชื่อหัวหน้างาน column names the หัวหน้า who typed
it in.** The step was skipped because they were the person who would have signed
it, so the column says who that was instead of dropping to the ฝ่ายบุคคล
confirmation underneath — or printing nothing while the row waits at รอ HR. The
rule is `managerSignature` in [`lib/approverLine.js`](lib/approverLine.js) and it
reads the skip note this section describes, so a filing that skipped nothing
still prints blank. See §Outputs.
...
same **สถานะที่นับ**, same **แผนก**. Somebody with no OT that month has no row and gets no
...
### ตรวจสอบประจำเดือน กรองเป็นแผนกได้
...
whole company, and every question HR actually asks of it — *has ผลิต1 gone over
its ceiling? print ผลิต3's sheets* — is a question about one แผนก out of the
eighteen. **ทุกแผนก** is the first row of the list and is where the screen opens,
so nothing about an unnarrowed month changed.

**It is a request to the server, and that is the whole difference between it and
ค้นหา beside it.** The search box narrows what is *drawn* out of a month already
fetched: it costs no request, cannot move a total, and the screen prints a hint
saying so. แผนก goes into the URL, so the month comes back already cut — and
therefore **every figure on the screen is that department's**:

| what | follows แผนก | follows ค้นหา |
|---|---|---|
| the rows | ✓ | ✓ |
...
the month, and a ceiling narrowed by one control and not the other is that same
false negative by another route.

**`?department=` was already understood by all three routes and no screen had
ever sent it.** What the round added on the server is not the parameter but
`departmentScope` in `lib/reports.js` — the three lines each of
`app/api/reports/monthly/[period]/route.js`,
`app/api/exports/entries.csv/route.js` and `app/api/exports/monthly.csv/route.js`
had been carrying as identical copies, now one function they all call. They were
identical because they *had* to be: the table and the two export buttons under it
are one screen, and a scope rule that drifted between them is a CSV that does not
hold the rows its own table showed.

**It can only ever narrow, and that is what makes it safe to accept from a URL.**
The property is the one `teamScoped` already held for `?scope=`:

- ฝ่ายบุคคล, ผู้ดูแลระบบ and การเงิน on ตรวจสอบประจำเดือน read every แผนก, so
  the parameter hands them a subset of what they had.
- A **หัวหน้า** gets one of the แผนก they sign for. A `?department=` naming one
  they do **not** sign for is *ignored* — not honoured, which would hand them
  hours that are not theirs, and not refused, which would turn a stale tab into
  an error page. They get their own list, which is what they would have had
  without the parameter.

So the dropdown is drawn on **รายงาน OT ประจำทีม** as well, rather than hidden
there: a ผู้จัดการฝ่าย signs for eight แผนก and reads all eight on that tab, and
is the reader with the most use for it. The list offered is
`coversDepartments` — the same list the server will honour and nothing wider —
so a หัวหน้างาน holding one แผนก sees ทุกแผนก and their own, which narrows
nothing and is the same one-row list คิวรออนุมัติ accepted rather than have a
toolbar that is a different shape for every บทบาท.

**The options come from `GET /api/departments`, not from the rows.** A list built
...
heading (which does not scroll away, unlike the dropdown two rows below it); the
แผนก cell of the **รวมทั้งหมด** row, which was blank because there had never
been anything true to put in it; and the empty month, which says
**ไม่มีรายการของ "ผลิต2" ในเดือนนี้** with **ดูทุกแผนก** under it rather than
**ไม่มีรายการในเดือนนี้** — a sentence that with one แผนก chosen is very nearly
a lie, since the month may be full and this is one department of eighteen that
filed nothing.

**⚠ It cuts the month by the แผนก ON THE ENTRY — where the hour was worked,
snapshotted when the request was filed.** That is deliberately the same dimension
this screen's own แผนก column prints, so the dropdown, the column, รวมทั้งหมด and
both CSVs all cut the month the same way. **It is not the dimension รายงาน OT
แยกแผนก and รายงาน OT ฝ่ายบัญชี use** — those group by the employee's สังกัดหลัก
on the roster, which since 2026-09-08 is read fresh every time a sheet is built
(`groupEntriesByEmployee`, and see [`test/reportDimension.test.js`](test/reportDimension.test.js)).
The two answers differ for exactly one person: somebody whose แผนก was moved on
...
| `.hr-table th.cap-col` | 224 *(shared)* | *"152"* → **140** | the longest thing in it is `รวมรออนุมัติ 45 / 40` — *"~128px"* was an estimate and 107.2px is the measurement; 140 still keeps 9px over it |
...
| `.hr-table th.who-col` | 168px shared, sized so the widest *token* does not break | *"196px"* → **168px again** since 2026-09-11 รอบสาม — it still holds `นางสาวสสุคนธ์ ข่าค่ำ` whole (128.6px measured, 152.6 with the gutters), and the 196 was never a measurement, only what was left of the 194 |
...
what is above the buttons now is ประจำเดือน, แผนก, ค้นหา and สถานะที่นับ, and not
one of the four is a tag the operating system draws a list for. It read
"ประจำเดือน, ค้นหา and สถานะที่นับ … not one of the three" until 2026-09-10,
when แผนก joined the row.) The
...
> see §ตรวจสอบประจำเดือน กรองเป็นแผนกได้ below for why it is in THAT row and not
> on the heading line with สถานะที่นับ.
...
a desktop and a stack of three on a phone; two is a row. That is also the whole
of the answer to where แผนก went on 2026-09-10: the same objection applies
unchanged, and a row of controls takes a third without changing shape.
...
### รายงาน OT การเงิน — the submission sheet

*(The tab read* "รายงาน OT ฝ่ายบัญชี" *from 2026-08-31 until 2026-09-08, and*
"สรุป OT ส่งบัญชี" *before that. The key, the route, the CSV and
`lib/accounting.js` are all still `accounting`.)*
...
**Two marks on the rows, added 2026-09-09 at ฝ่ายบุคคล's request** — *เหมือนกับ
ฟอร์มของการเงิน*, and they are literally the accounting screen's: `รวม ชม.` goes
`--danger-ink` on a row any of whose entries was signed past its department's
ceiling, carrying the approvers' own sentences in a `title`; the same account is
written out in full below, together with `วันเกิด · N ชม. คิดอัตราวันหยุด` where
the month holds วันหยุดวันเกิด hours — `คิดอัตราวันหยุด` and not ส่งบัญชี's
`อยู่ในช่องวันหยุด`, because this table has no วันหยุด column and a birthday's
hours land in 1.50 and 3.00 by the ordinary holiday rule.
...
under the row it explains, the same block is 211px wide and 199px tall on a phone
and 776px wide on a desktop. `ลำดับที่` and `ชื่อ-นามสกุล` stay real (empty)
cells in that row and the span covers only the four columns after them — the name
is frozen below 860px, and a frozen column with a hole in it would let a remark
travel under the names when the sheet is scrolled sideways. The row is drawn only
when there is something to say, so an ordinary month is the table it always was.

`OverCeilingFigure`, `OverCeilingNote` and `BirthdayNote` all live in
`components/common.jsx` and both screens call them — see §เกินเพดานแล้วยังเซ็น
for why the marks are one implementation and where each screen still decides for
itself.

**And on the paper, after a second ask the same afternoon.** The marks went to
the screen first and ฝ่ายบุคคล sent back a printed สาขาชลบุรี whose 45.00 was
still black — *หมายถึงเกินเพดาน ตัวเลขในฟอร์ม* — then asked for วันเกิด on it as
well. `components/DepartmentPrint.jsx` now reddens both rate cells of a marked
row and carries the word วันเกิด in a white strip right of the yellow total.
**Nothing ruled moved**: the strip is 26mm of the 32mm of blank paper the sheet
already left to the right of its grid, the table went from 152mm to 178mm, and
the four columns and the yellow cell keep the millimetres they had — a printout
...
- **ป้ายในคอลัมน์ `รายการ` ได้คำว่า `รอ` — และ 40px ที่มันกินถูกจ่ายจากคอลัมน์
  ...
  `รอHR 1`*
  · **`หัวหน้า 1` เอ่ยชื่อคน แล้วปล่อยให้ผู้อ่านเติมกริยาเอง** — คอลัมน์นี้มีไว้ตอบ
  ว่า *ของคนนี้ยังมีอะไรค้างอยู่ไหม* คำว่ารอจึงเป็นคำที่ตัดออกไม่ได้ ·
  `รอหัวหน้า n` กับ `รอHR n` เขียนติดกันตามที่สั่ง ไม่มีวรรคระหว่าง `รอ` กับ `HR`
  · **ไม่ใช่การเปลี่ยนคำเฉย ๆ** — บรรทัดที่ใช้ร่วมกันโตจาก 81.6px เป็น 109.5px
  **วัดจากไฟล์ฟอนต์จริง** (IBM Plex Sans Thai Looped 600 ที่ 11.5px ดึง `hmtx`
  มาบวกเอง ไม่ใช่กะจากจำนวนตัวอักษร) · บวกช่องไฟสองข้างของเซลล์ข้างละ 12px เป็น
  133.5 ปัดเป็น **`count-col` 136px** ด้วยวิธีเดียวกับที่ `act-col` ปัด 98 เป็น 104
  · **บนตาราง `table-layout: auto` ความกว้างที่ประกาศไม่ใช่กล่องที่ตัดเซลล์ทิ้ง
  มันคือส่วนแบ่งของแถว** — ประกาศเกินไป 40px เบราว์เซอร์ไม่ได้ไปหาจากขอบตาราง
  แต่ดึงจากคอลัมน์เดียวที่ตัดคำได้ ซึ่งคือ `แผนก` และ **`แผนก` แตกกลางคำเป็นบั๊ก
  ที่ตารางนี้เคยเป็นมาแล้วสองครั้ง**
  · **จึงจ่ายก่อน ไม่ใช่ยืม** — `who-col` 196 → **168** กับ `cap-col` 152 → **140**
  รวม 40px พอดี · ทั้งคู่ยังกว้างกว่าของที่มันถือจริง — `นางสาวสสุคนธ์ ข่าค่ำ`
  วัดได้ 128.6px (ต้องการ 152.6 รวมช่องไฟ) และ `รวมรออนุมัติ 45 / 40` วัดได้
  107.2px (ต้องการ 131.2) · **`dept-col` ไม่ถูกแตะ** เพราะมันคือคอลัมน์ที่ตัดคำ
  และเป็นสิ่งที่การย้ายนี้มีไว้ป้องกัน
  · ⚠ **"196" ไม่เคยเป็นค่าที่วัดมา** — มันคือเศษของ 194 ที่ `act-col` คืนให้เมื่อ
  2026-09-10 แล้วถูกใช้ไปเพราะมันว่างอยู่ · เหตุผลที่เขียนไว้ให้มัน (*holds
  `นางสาวสสุคนธ์ ข่าค่ำ` whole*) ยังเป็นจริงที่ 168 เหมือนกัน
  · ⚠ **เคสเดียวที่ 136 ไม่พอคือเลขสองหลักทั้งสองฝั่งพร้อมกัน** —
  `รอหัวหน้า 12 · รอHR 12` วัดได้ 123.3px ต้องการ 147 · **ปล่อยให้ล้นแทนที่จะซื้อ
  เพิ่ม** เพราะคนที่มีสิบใบค้างพร้อมกันทั้งสองขั้นในเดือนเดียวไม่ใช่แถวที่ตารางนี้
  ถูกอ่านเพื่อหา และอีก 11px จะต้องไปเอาจาก `แผนก` · `nowrap` ยังอยู่ มันจึงดัน
  คอลัมน์ให้กว้างขึ้น ไม่ใช่ตัดบรรทัด
  · **เทสต์ 2649/2649 ผ่าน** — `monthCountStatus` ได้เคสที่เก้า ตรึงความกว้าง
  สามคอลัมน์เป็น *งบ* ไม่ใช่สามตัวเลข: ผลรวมของ `count-col + who-col + cap-col`
  ต้องเท่าเดิม (96+196+152) และ `dept-col` ต้องยังเป็น 160 · เคสป้ายตรึงทั้งป้าย
  ไม่ใช่ชิ้นส่วน เพราะ `HR 1` แมตช์อยู่ข้างใน `รอHR 1` และจะผ่านทั้งสองแบบ
  · บิลด์ผ่านบน distDir แยก
  · ⚠ **ยังไม่ได้เดินด้วยตาบนหน้าจอ**

- **คอลัมน์ `รายการ` ออกแบบใหม่ — จำนวนใบทั้งเดือน แล้วเขียนเฉพาะส่วนที่ค้าง**
  — 2026-09-11 รอบสอง · แจ้งมาพร้อมภาพหลังเห็นของจริงว่า *ผมว่าจะต้องออกแบบใหม่ครับ
  ดูแล้วเข้าใจยาก*
  · **ภาพบอกความผิดสองข้อพร้อมกัน** — แถวหนึ่งอ่านว่า `11` แล้วข้างใต้เป็น
  `11  1` · เลขบนนับตามตัวกรอง สามเลขล่างนับทั้งเดือน **ถูกทั้งคู่ แต่คู่กันแล้ว
  อ่านเหมือนตารางที่บวกไม่ลง** และสีจางที่ใส่ไว้อธิบายช่องว่างนั้น กลายเป็นของอีก
  ชิ้นที่ต้องถอดรหัสแทนที่จะเป็นคำอธิบาย · ข้อสอง **เกือบทุกช่องเป็นขีด** เพราะ
  `อนุมัติ` แทบเท่ากับเลขรวมทุกแถว สองในสามของความกว้างคอลัมน์กับคำอธิบายทั้งแถว
  จึงหมดไปกับการพูดซ้ำสิ่งที่ผู้อ่านมีอยู่แล้ว
  · **สิ่งที่แก้ไม่ใช่วิธีแสดงช่องว่างที่ดีกว่า แต่คือเอาช่องว่างออก** — ทั้งเซลล์
  อ่านจาก `monthStatus` ฐานเดียว เลขรวมคำนวณเป็นผลบวกของสามสถานะ
  (`m.approved + m.pendingMgr + m.pendingHr`) ไม่ใช่ `entryCount` อีกต่อไป
  **ตัวเลขจึงบวกกันได้โดยโครงสร้าง** ไม่ใช่โดยบังเอิญจนกว่าจะมีคนหรี่ตัวกรอง
  · **เขียนเฉพาะส่วนที่ค้าง** ไม่มีเลข `อนุมัติ` เลย เพราะมันคือเลขรวมลบสิ่งที่
  เขียนไว้ · คนที่ไม่มีอะไรค้างได้เลขเปล่า ๆ **แถวว่างจึงเป็นคำตอบ ไม่ใช่การไม่มี
  คำตอบ** และตากวาดลงมาเจอตัวหนังสือเฉพาะแถวที่ต้องทำอะไร
  · **ยังสองบรรทัดเหมือนเดิม** มีทั้งสองอย่างก็แชร์บรรทัดเดียว
  (เคยเป็น `หัวหน้า 1 · HR 2` *"ราว 90px ในคอลัมน์ 96px"* — ป้ายได้คำว่า `รอ`
  ในรอบสามของวันเดียวกัน จึงเป็น `รอหัวหน้า 1 · รอHR 2` 109.5px ในคอลัมน์ 136px
  และ 90 กับ 96 นั้นก็กะเอา ไม่ได้วัด) และบรรทัดนั้นสั่ง `nowrap` ไว้ —
  ถ้าแคบเกินจะล้นให้เห็น ไม่ใช่ตัดบรรทัดเงียบ ๆ แล้วทำให้ทุกแถวในตารางสูงขึ้น
  · **ของที่ถูกถอดทิ้งทั้งชุด** — คำอธิบายสามคำที่หัวคอลัมน์ (`.count-legend`) ·
  กริดสามช่อง · ขีดแทนศูนย์ · กฎ `.out` ที่ทำสีจาง · และ `counted` ในคอมโพเนนต์
  ซึ่งไม่เหลืออะไรให้ตัดสินแล้ว · หัวคอลัมน์กลับมาเป็นคำเดียวว่า `รายการ` พร้อม
  `title` ที่บอกข้อเดียวที่ผู้อ่านมองแล้วเดาเองไม่ได้ — ว่ามันไม่ขึ้นกับ `สถานะที่นับ`
  · **เทสต์ 2648/2648 ผ่าน** — `monthCountStatus` เขียนใหม่ทั้งไฟล์ แปดเคส
  ตรึงว่าเลขรวมเป็นผลบวกไม่ใช่ `entryCount` · ตรึงว่าไม่มี `cs-a` ไม่มีขีด
  ไม่มี `.count-legend` ไม่มี `.out` ไม่มี `counted` · ตรึงว่า `nowrap` ยังอยู่
  · บิลด์ผ่านบน distDir แยก
  · ⚠ **ยังไม่ได้เดินด้วยตาบนหน้าจอ**
...
- **คอลัมน์ `รายการ` แยกใบตามขั้นที่มันค้างอยู่ — และนับทั้งเดือนจริง
  ...
  · ⚠️ **ข้อนี้ยืนอยู่ไม่กี่ชั่วโมง** เคยเขียนไว้ว่า *สามช่องจึงไม่บวกกันได้เท่า
  เลขรวมที่ตัวกรองแคบ ๆ และตั้งใจ — ช่องที่ตัวกรองไม่ได้นับวาดด้วย `--muted-2`
  ไม่ใช่ซ่อนทิ้ง* · **ถูกแจ้งจากหน้าจอว่าอ่านไม่รู้เรื่อง** และของที่แก้จริงคือ
  เอาช่องว่างนั้นออก ไม่ใช่หาวิธีอธิบายมันให้ดีขึ้น — ดูข้อบนสุด
  ...
  · **คอลัมน์ได้ความกว้างเป็นครั้งแรก** *"96px"* (เดิม `auto` ราว 46) —
  136px ตั้งแต่รอบสามของวันเดียวกัน เมื่อป้ายได้คำว่า `รอ` · จัดกึ่งกลาง
  ...
  **รอบสามวัดจากไฟล์ฟอนต์จริงแล้ว** และสิ่งที่เจอคือ 96 ไม่เคยพอตั้งแต่แรก:
  `หัวหน้า 1 · HR 2` ต้องการ 105.6 รวมช่องไฟ ส่วนต่างถูกดึงจาก `แผนก` เงียบ ๆ
  มาตลอด · ส่วนการเดินด้วยตายังไม่ได้ทำ
...
- **ช่อง เวลาเริ่ม กับ เวลาสิ้นสุด ในแผง แก้ไขชั่วโมง กลับมาอยู่บรรทัดเดียวกัน**
  ...
  *ล็อก 08:00–17:00 น. แก้เวลาไม่ได้* ทำให้สองช่องสูงไม่เท่ากัน และ `.row` ของทั้งแอป
  เรียงตาม **ขอบล่าง** (`align-items: flex-end`) ซึ่งถูกสำหรับแถวที่จับคู่ช่องกรอกกับ*ปุ่ม*
  แต่ที่นี่ทั้งสองเป็น*ช่องกรอก* · แก้ด้วยกฎเดียว `.quick-edit .row { align-items: flex-start; }`
  ป้ายตรงกัน ช่องตรงกัน · **แล้วบรรทัดล็อกเองก็ออกจากคอลัมน์ในรอบถัดมา** —
  *ให้มันตรงกับช่องเวลาเริ่ม* · มันพูดถึง**ทั้งสองช่อง** (ติ๊กเดียวปิดทั้งคู่) จึงเป็น
  บรรทัดของ*ทั้งแถว* กินเต็มความกว้างและเริ่มตรงที่ เวลาเริ่ม เริ่ม แทนที่จะเริ่มกลางแผง
  ใต้ช่องที่มันไม่ได้พูดถึง (`.quick-edit .row > .field-note`) · **ฟอร์มยื่นย้ายประโยค
  เดียวกันออกจากคอลัมน์เดียวกันเมื่อ 2026-09-09 แล้วไปชิดขวา** (`.field-note.lock-note`)
  — กฎเดียวกัน สองคำตอบ: บรรทัดอยู่ใต้ช่องที่มันพูดถึง และแถวนั้นมี วันที่เริ่ม
  นำหน้าเวลาสองช่อง ส่วนแถวนี้มีแค่เวลาสองช่อง · **`flex-start` ถูกเก็บไว้ทั้งที่วันนี้
  ไม่เปลี่ยนอะไรให้เห็นแล้ว** เพราะมันคือประโยคที่จริงสำหรับแถวที่มีแต่*ช่องกรอก*
...
- **แถบช่องติ๊กในแผง แก้ไขชั่วโมง บน รออนุมัติ OT เหลือสองช่อง และแต่ละช่องมีกฎของตัวเอง**
  — 2026-09-10 · สั่งมาสามประโยคขณะอ่านแผงนั้นอยู่: *ตัดข้ามคืนออกไปเลย* ·
  *เหมารายวันเห็นเฉพาะตำแหน่งเจ้าหน้าที่บริการ HR และ admin เท่านั้น* ·
  *ช่องติ๊กไม่พักกลางวันขึ้นเฉพาะวันหยุดบริษัทและวันเสาร์อาทิตย์* ·
  **สองในสามคือกฎที่ฟอร์มยื่นได้ไปแล้วเมื่อ 2026-09-08** — จอที่*แก้*ใบตามจอที่*ทำ*ใบ
  ช้าอยู่สองวัน และทั้งคู่เรียก `isFlatDailyPosition` / `isCompanyOffDay` ตัวเดียวกัน
  ใน `lib/entries.js` ไม่ได้เขียนกฎซ้ำคนละที่ · **`เหมารายวัน` ต้องผ่านสองประตูที่ถาม
  คนละคำถาม**: ตำแหน่งของ*เจ้าของใบ* กับบทบาทของ*คนอ่าน* (`mayCorrectEntries` —
  predicate เดียวกับที่ `editPermission` ยอมรับการแก้ จอจึงไม่วาดช่องที่ route ตอบ 403)
  · **`ไม่พักเที่ยง` อ่านปฏิทินของ `entry.workDate`** ไม่ใช่ preview และไม่ใช่วันที่สอง
  ของกะข้ามคืน · **ช่องที่ติ๊กไว้แล้วยังเห็นเสมอทั้งสองช่อง** ซึ่งเป็นความปลอดภัยทั้งหมด
  ของกฎที่*ซ่อนตัวควบคุม* · **`ข้ามคืน` ตัดทั้งช่อง แต่ธงไม่ได้หายไปไหน** —
  `endsNextDayFor` ยังถูกถามทุกครั้งที่เวลาขยับ ค่ายังถูกส่งไปกับ preview และ PATCH
  และยังขึ้นสองที่ก่อนเปิดแผง (ป้ายบนแถว · `เวลาที่ขอ` ในป๊อปอัป) · กฎ CSS
  `.check.derived` ถูกลบพร้อมช่องที่มันวาด
  ✅ `npm test` **2448/2448** ผ่าน 2026-09-10 (เดิม 2438 · `quickEditChecks`
  เป็นไฟล์เทสต์ที่ 135)

- **ชื่อพนักงานที่ยาวเลิกทับคอลัมน์วันที่บนคิวรออนุมัติ** — 2026-09-10,
  *หน้ารออนุมัติ เวลาที่เปิดแถบบาร์ด้านข้าง ชื่อของพนักงานที่ยาว ๆ มันจะทับกับวันที่*.
  **`nowrap` ไม่ได้ตัดข้อความทิ้ง มันวาดทับเซลล์ข้าง ๆ** — `.who-name` อยู่ในรายการ
  `white-space: nowrap` ของคิว นามสกุลที่ยาวเกินเซลล์จึงถูกวาดทับ `21/08/2569`
  · **สิ่งที่ขยับคือทะเบียน ไม่ใช่คอลัมน์** — 168px ของ `th.who-col` ถูกวัดกับทะเบียน
  ตัวอย่างที่ไม่มีคำนำหน้า ส่วนทะเบียนจริง 164 คนที่เข้ามาเมื่อ 2026-09-08 ต่อคำนำหน้า
  ติดกับชื่อทุกคน · **แก้ด้วยการให้ชื่อขึ้นบรรทัดที่สองได้ ไม่ใช่ขยายคอลัมน์** — ชื่อที่
  ยาวที่สุด `นางสาวจุฬาลักษณ์ ทรัพย์ประเสริฐ` กว้าง 209.6px จะต้องใช้คอลัมน์ 234px
  ขณะที่สิบสองคอลัมน์ของฝ่ายบุคคลเหลือที่ว่าง 8px ที่ 1440.
  **⚠ กฎ CSS อย่างเดียวหยุดการทับได้ แต่ตัดผิดที่** — เบราว์เซอร์ไล่เติมบรรทัดให้เต็ม และ
  ภาษาไทยมีจุดตัดคำระหว่างทุกคำไม่ว่าจะมีช่องว่างหรือไม่ `นางสาวพรพรรณ บุญเรือง` จึงออกมา
  เป็น `นางสาวพรพรรณ บุญ` เหนือ `เรือง` — นามสกุลถูกผ่าครึ่งและช่องว่างค้างอยู่ปลายบรรทัดแรก
  · **หกจากหกชื่อที่ตัดบรรทัด ตัดในนามสกุลทั้งหมด** และ `word-break: keep-all` กับ
  `line-break: strict` ขยับจุดตัดไม่ได้สักตัวอักษร (ลองทั้งคู่บนแถวชุดเดียวกันในรอบเดียวกัน —
  สองตัวนั้นพูดแทน CJK ส่วนไทยไม่ได้อยู่ในคลาสที่มันพูดถึง) · **อีกครึ่งจึงอยู่ในคอมโพเนนต์**
  `WhoName` ใส่ทุกคำไว้ใน `.nb` คลาสเดียวกับที่รหัสพนักงานข้าง ๆ ใช้ และปล่อยช่องว่างระหว่างคำ
  เป็นข้อความเปล่าของบล็อกแม่ — คำจึงตัดไม่ได้ ช่องว่างตัดได้ และนั่นคือจุดตัดเดียวที่เหลืออยู่
  · **ทางเดียวที่จะทับได้อีกคือคำเดี่ยวที่กว้างกว่าเซลล์** ซึ่งความกว้างห้ามไว้: ไม่มี token ไหน
  บนทะเบียนกว้างกว่าเซลล์ (กว้างสุด `นางสาวฟ้าประทาน` 115.4px ต่อเนื้อที่ 145.2 ที่ 1440
  และ 144 ที่พื้นความกว้างของตาราง)
  · ✅ `npm test` **2438/2438** ผ่าน 2026-09-10 (เดิม 2430 · `queueNameWrap` เป็นไฟล์เทสต์ใหม่)
  · ✅ **วัดบนแอปจริงด้วย CDP** — ทะเบียนจริง คิว 313 แถว เปิดกฎแล้วปิดกฎที่ความกว้างเดิม:
  ก่อนแก้ 116 แถวล้นที่ 1440 (แย่ที่สุด 42px) · หลังแก้ 0 แถว ทั้งที่ 1920 / 1440 / 1280
  และทั้งตอนเปิดกับตอนพับแถบข้าง · **และอ่านจุดตัดออกมาดูด้วย** — หลังใส่ `WhoName` แล้ว
  หกจากหกชื่ออ่านว่า `นางสาวพรพรรณ` / `บุญเรือง` คือตัดที่ช่องว่าง นามสกุลเต็มทุกชื่อ · **ราคาของบรรทัดที่สองวัดแล้ว** ตารางสูงขึ้น 184px
  ต่อ 313 แถว (27675.9 → 27859.8) แถวกลางกับแถวที่สูงที่สุดไม่ขยับเลย
  · ✅ **build ผ่านบน `distDir` แยก** และ :3000 ไม่ถูกแตะตลอดรอบ — ยังไม่ได้ deploy
...
- **ช่องลงชื่อหัวหน้างานบน F-HR-027 ขึ้นชื่อหัวหน้าที่กดบันทึกแทน** — 2026-09-09,
  ...
  *เพราะผู้บันทึกคือผู้ที่จะอนุมัติเอง ระบบจึงข้ามขั้นนั้นมา* และช่อง หัวหน้างานอนุมัติ
  ขึ้น *ข้ามขั้นหัวหน้า — ผู้บันทึกคือผู้อนุมัติเอง* ส่วนใบพิมพ์ตกไปหยิบชื่อ**ฝ่ายบุคคล**
  มาใส่ในช่องหัวหน้างาน หรือพิมพ์ว่างไว้ตอนใบยังรอ HR
  · **อ่านที่โน้ตบนแถวการยื่น ไม่ใช่ที่ `submit_proxy` เฉย ๆ** — แผนกจัดซื้อกับ
  แผนกทรัพยากรมนุษย์มีฝ่ายบุคคลเป็นหัวหน้างาน*ตามกติกา* ใบที่ยื่นแทนในสองแผนกนั้น
  ไปถึงรอ HR โดยไม่ผ่านโต๊ะใครเลย และต้องเว้นว่างต่อไปจนกว่าจะมีคนเซ็น · สาขานั้น
  ไม่เขียนโน้ตโดยตั้งใจ (`skipped` เป็น false) โน้ตจึงเป็นสิ่งเดียวบนแถวที่แยก
  *ผู้บันทึกเซ็นด้วยการบันทึก* ออกจาก *ไม่มีใครถูกข้าม* · ถ้าปิด
  `proxySkipsOwnApproval` ก็ไม่มีอะไรถูกข้าม ใบรออยู่ที่รอหัวหน้า และช่องพิมพ์
  ลายเซ็นที่เกิดขึ้นจริง
  · **การอนุมัติ ในป็อปอัพไม่ได้เพิ่มแถว และนั่นไม่ใช่กระดาษกับจอไม่ตรงกัน** —
  `approvalSteps` แสดง*การกดปุ่ม* และใบที่ข้ามขั้นไม่มีใครกดอนุมัติเลย การเพิ่มแถว
  ตรงนั้นคือระบบอ้างว่ามีการอนุมัติที่ไม่เคยเกิด ซึ่งเป็นคำโกหกที่
  `proxySkipsOwnApproval` มีไว้ปฏิเสธพอดี
  · ✅ `npm test` **2398/2398** ผ่าน 2026-09-09 (เดิม 2391 · เจ็ดเคสใหม่ ไม่มีไฟล์ใหม่ ·
  ทรีตอนวัดได้ 2392 เพราะมีงานอีกรอบหนึ่งค้างอยู่ในไดเรกทอรี จึงเก็บงานนั้นเข้า stash
  แล้ววัดใหม่บนทรีที่จะ commit จริง)
  · ✅ **เดินบนแอปที่ build แล้วที่ :3004** (`VERIFY_DIST_DIR=.next-verify-sig` ·
  :3000 กับ :3001 มีคนใช้อยู่ ตรวจก่อนแล้วและไม่ถูกแตะ · ปิดเซิร์ฟเวอร์และลบ distDir
  แล้ว) — **ทั้งฐาน 340 ใบมี `submit_proxy` อยู่สองใบ และเปลี่ยนทั้งสองใบ**:
  `15/07/2569` ของ `PM00112 นางสาวรณพร สอนฤทธิ์` จาก `"นางสาวนันทิตา ธรรมนู"`
  (ชื่อฝ่ายบุคคลที่กดยืนยันทีหลัง) เป็น `"นายรุ่งโรจน์ ขำคำ"` และ `09/09/2569`
  ใบที่ยังรอ HR จาก `null` เป็น `"นายรุ่งโรจน์ ขำคำ"` — สองใบนี้คือสองอาการที่
  อธิบายไว้ข้างบน ใบละอาการพอดี
  · ✅ **ใบธรรมดาไม่ขยับ** อีกหกแถวของเดือน ก.ค. ที่หัวหน้าเซ็นตามปกติยังอ่านชื่อ
  เดิมทุกแถว
...
- **แท็ก ล่วงหน้า / ย้อนหลัง ใต้วันที่บนคิวรออนุมัติ** — 2026-09-09,
  ...
  *ไม่ต้องมีสก็อลบาร์เลื่อนๆ* คอลัมน์วันที่ไม่ได้ถูกแตะเลยในรอบนั้น ที่ให้คือคอลัมน์ที่กำลัง
  ...
  ข้อ *ยื่นย้อนหลังได้ถึงกี่วัน* ใน docs/hr-briefing.md ยังเปิดอยู่เหมือนเดิม เห็นกับห้ามเป็นคนละเรื่อง
...
- **`สถานะที่นับ` บน ตรวจสอบประจำเดือน เปิดมาที่ `อนุมัติแล้ว + รอ HR`** —
  ...
  · **เทสต์ผ่านทั้งหมด** (อ่านว่า "2380 เทสต์ผ่านทั้งหมด" จนถึง 9 ก.ย. 2569 ที่ป้ายเทาถูกถอน) · build ผ่านบน `distDir` แยก (`.next-verify`
...
- **คิวขั้นแรกโชว์เฉพาะใบที่ยังไม่ได้อนุมัติ — จอ ฝ่ายบุคคล ไม่ขยับ** — 2026-09-09 ·
  ...
  โชว์แค่ใบที่ยังไม่ได้อนุมัติ แต่ของ HR คงไว้เหมือนเดิม* · **ถอนครึ่งหนึ่งของรอบ
  ...
  `maySignFirstStep` และ `signableHere` ไม่ถูกแตะ ที่เปลี่ยนคือ query string ที่จอยิงออกไป
  · **กองที่ดูอย่างเดียวบนจอหัวหน้าไม่ได้หายไป เหลือแบบเดียว** — ใบขั้นเดียวกันที่
  routing matrix ไม่ได้ให้เซ็น (แผนกที่มีหัวหน้างานสี่คน) และใบของตัวเอง
  · ประโยคที่สี่ของ `watchingNote` (`ผ่านขั้นของคุณแล้ว — รอฝ่ายบุคคลยืนยัน`) ถูกลบพร้อม
  แถวที่มันอธิบาย และ `isHr` ออกจากพารามิเตอร์ของฟังก์ชันไปด้วย · ประโยคใต้ตารางของ
  คิวว่างเคยอ่านว่า "`และจะอยู่ต่อจนฝ่ายบุคคลยืนยัน`" ตอนนี้บอกว่าใบจะหายไปเมื่อ
  อนุมัติแล้ว และชี้ไป `รายงาน OT ประจำทีม`
  · **เทสต์ผ่านทั้งหมด** (อ่านว่า "2380 เทสต์ผ่านทั้งหมด" จนถึง 9 ก.ย. 2569 ที่ป้ายเทาถูกถอน) และ build ผ่านบน `distDir` แยก (`VERIFY_DIST_DIR=
  .next-verify-queue`, เสิร์ฟที่ :3005 ตอบ 200 แล้ว kill ทิ้ง · :3000 ไม่ถูกแตะ ตรวจแล้ว
  ยังตอบ 200) · ในบันเดิลที่ build แล้วอ่านได้ว่า `j=u&&!o&&!d` และ `g=j?no:[n]` — คือ
  `wholeFlow` กับ `listed` ตัวจริงที่คอมไพล์ออกมา
  · ⚠ **ยังไม่ได้เดินด้วยตาบนหน้าจอ** — ไม่ได้ mint JWT ของผู้เซ็นแล้วเปิดคิวจริง
  ที่ตรึงไว้คือเทสต์ที่อ่านซอร์สกับบันเดิล

- **ช่องลงชื่อหัวหน้างานว่างบนแถว `รอหัวหน้า` — ตรวจซ้ำตอน merge 2026-09-09**
  รายการข้างบนสองรายการมาจากคนละสาย และรายการแรกพิงข้อเท็จจริงที่รายการที่สอง
  แก้พอดี: *`managerSignature` ตอบ `null` บนแถวที่ยังไม่มีใครกดอนุมัติ* — ซึ่งเป็น
  เหตุผลทั้งหมดที่ `draft` ปลอดภัย เพราะกล่องเปล่าคือหน้าตาของบรรทัดที่ยังไม่เซ็น
  · **ยังจริงหลังรวม** — `managerSignature` เติมชื่อจากสามทางเท่านั้น: `approve_mgr`
  (มีคนกดอนุมัติจริง) · `isOwnApprovalFiling` (ยื่นแทนแล้วข้ามขั้น ซึ่ง `initialStatus`
...
- **แถบช่องติ๊กบนฟอร์มบันทึก OT เหลือสามช่อง เรียงใหม่ และเหมารายวันแสดงเฉพาะเจ้าหน้าที่บริการ**
  ...
  **เป็นกฎว่าวาดอะไร ไม่ใช่ว่ารับอะไร** ทางเขียนไม่ถูกแตะ — `flatDaily` ยังเป็นฟิลด์ที่
  ...
  ไหนบนจอเอาออกได้.
  **เคยอ่านว่า "`แก้ไขชั่วโมง` บน รออนุมัติ OT ไม่ถูกแตะทั้งสองข้อ" จนถึง 2026-09-10** —
  ...
  มาก่อนรอบนั้นแล้ว ดูบรรทัด `npm test` ท้าย §Status) · ✅ **build ผ่านแล้ว** บน distDir แยกที่ :3001 — `/` ตอบ 200
...
- **คิวของผู้เซ็นขั้นแรกเห็นใบทั้งสาย และช่อง บทบาท มาแทนช่องสถานะบนจอนั้น** — 2026-09-04 ·
  ⚠️ **ครึ่งแรกของหัวข้อนี้ถูกถอนเมื่อ 2026-09-09** — คิวขั้นแรกกลับไปโชว์เฉพาะใบที่ยัง
  ไม่ได้อนุมัติ ทุกอย่างที่บันทึกไว้ข้างล่างเกี่ยวกับแถว `รอ HR` บนจอผู้เซ็น (คอลัมน์ที่ 12,
  ประโยค `ผ่านขั้นของคุณแล้ว`, `รอ HR 2` ในหัวการ์ด และประโยค `และจะอยู่ต่อจนฝ่ายบุคคล
  ยืนยัน` ใต้ตาราง) จึงเป็น**บันทึกของวันนั้น ไม่ใช่จอวันนี้** · ช่อง บทบาท, `OPENS_ON`,
  แถบตัวกรองบนคิวว่าง และขอบเขตหลายแผนก **ยังอยู่ครบทุกข้อ** — ที่ถอนคือ*ใบที่ลิสต์*
  ...
  — **⚠ ท้ายกล่องที่ว่างเปล่าเป็นของวันนั้น ไม่ใช่ของวันนี้** ตั้งแต่ 2026-09-14
  กล่องที่ตัดสินไม่ได้มีแถบท้ายกล่องเสมอ เป็นปุ่มคู่เดิมแบบปิดไว้ และบรรทัดแรก
  พูดครบทั้งสามเหตุผล ไม่ใช่เหตุผลเดียว — ดู *ทำไม admin ถึงอนุมัติไม่ได้* ข้างบน
  ...
  — **⚠ สองประโยคนี้เป็นของวันนั้น ไม่ใช่ของจอวันนี้** ตั้งแต่ 2026-09-11 ประโยคอ่านว่า
  `ยังไม่ถึงขั้นของคุณ — รอหัวหน้าแผนกเซ็นก่อน` (ไม่ใช่ `ยังไม่ถึงขั้นอนุมัติ` — ขั้นหัวหน้า
  ก็เป็นการอนุมัติ ประโยคที่แปลตรงตัวจะกลายเป็นประโยคที่ผิด) แถวนั้นได้**ปุ่ม อนุมัติ /
  ไม่อนุมัติ แบบปิดไว้**แทนขีด และปุ่มบนแถว รอ HR อ่านว่า **อนุมัติ** ไม่ใช่ ยืนยัน แล้ว —
  ดู docs/features.md แถวสุดท้ายของหัวข้อ `รออนุมัติ OT`
...
- It read "**2689 tests**, about 4 s, measured 2026-09-14 across 150 files" until
  ...
  fixture rows and no DOM. It read "2667 tests … across 149" until that round.
  **`modalNoticeFlow` is the file before it** — คำเตือนสองใบในโมดัลพูดจบใน
  กล่องเดียว, the last commit of `docs/plan-notice-compact.md`, and its four
  cases are as much about what the shorter notices still have to SAY as about
  the shape: three of the four read back the facts the merge was not allowed to
  drop. It read "2663 tests … across 148" until that round.
  ...
  **"2634 tests … across 146"** until ทะเบียนพนักงาน
  and สรุป OT ส่งบัญชี took the shared pager later the same day — thirteen cases
  as §12 and §13 of `tablePager`, no new file. Before that it read
  **"2621 tests … across 145"** until คอลัมน์ รายการ
  ...
  **"2598 tests … across 144"** until the วันเกิด
  warning was made to agree with the route it describes — one case in
  `birthDateReplay`, no new file. Before that it read **"2561 tests … across 143"** until reload was
  ...
  **"2541 tests … across 142"** until
  ...
  this side, so neither figure survived the join; 2541 + 8 = 2549 is the one
  measured after it. Before that it read **"2532 tests … across 141"** until
  the monthly report learned to say which rows this reader may confirm
  (`monthApprovable`, nine cases). Before that it read
  **"2521 tests … across 140"** until ผลเทียบสแกน
  ...
  **"2449 tests … across 135"** until ตรวจสอบประจำเดือน grew its
  แผนก filter later the same day — one new file (`monthDepartmentFilter`) and
  fifteen cases, three of them in files that already existed and whose pinned
  source shapes that round changed on purpose. And it read
  **"2378 tests … across 131"** until ot-hardening-and-slips was
  merged a SECOND time the same day — seven more commits and one new file,
  ...
  cases and the branch's **"2380, all green"** never saw `birthdayTick`
  rewritten. Before that it read **"2374 tests, all green … about 3.4 s"**
  ...
  **"All green" is back on this line, and the reason is not this round either.**
  ...
  and rewriting those to say the new thing (and to ban the old shape) is the
  work — not a count that goes up. It read **"2391 … across 132"** until ช่องลงชื่อหัวหน้างาน started naming the
  หัวหน้า who filed a row on somebody's behalf — seven cases, no new file, all in
  `test/formSignatures.test.js`, because the change is one more step in one pure
  function. **THE WORKING DIRECTORY READ 2392 AND THAT FIGURE IS NOT THIS TREE'S**:
  another round's edits were sitting uncommitted beside these, so the count was
  taken again with them stashed and the sum is 2391 + 7.
  It read **"2387 … across 132"** until the เกินเพดาน red and the วันเกิด strip
  ...
  **THE "2377" IT REPLACES WAS TWO HIGH** — the commit before this one actually
  ran 2375, measured in a throwaway worktree at that commit — so the sum is
  2375 + 12 and not 2377 + 12. **Measure the tree you are committing, not the
  tree you are sitting in**: this working copy also held unrelated uncommitted
  work whose two extra cases are not in this commit and are not in this figure.
  It read **"2375 … across 131"** until รายงาน OT แยกแผนก gained the เกินเพดาน
  and วันเกิด marks — two more cases in `overCeiling` and NO new file, because the
  marks themselves moved into a component both report screens call. It read
  **"2380 … across 131"** until the grey `ไม่ได้สแกนเข้า OT` chip was
  withdrawn — the count went DOWN by five, which is what taking a feature out looks
  like: หมวด `1ข` of `scanMatch` was six cases guarding a chip and is now one
  guarding its absence. It read **"2379 … across 131"** until ไม่พักเที่ยง became a red highlight —
  ...
  is the same strip of tick-boxes under a second HR sentence. It read **"2366 … across 130"** until แถบช่องติ๊กบนฟอร์มบันทึก OT was cut to
