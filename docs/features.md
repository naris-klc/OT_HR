...
| กล่องรายชื่อ + แผงเปลี่ยนหน้า บนมือถือ | ต่ำกว่า 860px รายชื่ออยู่ในกล่องสูง `42dvh` (ขั้นต่ำ 260px) เลื่อนดูข้างในได้ · ในกล่องเรียงเป็น การ์ด → แผงเปลี่ยนหน้า → `รวมทั้งหมด` · หน้าละ 5 คน ปุ่มหัวท้าย `disabled` ไม่ใช่ซ่อน · `overscroll-behavior: contain` กันไม่ให้ปัดจนหลุดไปเลื่อนหน้าข้างหลัง · หน้าที่หายไปเพราะรายการสั้นลงถูก clamp ตอน render · เปลี่ยนเดือน/สถานะ/พิมพ์ค้นหา กลับหน้า 1 · **สองกลไกต่อหนึ่งรายการ** (เลื่อนในกล่อง กับ เปลี่ยนหน้า) ซึ่งเป็นราคาที่จ่ายโดยตั้งใจ เขียนเหตุผลไว้เหนือ CARD_PAGE ใน `components/HrView.jsx` · เดสก์ท็อปซ่อนเฉพาะ `.pager-row` ส่วน `.off-page` **ไม่มีกฎเลย** จึงวาดครบทุกแถว | `components/HrView.jsx` · `app/styles.css` (`.hr-table tbody` · `.pager-row` · `.off-page`) | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 กล่องสูง 328px เนื้อใน 920px · ในกล่องมี 7 แถว = 5 คน + แผง + ยอดรวม · ลำดับในพิกัดของกล่องเอง: การ์ดใบสุดท้าย 618 → แผง 769 → ยอดรวม 854 · ปุ่มทั้งสอง 101×44 อยู่ในกล่องและกดได้จริง (ยิง hit-test กลางปุ่มแล้วชี้กลับมาที่ปุ่มเอง) · `ก่อนหน้า` disabled ที่หน้า 1 · กด `ถัดไป` แล้วเป็น `หน้า 2 / 5` `แสดง 6–10 จาก 25 รายการ` · เลื่อนกล่องจนสุดแล้วหน้าไม่ขยับ · 1440px ไม่มีกล่อง วาดครบ 26 แถว แผงถูกซ่อน ยอดรวม `static` |
| `วันเกิดของเดือนนี้` ไม่ตกจอ | อยู่นอกกล่อง ต่อจากกล่องทันที และเพราะกล่องสูงคงที่ ส่วนนี้จึงอยู่ที่เดิมเสมอไม่ว่าจะมีพนักงานกี่คนหรืออยู่หน้าไหน | `app/styles.css` (`.hr-table tbody` · `.month-card` order) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 หัวข้ออยู่ที่ y=483 ห่างจากก้นกล่อง 26px ขณะแถบล่างอยู่ y=703 (เคยวัดได้ y=499 ตอนกล่องสูง 58dvh ยังไม่มีแผงเปลี่ยนหน้าอยู่ข้างใน) |
| `รวมทั้งหมด` เป็นพื้นของกล่อง | ต่ำกว่า 860px แถวยอดรวมเป็น `position: sticky; bottom: 0` ติดก้นกล่อง เห็นตลอดไม่ว่าจะเลื่อนไปแถวไหนหรืออยู่หน้าไหน · เงาชี้ขึ้นเพราะเป็นพื้น ไม่ใช่แถบลอยทับ · เดสก์ท็อปเป็น `static` | `app/styles.css` (`.hr-table tbody tr.total-row` · `.table-wrap.card-list`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — ติดก้นกล่องทุกตำแหน่งการเลื่อน · **มี 1 ใน 15 จุดเลื่อน (ทีละ 40px) ที่มันบังกลางปุ่มเปลี่ยนหน้า** เลื่อนต่ออีกนิดก็พ้น และตรงตำแหน่งที่แผงมาหยุดจริง ๆ (สุดกล่อง) ปุ่มกดได้ทั้งคู่ · เคยเป็น `bottom: calc(82px + env(safe-area-inset-bottom))` ตอนที่ยังลอยเทียบกับ viewport |
| กดเปลี่ยนหน้าแล้วกลับไปบนสุดของกล่อง | แผงเปลี่ยนหน้าอยู่ก้นกล่อง คนกดจึงกดตอนเลื่อนสุดแล้ว — ถ้าไม่รีเซ็ต `scrollTop` การ์ดห้าใบใหม่จะถูกวาดเหนือจอที่ยังมองแผงอยู่ กดแล้วเหมือนไม่มีอะไรเกิดขึ้น · รีเซ็ตเมื่อ `current`/`find`/เดือน/สถานะ เปลี่ยน · เป็น ref ไม่ใช่คำถามเรื่อง layout — เดสก์ท็อปไม่มีกล่อง `scrollTop` เป็น 0 อยู่แล้ว | `components/HrView.jsx` (`listRef`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — **เจอบั๊กนี้จากการเดิน ไม่ใช่จากการอ่านโค้ด** ก่อนแก้ ตำแหน่งเลื่อนของกล่องค้างที่ 592 หลังกด `ถัดไป` · หลังแก้เป็น 0 |
