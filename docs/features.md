...
| กล่องรายชื่อสูงคงที่บนมือถือ | ต่ำกว่า 860px รายชื่อพนักงานอยู่ในกล่องที่สูง `58dvh` (ขั้นต่ำ 260px) เลื่อนดูข้างในเอง · วาดครบทุกแถวไม่ซ่อนใคร ไม่มีปุ่มแบ่งหน้า/โหลดเพิ่มอีกแล้ว · `overscroll-behavior: contain` กันไม่ให้ปัดจนหลุดไปเลื่อนหน้าข้างหลัง · ไม่มีค่าคงที่ในคอมโพเนนต์อีกต่อไป (เคยเป็น `CARD_FOLD` → `CARD_PAGE`+`CARD_STEP` → `CARD_PAGE`) ความสูงอยู่ในสไตล์ชีตที่เดียว · เดสก์ท็อปไม่มีกล่อง เพราะกฎไม่ได้ใช้กับมัน ไม่ใช่เพราะถูกยกเลิก | `app/styles.css` (`.hr-table tbody`) · `components/HrView.jsx` | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 กล่องสูง 452px เนื้อใน 3947px · **26 แถวใน DOM ซ่อน 0** · เลื่อนกล่องจนสุดแล้วหน้าไม่ขยับเลย · คนสุดท้ายของเดือนเลื่อนถึงได้ · พิมพ์ `PM-09` เหลือ 6 คน · 1440px `overflow-y: visible` `max-height: none` วาดครบ 26 แถว · เคยตั้ง 44dvh หนึ่งรอบแล้วสั้นไป (1.7 การ์ด ยอดรวมทับการ์ดที่สอง) |
| `วันเกิดของเดือนนี้` ไม่ตกจอ | เพราะกล่องรายชื่อสูงคงที่ ส่วนนี้จึงอยู่ห่างจากยอดรวมเท่าเดิมเสมอ ไม่ว่าจะมีพนักงานกี่คน | `app/styles.css` (`.hr-table tbody` · `.month-card` order) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 หัวข้ออยู่ที่ y=499 ขณะแถบล่างอยู่ y=703 · อยู่ในจอทั้งตอนกล่องยังไม่เลื่อนและตอนเลื่อนจนสุด |
| `รวมทั้งหมด` เป็นพื้นของกล่อง | ต่ำกว่า 860px แถวยอดรวมเป็น `position: sticky; bottom: 0` ติดก้นกล่องรายชื่อ เห็นตลอดไม่ว่าจะเลื่อนไปแถวไหน · เว้นจากขอบกล่อง 12px ตาม padding ของรายการ · เงาชี้ขึ้นเพราะเป็นพื้น ไม่ใช่แถบลอยทับ · เดสก์ท็อปเป็น `static` เหมือนเดิม | `app/styles.css` (`.hr-table tbody tr.total-row` · `.table-wrap.card-list`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — ติดก้นกล่องทั้งตอนบนสุดและตอนเลื่อนสุด · เคยเป็น `bottom: calc(82px + env(safe-area-inset-bottom))` ตอนที่ยังลอยเทียบกับ viewport เหนือแถบล่าง ตอนนี้ไม่ต้องคำนวณแถบล่างแล้ว |
