...
| หน้าละ 5 คน บนมือถือ — ไม่มีกล่องเลื่อนซ้อน | ต่ำกว่า 860px รายชื่อเป็นการ์ด **หน้าละ 5 คน** แล้วเลื่อนด้วยสกรอลล์ของหน้าเว็บตามปกติ · **ไม่มีกล่องเลื่อนซ้อนอีกแล้ว** — `.hr-table tbody` ไม่มี `max-height`/`overflow-y` และ `.table-wrap.card-list` เป็น `overflow: visible` (จำเป็น เพราะ `overflow-x: auto` ทำให้แกน y กลายเป็น `auto` ตามไปด้วย) · เรียงเป็น การ์ด 5 ใบ → แผงเปลี่ยนหน้า → `รวมทั้งหมด` → `วันเกิดของเดือนนี้` · ปุ่มหัวท้าย `disabled` ไม่ใช่ซ่อน · หน้าที่หายไปเพราะรายการสั้นลงถูก clamp ตอน render · เปลี่ยนเดือน/สถานะ/พิมพ์ค้นหา กลับหน้า 1 · แถวที่ไม่ได้อยู่ในหน้านี้ใส่คลาส `off-page` **ไม่ได้ `slice` อาร์เรย์** เพราะเดสก์ท็อปไม่มีแผงเปลี่ยนหน้า ถ้า slice เดือนจะเหลือ 5 คนบนจอใหญ่โดยไม่มีปุ่มไปหาที่เหลือ · เดสก์ท็อปซ่อนเฉพาะ `.pager-row` ส่วน `.off-page` **ไม่มีกฎเลย** จึงวาดครบทุกแถว | `components/HrView.jsx` · `app/styles.css` (`.hr-table tbody` · `.pager-row` · `.off-page`) | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 เดือนที่ปั้นไว้ 25 คน (มีใบอนุมัติ 24) · ไล่ทุก element ที่ `overflow-y` เลื่อนได้ **ได้ลิสต์ว่าง** · `.hr-table tbody` คำนวณออกมาเป็น `max-height: none` `overflow-y: visible` · หน้าสูง 3861px · วาด 7 แถว = 5 คน + แผง + ยอดรวม · ปุ่มบนการ์ด 121×44 · `ก่อนหน้า` disabled ที่หน้า 1 · กด `ถัดไป` เป็น `หน้า 2 / 5` `แสดง 6–10 จาก 24 รายการ` · 1440px วาดครบ 25 แถว แผงถูกซ่อน (เคยวัดตอนเป็นกล่อง: กล่องสูง 328px เนื้อใน 920px ปุ่ม 101×44) |
| `วันเกิดของเดือนนี้` อยู่ท้ายสุด | ต่อจาก `รวมทั้งหมด` ทันที และเป็นส่วนสุดท้ายของหน้า · ระยะจากยอดรวมไม่คงที่เท่าตอนเป็นกล่องแล้ว แต่แปรตามความสูงของการ์ด 5 ใบเท่านั้น ไม่ได้แปรตามจำนวนคนในเดือน | `app/styles.css` (`.hr-table tbody` · `.month-card` order) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 ในพิกัดหน้า: การ์ดใบแรก 842 → แผง 1707 → ยอดรวม 1792 → หัวข้อวันเกิด 1870 · ในจอขณะเห็นการ์ดใบที่ห้า: 120–302 → 314–387 → 399–453 → 477 (เคยวัดได้ y=483 ตอนอยู่นอกกล่องสูง 42dvh) |
| `รวมทั้งหมด` อยู่ใต้แผงเปลี่ยนหน้า | ต่ำกว่า 860px แถวยอดรวมเป็นการ์ดธรรมดาในสายเนื้อหา `static` วางต่อจากแผงเปลี่ยนหน้า · พื้น `--neutral-wash` ยังอยู่เพราะเป็นตัวบอกว่าแถวนี้เป็นของเดือน ไม่ใช่ของคน · ไม่มี `position`/`z-index`/เงาแล้ว เพราะไม่มีอะไรลอดใต้มันอีก · เคยเป็น `sticky` สองแบบ: ลอยเหนือแถบล่าง แล้วติดก้นกล่อง | `app/styles.css` (`.hr-table tbody tr.total-row` · `.table-wrap.card-list`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — `position` คำนวณออกมาเป็น `static` · ยิง hit-test กลางปุ่มเปลี่ยนหน้าทั้งสอง **ชี้กลับมาที่ปุ่มเอง** (เดิมตอนยอดรวมเป็น sticky มี 1 ใน 15 จุดเลื่อนที่มันบังกลางปุ่ม) |
| กดเปลี่ยนหน้าแล้วเลื่อนกลับไปหัวรายชื่อ | แผงเปลี่ยนหน้าอยู่ใต้การ์ดใบที่ห้า คนกดจึงกดโดยมีรายชื่อห้าใบอยู่เหนือหัวแม่มือ — ถ้าไม่เลื่อน การ์ดห้าใบใหม่จะถูกวาดเหนือจอที่ยังมองแผงอยู่ กดแล้วเหมือนไม่มีอะไรเกิดขึ้น · `goPage()` เรียก `scrollIntoView({ block: 'start' })` บนกล่องครอบรายชื่อ · จะหยุดต่ำแค่ไหนเป็นเรื่องของสไตล์ชีต — `scroll-margin-top: 156px` (แถบหัว 62 + ช่องค้นหา ~69 + บรรทัดที่โผล่ตอนค้นหา) · **อยู่ใน handler ไม่ใช่ `useEffect`** เพราะบน effect มันจะยิงตอน mount และทุกครั้งที่พิมพ์ในช่องค้นหา = จอกระโดดลงมาที่รายชื่อขณะคนกำลังพิมพ์ | `components/HrView.jsx` (`goPage` · `listRef`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — เลื่อนจนเห็นแผงแล้วกด `ถัดไป`: หน้าเลื่อนจาก 1000 → 674 · กล่องรายชื่ออยู่ที่ 156 ในจอ · การ์ดใบแรกของหน้า 2 ที่ 168 = ห่างขอบล่างช่องค้นหา (144) **24px** · `ก่อนหน้า` กลับมากดได้ (เดิมตอนเป็นกล่อง: `scrollTop` ค้างที่ 592 ก่อนแก้) |
