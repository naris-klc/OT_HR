...
| แถบแจ้งเตือนของเดือน | กล่องเดียว นับและระบุชื่อทุกข้อความ · กางเป็น **รายการในกล่องเดิม** (หัวข้อ · ตัวเลข · หนึ่งประโยค) ไม่ใช่กล่องใหม่ซ้อนใต้ · สีตามข้อความที่แรงที่สุด · ปุ่มกาง/พับและ ✕ อย่างละหนึ่งจุด · ✕ ปิดจนกว่าจะ refresh แล้วเหลือบรรทัด `แสดงแจ้งเตือนของเดือนนี้ (n)` | `components/HrView.jsx` (`MonthAlerts`) · `components/PolicyVersion.jsx` (`policyVersionNotice`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 มี `.alert` ใบเดียวทุกสถานะ ไม่มี `<details>` ข้างใน · พับ 103px (ถึงช่องค้นหา 144px) กาง 308px ปิดแล้ว 0px · ป้ายบนแถวอยู่ครบ |
| พับรายการบนมือถือ | ต่ำกว่า 860px แสดง 5 การ์ดแรก แล้วปุ่ม `แสดงทั้งหมด (n รายการ)` · กดซ้ำย่อกลับ · เดสก์ท็อปไม่มีปุ่มนี้ · ไม่ใช่ตัวกรอง — ยอดรวม/CSV/พิมพ์รวม ไม่อ่านค่านี้ | `components/HrView.jsx` (`CARD_FOLD`) · `app/styles.css` | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 เห็น 5 จาก 18 · กดแล้วครบ 18 · `รวมทั้งหมด` 83.5 เท่าเดิมทั้งสามจอ |
