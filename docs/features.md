...
| พับไว้ 5 คน คลี่ได้ทั้งเดือน (มือถือ) | ต่ำกว่า 860px รายชื่อเป็นการ์ด **ตั้งต้นพับไว้ 5 คน** · ถ้ามีมากกว่านั้นมีแถบปุ่มเต็มความกว้างต่อจากการ์ดใบที่ห้า `แสดงพนักงานเพิ่มอีก (+n รายชื่อ) ▾` กดแล้วคลี่ทั้งเดือนตรงจุดนั้น ปุ่มเปลี่ยนเป็น `ย่อรายการกลับ ▴` · ตัวเลขในป้ายมาจาก `shown.length` (ผลการค้นหา ไม่ใช่ทั้งเดือน) · ใช้ `aria-expanded` ไม่ใช่ live region เพราะป้ายของปุ่มเองคือคำประกาศ · **ไม่มีกล่องเลื่อนซ้อน** — `.hr-table tbody` ไม่มี `max-height`/`overflow-y` และ `.table-wrap.card-list` เป็น `overflow: visible` (จำเป็น เพราะ `overflow-x: auto` ทำให้แกน y กลายเป็น `auto` ตามไปด้วย) · เปลี่ยนเดือน/สถานะ/พิมพ์ค้นหา **พับกลับเสมอ** · แถวที่เกินห้าใส่คลาส `off-page` **ไม่ได้ `slice` อาร์เรย์** เพราะเดสก์ท็อปไม่มีแถบนี้ ถ้า slice เดือนจะเหลือ 5 คนบนจอใหญ่โดยไม่มีปุ่มไปหาที่เหลือ · **ราคาที่จ่าย:** แผงเปลี่ยนหน้าที่ถูกแทนเคยการันตีว่าไม่มีเดือนไหนยาวเกิน 5 การ์ด อันนี้ไม่การันตี — คลี่แล้วยาวเท่าเดือน เขียนไว้เหนือ `CARD_FOLD` | `components/HrView.jsx` · `app/styles.css` (`.hr-table tbody` · `.fold-row` · `.off-page`) | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-26 — 360×780 เดือนที่ปั้นไว้ 25 คน (มีใบอนุมัติ 24 พับไว้ 19) · **พับ:** วาด 5 การ์ด ซ่อน 19 · ป้าย `แสดงพนักงานเพิ่มอีก (+19 รายชื่อ) ▾` ปุ่ม 280×44 `aria-expanded="false"` ยิง hit-test กลางปุ่มชี้กลับมาที่ปุ่มเอง · หน้าสูง 3832px · ไล่ทุก element ที่ `overflow-y` เลื่อนได้ **ได้ลิสต์ว่าง** · **คลี่:** วาด 24 การ์ด ซ่อน 0 ป้ายเป็น `ย่อรายการกลับ ▴` `aria-expanded="true"` หน้าสูง 7535px · **1440px:** วาดครบ 24 การ์ดทั้งที่ 19 แถวยังมีคลาส `off-page` (ไม่มีกฎไหนอ่าน) แถบถูกซ่อน |
| `วันเกิดของเดือนนี้` อยู่ท้ายสุด | ต่อจาก `รวมทั้งหมด` ทันที และเป็นส่วนสุดท้ายของหน้า · ตอนพับอยู่ห่างจากช่องค้นหาเท่าความสูงของการ์ด 5 ใบเสมอ · ตอนคลี่ถูกดันลงไปเท่ากับความสูงของการ์ดที่เพิ่มมา ไม่มีอะไรอื่นขยับ | `app/styles.css` (`.hr-table tbody` · `.month-card` order) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-26 — พับ: การ์ดใบแรก 842 → ใบที่ห้า 1513 → แถบ 1707 → ยอดรวม 1763 → หัวข้อวันเกิด 1841 · คลี่: แถบ 5410 → ยอดรวม 5466 → วันเกิด 5543 การ์ดใบแรกยังอยู่ที่ 842 เท่าเดิม (เคยวัดตอนมีแผงเปลี่ยนหน้า: ยอดรวม 1792 วันเกิด 1870) |
| `รวมทั้งหมด` อยู่ใต้แถบพับ/คลี่ | ต่ำกว่า 860px แถวยอดรวมเป็นการ์ดธรรมดาในสายเนื้อหา `static` วางต่อจากแถบ · พื้น `--neutral-wash` ยังอยู่เพราะเป็นตัวบอกว่าแถวนี้เป็นของเดือน ไม่ใช่ของคน · ไม่มี `position`/`z-index`/เงาแล้ว เพราะไม่มีอะไรลอดใต้มันอีก · เคยเป็น `sticky` สองแบบ: ลอยเหนือแถบล่าง แล้วติดก้นกล่อง | `app/styles.css` (`.hr-table tbody tr.total-row` · `.table-wrap.card-list`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-26 — `position` คำนวณออกมาเป็น `static` ทั้งตอนพับและตอนคลี่ · ถ้ายังลอยอยู่ มันจะทับทั้งส่วนวันเกิดและการ์ด 19 ใบที่คลี่ออกมา |
| กดย่อรายการแล้วเลื่อนกลับไปหัวรายชื่อ | ตอนคลี่ไม่ต้องเลื่อนอะไร การ์ดใหม่โผล่ใต้ปุ่มซึ่งเป็นที่ที่คนกำลังมองอยู่ · ตอน**ย่อ**หน้าหดจาก 60 การ์ดเหลือ 5 ใต้คนที่เลื่อนลงไปไกลแล้ว เอกสารจบเหนือหัวเขา เบราว์เซอร์จะทิ้งไว้ที่ก้นหน้าใหม่ (ปกติคือส่วนวันเกิด) โดยที่รายชื่อที่เพิ่งย่อไม่อยู่ในจอเลย · `toggleFold()` จึงเรียก `scrollIntoView({ block: 'start' })` บนกล่องครอบรายชื่อเฉพาะขาย่อ · จะหยุดต่ำแค่ไหนเป็นเรื่องของสไตล์ชีต — `scroll-margin-top: 156px` (แถบหัว 62 + ช่องค้นหา ~69 + บรรทัดที่โผล่ตอนค้นหา) · **อยู่ใน handler ไม่ใช่ `useEffect`** เพราะบน effect มันจะยิงตอน mount และทุกครั้งที่พิมพ์ในช่องค้นหา = จอกระโดดลงมาที่รายชื่อขณะคนกำลังพิมพ์ | `components/HrView.jsx` (`toggleFold` · `listRef`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-26 — ย่อจากก้นรายการที่คลี่ไว้ (ตำแหน่งเลื่อนหน้า 5103 ตำแหน่งที่ยอดรวมอยู่ในจอ): หน้ากลับมา 3832px และคนอ่านมาอยู่ที่ตำแหน่งเลื่อนหน้า 674 · กล่องรายชื่ออยู่ที่ 156 ในจอ · การ์ดใบแรกที่ 168 = ห่างขอบล่างช่องค้นหา (144) **24px** |
