...
| แบ่งหน้ารายการบนมือถือ | ต่ำกว่า 860px แสดง **5 คนต่อหนึ่งหน้า** · ใต้การ์ดคนที่ 5 มีบรรทัด `แสดง 1–5 จาก 25 รายการ` แล้วแผงเปลี่ยนหน้า `[ ‹ ก่อนหน้า ] หน้า 1 / 5 [ ถัดไป › ]` · ปุ่มหัวท้าย `disabled` ไม่ใช่ซ่อน (ซ่อนแล้วปุ่มอีกอันจะเลื่อนมาอยู่ใต้นิ้วพอดี) · `aria-live="polite"` ครอบบรรทัดจำนวนกับเลขหน้า · หน้าที่หายไปเพราะรายการสั้นลงถูก clamp ตอน render ไม่ใช่วาดหน้าว่าง · เปลี่ยนเดือน/สถานะ/พิมพ์ค้นหา กลับหน้า 1 เสมอ · เดสก์ท็อปไม่มีแถวนี้ · ไม่ใช่ตัวกรอง — ยอดรวม/CSV/พิมพ์รวม ไม่อ่านค่านี้ | `components/HrView.jsx` (`CARD_PAGE`) · `app/styles.css` (`.off-page` `.pager-row`) | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 ครบ 5 หน้า หน้าละ 5 คน · **25 คนไม่ซ้ำไม่ขาด** · ปุ่ม 106×44 · `ก่อนหน้า` disabled ที่หน้า 1 · `ถัดไป` disabled ที่หน้า 5 · พิมพ์ `PM-09` กลับไป `หน้า 1 / 2` · ค้นเจอคนเดียวไม่มีแผงเลย · 1440px วาดครบ 25 แถวและไม่มีแผง · เดือนสิงหาคมของฐานจริงมีแค่ 4 คน จึงต้องเติมใบตัวอย่างในสำเนา (เคยเป็น "โหลดเพิ่มทีละ 10" และก่อนหน้านั้น "เห็น 5 จาก 18" ในวันเดียวกัน) |
| ความยาวหน้าจอไม่ขึ้นกับจำนวนคน | เพราะแบ่งหน้า ไม่ใช่ต่อรายการ — `วันเกิดของเดือนนี้` อยู่ใต้การ์ดที่ 5 ตำแหน่งเดิมทุกหน้า ไม่ถูกดันตกจอเมื่อรายการยาวขึ้น | `components/HrView.jsx` · `app/styles.css` (`.month-card` order) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — วัดจากบนสุดของเอกสาร y≈1775 ที่หน้า 1, 2, 4 · y≈1797 และ 1819 ที่สองหน้าที่มีการ์ดพก `เพดานนับ` เพิ่มมาหนึ่งบรรทัด |
| `รวมทั้งหมด` ลอยเหนือแถบล่าง | ต่ำกว่า 860px แถวยอดรวมเป็น `position: sticky` ห่างจากแถบล่าง 8px (บวก safe-area) · ลอยอยู่เหนือ **การ์ด** เท่านั้น พอหมดรายการก็ลงไปนั่งที่เดิม ไม่ลอยทับวันเกิด/เชิงอรรถ · อยู่ใต้แผงเปลี่ยนหน้า จึงบังปุ่มได้เฉพาะช่วงเลื่อนแคบ ๆ · เดสก์ท็อปเป็น `static` เหมือนเดิม | `app/styles.css` (`.hr-table tbody tr.total-row` · `.table-wrap.card-list`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-25 — 360×780 กลางรายการวัดได้ `bottom: 698` ขณะแถบล่างอยู่ที่ `top: 703` · 1440px เป็น `static` · วัดซ้ำหลังเปลี่ยนเป็นแบ่งหน้า: เลื่อนทีละ 40px จำนวน 28 จุด มี **2 จุด** ที่ยอดรวมบังกลางปุ่มเปลี่ยนหน้า เลื่อนต่อก็พ้น — จงใจปล่อยไว้ เหตุผลอยู่ใน README |
