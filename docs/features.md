...
| กล่องรายชื่อ + แผงเปลี่ยนหน้า บนมือถือ | ต่ำกว่า 860px รายชื่ออยู่ในกล่องสูง `42dvh` (ขั้นต่ำ 260px) เลื่อนดูข้างในได้ · ในกล่องเรียงเป็น การ์ด → แผงเปลี่ยนหน้า → `รวมทั้งหมด` (sticky ที่ก้นกล่อง) · หน้าละ 5 คน · **แผงเปลี่ยนหน้าแสดงทุกเดือน** ตั้งแต่ 2026-08-26 รวมถึงเดือนที่มีไม่ถึง 5 คน — ปุ่มหัวท้าย `disabled` และบรรทัด `แสดง 1–4 จาก 4 รายการ` ยังบอกความยาวของรายการอยู่ (เดิมมีเงื่อนไข `shown.length > CARD_PAGE` ทำให้เดือนสี่คนไม่มีแผงเลย) · `overscroll-behavior: contain` กันไม่ให้ปัดจนหลุดไปเลื่อนหน้าข้างหลัง · `pageCount` มีพื้นเป็น 1 จึงไม่มี `หน้า 1 / 0` · หน้าที่หายไปเพราะรายการสั้นลงถูก clamp ตอน render · เปลี่ยนเดือน/สถานะ/พิมพ์ค้นหา กลับหน้า 1 · **สองกลไกต่อหนึ่งรายการ** (เลื่อนในกล่อง กับ เปลี่ยนหน้า) ซึ่งเป็นราคาที่จ่ายโดยตั้งใจ เขียนเหตุผลไว้เหนือ `CARD_PAGE` ใน `components/HrView.jsx` · เดสก์ท็อปซ่อนเฉพาะ `.pager-row` ส่วน `.off-page` **ไม่มีกฎเลย** จึงวาดครบทุกแถว | `components/HrView.jsx` · `app/styles.css` (`.hr-table tbody` · `.pager-row` · `.off-page`) | `hrMonthCards` `monthSearch` | ✅ เดินบนสำเนาฐาน 2026-08-26 — 360×780 เดือนที่ปั้นไว้ 25 คน (มีใบอนุมัติ 24) · กล่องสูง 328px เนื้อใน 1083px และเป็น **element เดียวในหน้าที่ `overflow-y` เลื่อนได้จริง** · ในกล่องมี 7 แถว = 5 คน + แผง + ยอดรวม · `แสดง 1–5 จาก 24 รายการ` `หน้า 1 / 5` `ก่อนหน้า` disabled · ปุ่มบนการ์ด 116×44 · เลื่อนกล่องจนสุด (scrollTop 755) ยิง hit-test กลางปุ่มทั้งสองชี้กลับมาที่ปุ่มเอง · กด `ถัดไป` เป็น `หน้า 2 / 5` `แสดง 6–10 จาก 24 รายการ` และกล่องกลับไปบนสุด · ค้นหาจนเหลือ 2 คน แผงยังอยู่ `แสดง 1–2 จาก 2 รายการ` `หน้า 1 / 1` **ปุ่ม disabled ทั้งคู่** · 1440px ไม่มีกล่อง วาดครบ 24 การ์ด แผงถูกซ่อน ยอดรวม `static` |
| `วันเกิดของเดือนนี้` ไม่ตกจอ | อยู่นอกกล่อง ต่อจากกล่องทันที และเพราะกล่องสูงคงที่ ส่วนนี้จึงอยู่ที่เดิมเสมอไม่ว่าจะมีพนักงานกี่คนหรืออยู่หน้าไหน | `app/styles.css` (`.hr-table tbody` · `.month-card` order) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-26 — 360×780 อยู่ใต้ก้นกล่อง 12px ที่ y=489 ในจอ ขณะแถบล่างอยู่ y=703 · **กด `ถัดไป` แล้วยังอยู่ที่ 489 เท่าเดิม** · ค้นหาจนเหลือ 2 คนก็ยังห่างก้นกล่อง 12px · (ตอนเอากล่องออกเมื่อ 2026-08-26 วัดได้ 1,841px ลงไปในเอกสารตอนพับ และ 5,543px ตอนคลี่ — นี่คือเหตุผลที่กล่องกลับมา) |
| `รวมทั้งหมด` เป็นพื้นของกล่อง | ต่ำกว่า 860px แถวยอดรวมเป็น `position: sticky; bottom: 0` ติดก้นกล่อง เห็นตลอดไม่ว่าจะเลื่อนไปแถวไหนหรืออยู่หน้าไหน · เงาชี้ขึ้นเพราะเป็นพื้น ไม่ใช่แถบลอยทับ · เดสก์ท็อปเป็น `static` | `app/styles.css` (`.hr-table tbody tr.total-row` · `.table-wrap.card-list`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-26 — `position` คำนวณออกมาเป็น `sticky` ติดก้นกล่องทุกตำแหน่งการเลื่อน · **มี 2 ใน 19 จุดเลื่อน (ทีละ 40px บนสกรอลล์ 755px) ที่มันบังกลางปุ่มเปลี่ยนหน้า** เลื่อนต่ออีกนิดก็พ้น และตรงตำแหน่งที่แผงมาหยุดจริง ๆ (สุดกล่อง) ปุ่มกดได้ทั้งคู่ · ระหว่างค้นหาแถวนี้เปลี่ยนเป็น `รวมทั้งเดือน` + `ไม่ใช่ยอดของผลการค้นหา` |
| กดเปลี่ยนหน้าแล้วกลับไปบนสุดของกล่อง | แผงเปลี่ยนหน้าอยู่ก้นกล่อง คนกดจึงกดตอนเลื่อนสุดแล้ว — ถ้าไม่รีเซ็ต `scrollTop` การ์ดห้าใบใหม่จะถูกวาดเหนือจอที่ยังมองแผงอยู่ กดแล้วเหมือนไม่มีอะไรเกิดขึ้น · รีเซ็ตเมื่อ `current`/`find`/เดือน/สถานะ เปลี่ยน · เป็น ref ไม่ใช่คำถามเรื่อง layout — เดสก์ท็อปไม่มีกล่อง `scrollTop` เป็น 0 อยู่แล้ว | `components/HrView.jsx` (`listRef`) | `hrMonthCards` | ✅ เดินบนสำเนาฐาน 2026-08-26 — เลื่อนกล่องจนสุด (scrollTop 755) แล้วกด `ถัดไป` กล่องกลับมาที่ 0 และหน้าเว็บไม่ขยับ (pageScrollY 680 เท่าเดิม) · **เจอบั๊กนี้จากการเดิน ไม่ใช่จากการอ่านโค้ด** ก่อนแก้ ตำแหน่งเลื่อนของกล่องค้างที่ 592 หลังกด `ถัดไป` |
