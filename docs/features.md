...
| กล่องแนะนำใต้ช่องค้นหา | ขณะพิมพ์มีกล่องลอยเปิดใต้ช่อง หนึ่งแถวต่อหนึ่งคนที่ตรง — บรรทัดบน `ชื่อ (รหัส)` ไฮไลต์ทั้งคู่ บรรทัดล่าง `แผนก | n ชม.` · ใช้ `hours()` ไม่ใช่ `cell()` เพราะศูนย์ในคำแนะนำคือคำตอบ ส่วนช่องว่างในตารางคือคอลัมน์ที่อ่านผ่าน · **ใช้แผงเดียวกับกรองตามพนักงาน** `.pick-menu` + ตัวขยาย `.find-menu` สำหรับแถวสองบรรทัด `role="combobox"` บน input `role="listbox"` บนกล่อง ↑↓ เดิน Enter เลือก Escape/Tab ปิด และแถวที่คีย์บอร์ดยืนอยู่เดินตามเมาส์ จึงมีนิยาม "แถวปัจจุบัน" เดียว · ทุกกฎ CSS ระบุสองคลาส เพราะบล็อก `.pick-menu` อยู่ล่างกว่าในไฟล์ กฎคลาสเดียวจะแพ้ตามตำแหน่ง (กับดักเดียวกับ `.dept-menu`) · `max-height: 264px` `overflow-y: auto` มาจาก `.pick-menu` อยู่แล้ว และบนมือถือถูกครอบด้วย `min(264px, 46vh)` เพราะแถวสองบรรทัดเต็มเร็วกว่า · **ไม่มีผลลัพธ์ = ไม่วาดอะไรเลย** เพราะการ์ดข้างล่างพูด `ไม่พบพนักงานที่ค้นหา` พร้อมทางออกอยู่แล้ว แผงลอยที่พูดซ้ำจะไปบังปุ่มที่ตอบมันพอดี · เนื้อในกล่องมาจาก `narrowed` จึงไม่มีทางแนะนำคนที่ตารางไม่ได้แสดง | `components/AccountingView.jsx` · `app/styles.css` (`.pick-menu.find-menu`) | `monthSearch` | ✅ เดินบนแอป 2026-08-26 — 360px กล่องกว้าง 304 ห่างช่อง 4px `max-height` 264 `overflow-y: auto` `z-index: 5` · สามรายการ `วิชัย ศรีสุข (PM-0100) วิศวกรรม \| 8 ชม.` ฯลฯ มีมาร์ก `สุ ส สุ สิ` · `aria-expanded=true` `aria-activedescendant` ชี้แถวแรก · 1280px กล่องกว้าง 831 พฤติกรรมเดียวกัน |
