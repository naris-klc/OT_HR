  ...
  return (
    <div className="acct">
      ...
      {/* Four ruled columns, and the white strip beside them.
          ...
          194mm เท่าเดิมทั้งสองโหมด: 22 + 57 + หกช่องอัตราที่ 14mm + แถบขาว 31mm
          แบนเนอร์เดือนย่อคร่อมสองช่องได้ 28mm ส่วน `พ.ย. 69` ที่ 10.5pt กว้าง
          12.96mm — เหลือเฟือ และเป็นเหตุผลที่หัวเดือนของใบสองเดือนยังต้องย่อ
          (ดู `monthHead`)

          ── ทุกตัวเลขข้างบนถูกคำนวณใหม่ที่ 10.5pt เมื่อ 2026-09-14 ───────────
          ชุดเดิมคือ `22/50/12×6/50` และ `24/54/17/17/82` ซึ่งวัดไว้ที่ 9pt ·
          ตัวอักษรทั้งใบโตขึ้น 2px ตามที่ขอมา และความกว้างข้อความโตตามขนาดฟอนต์
          แบบเชิงเส้นพอดี สามช่องจึงคับขึ้นมาพร้อมกัน (วัดด้วย Chrome วันเดียวกัน
          ฟอนต์เดียวกับที่ใบนี้ใช้):

            ชื่อยาวที่สุดในทะเบียน  44.9mm ที่ 9pt → 52.4mm  ช่องเดิม 54mm ให้เนื้อที่ 51mm
            `เดือนพฤศจิกายน 69`     28.96mm       → 33.78mm  แบนเนอร์เดิม 34mm ให้ 31mm
            `120.00` (ยอดรวมสองเดือน) 10.39mm     → 12.12mm  ช่องเดิม 12mm ให้ 9mm

          ที่แก้จึงเป็นความกว้าง ไม่ใช่ขนาดตัวอักษร: ชื่อได้ 58/57mm (เนื้อที่
          55/54mm — เผื่อจากชื่อที่ยาวที่สุด 2.6mm และ 1.6mm) · ช่องอัตราใบเดือนเดียวได้ 19mm ให้แบนเนอร์ 38mm · ช่องอัตรา
          ใบสองเดือนได้ 14mm คู่กับ padding ข้าง 0.8mm ใน print.css · ส่วนที่จ่าย
          ให้ทั้งหมดคือแถบขาว ซึ่งเป็นคอลัมน์เดียวในใบที่ยืดได้โดยไม่มีตัวเลข
          เคลื่อน — คำว่าวันเกิดที่ 10.5pt กว้าง 9.78mm ใน 74/31mm ยังเหลือที่ให้
          เขียนด้วยมืออีกมาก

          ⚠ ทั้งสองชุดต้องรวมได้ 194mm พอดีเสมอ (`.acct table` ใน print.css)
          และห้ามมีช่องไหนแคบจนหัวตารางตกบรรทัด — แถวหัวที่สูงขึ้นหนึ่งบรรทัด
          ทำให้ `ROWS_PER_PAGE` ข้างบนผิดทั้งใบ ไม่ใช่แค่หน้าเดียว */}
      <table>
        <colgroup>
          {many ? (
            <>
              ...
              {periods.flatMap((p) => [
                <col key={`${p}-15`} style={{ width: '14mm' }} />,
                <col key={`${p}-3`} style={{ width: '14mm' }} />,
              ])}
 ...
 * สองช่องที่ 14mm คือ 28mm ส่วน `เดือนพฤศจิกายน 69` ที่ 10.5pt กว้าง 33.78mm
 * มันจะตกบรรทัด แถวหัวสูงขึ้น และ `ROWS_PER_PAGE` ข้างบน — ที่นับจากหน้าซึ่งมี
 * หัวสองแถวพอดี — ก็ผิดทั้งใบ ทุกหน้าหลังหน้าแรกจะจบผิดที่
 *
 * (อ่านว่า "สองช่องที่ 12mm คือ 24mm ส่วน … ที่ 9pt กว้างราว 30mm" จนถึง
 * 2026-09-14 ตอนตัวอักษรทั้งใบโตขึ้น 2px ช่องว่างที่เหลือกว้างขึ้น 4mm และ
 * ตัวหนังสือกว้างขึ้น 4.8mm — เหตุผลของการย่อจึงไม่ได้หายไปไหน มันแคบลง)
 *
 * ใบเดือนเดียวยังพาดหัวเต็มเหมือนเดิมทุกตัวอักษร มันมี 38mm ให้ใช้ (สองช่องที่
 * 19mm) เหลือเนื้อที่ 35mm หลัง padding ซึ่งพอดีกับ 33.78mm ของเดือนที่ยาวที่สุด
 * และเป็นใบที่ออกทุกเดือน · `shortMonth` อยู่ใน lib/accountingCycle.js ที่เดียวกับที่หัว
