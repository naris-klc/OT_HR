...
function Shell({ session, onRefresh, onLogout }) {
  ...
  useEffect(() => {
    function onPop() {
      ...
      /* ที่อยู่ที่ตรงกับจออยู่แล้ว ห้ามเขียนทับ — การกดหัวข้อในสารบัญคู่มือเข้า
         มาทางนี้ทุกครั้ง เพราะลิงก์ `#sec-...` ทำให้เบราว์เซอร์ยิง popstate และ
         `#sec-approve` ก็คือแท็บ `manual` จริง ๆ (ดู `tabFromHash`) ถ้าเขียนทับ
         เป็น `#manual` `:target` จะดับและที่อยู่ที่ส่งต่อกันได้ก็หายไปพร้อมกัน */
      if (next === tab) { if (tabFromHash() !== tab) writeHash(tab, false); return; }
...
function tabFromHash() {
  ...
  /**
   * `#sec-approve` คือหัวข้อในคู่มือ ไม่ใช่ hash ของคนอื่น
   *
   * สารบัญของคู่มือเป็นลิงก์จริง (`components/ManualView.jsx` — `#sec-<คีย์>`
   * เป็นที่อยู่ที่ส่งต่อกันได้ และเบราว์เซอร์เป็นคนเลื่อนเอง) ซึ่งมีมาก่อน
   * "แท็บอยู่ใน URL" · วันแรกที่ทั้งสองอย่างอยู่ในแอปเดียวกัน (2026-09-11) การ
   * กดหัวข้อในสารบัญเด้งผู้อ่านออกจากคู่มือทันที: hash กลายเป็น `sec-admin`
   * ซึ่งไม่ผ่าน `/^[a-z]+$/` ข้างล่าง `tabFromHash` จึงคืน `null` แล้ว
   * `popstate` ก็พาไปหน้าเริ่มต้นของบทบาท — ที่อยู่ที่ผู้ใช้รายงานคือ `#admin`
   *
   * หัวข้อในคู่มือจึงเป็น "แท็บ `manual` ที่เลื่อนไปหยุดตรงหัวข้อนั้น" และนี่คือ
   * ที่เดียวที่ hash พกอะไรมากกว่าชื่อแท็บ — `:target` ในสไตล์ชีตต้องการชื่อ
   * หัวข้อดิบ ๆ จึงเติมชื่อแท็บนำหน้าไม่ได้
   */
