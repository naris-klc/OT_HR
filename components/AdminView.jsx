...
const POLICY_FIELDS = [
  ...
  {
    key: 'birthdayReasonOnForm', label: 'ใบพิมพ์ F-HR-027 ระบุว่าวันนั้นเป็นวันเกิด', bool: true,
    options: [
      [true, 'ใช่ — ตารางในใบขึ้นเป็นวันหยุด พร้อมหมายเหตุ “วันเกิด” (ค่าเริ่มต้น)'],
      [false, 'ไม่ — ตารางในใบใช้ปฏิทินวันหยุดบริษัทอย่างเดียว'],
    ],
    hint: 'ไม่กระทบชั่วโมงในช่องใดเลย เปลี่ยนแล้วไม่มีการคำนวณใหม่ '
      + '· ถ้าปิด ตารางจะขึ้นว่าเป็นวันทำงานทั้งที่ชั่วโมงลงช่องวันหยุด ซึ่งขัดกันเองบนใบที่หัวหน้าต้องเซ็น '
      + '· ใบนี้เป็นของเจ้าตัว ไปถึงหัวหน้า HR และบัญชี — วันเกิดไม่ขึ้นในรายงานรวม '
      + 'และไม่ถูกเขียนลงปฏิทินวันหยุดบริษัท ไม่ว่าตั้งค่าไว้อย่างไร',
  },
  {
